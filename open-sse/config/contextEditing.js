 ...
 */
export function applyContextEditingToBody(body, opts) {
  ...
  if (!hasToolUseEdit) {
    edits.push({
      ...
      keep: {
        type: "tool_uses",
        value: CONTEXT_EDITING_DEFAULT_KEEP_TOOL_USES
      }
    });
 ...
 * fallback.
 */
export function extractContextEditingTelemetry(responseBody) {
  if (!responseBody || typeof responseBody !== "object") return null;
  const candidates = [getNested(responseBody, ["context_management", "applied_edits"]), getNested(responseBody, ["usage", "context_management", "applied_edits"]), getNested(responseBody, ["usage", "applied_edits"])];
