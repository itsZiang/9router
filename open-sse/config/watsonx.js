...
export function normalizeWatsonxBaseUrl(value) {
  const normalized = normalizeBaseUrl(value || WATSONX_DEFAULT_BASE_URL);
  if (!normalized) return WATSONX_DEFAULT_BASE_URL;
  const stripped = normalized.replace(/\/(?:chat\/completions|completions|embeddings|models)$/i, "");
...
export function buildWatsonxChatUrl(value) {
  return `${normalizeWatsonxBaseUrl(value)}/chat/completions`;
}
export function buildWatsonxModelsUrl(value) {
