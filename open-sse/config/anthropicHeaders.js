...
export const ANTHROPIC_BETA_API_KEY = ANTHROPIC_BETA_BASE.filter(beta => beta !== "oauth-2025-04-20").join(",");
export const ANTHROPIC_BETA_CLAUDE_OAUTH = [...ANTHROPIC_BETA_BASE.slice(0, 3), ...CLAUDE_OAUTH_EXTRA_BETAS, ...ANTHROPIC_BETA_BASE.slice(3)].join(",");
 ...
 */
export function mergeClientAnthropicBeta(base, clientBeta, allow = FORWARDABLE_CLIENT_BETAS) {
...
function uniqueCommaValues(values) {
  return [...new Set(values.filter(value => value !== undefined && value !== null && value !== "").flatMap(value => String(value).split(",")).map(value => value.trim()).filter(Boolean))];
}
 ...
 */
export function normalizeAnthropicHeaderVariants(headers) {
