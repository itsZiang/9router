...
export const PROVIDER_MAX_TOKENS = {
  groq: 16384,
  // Groq strict per-model enforcement
  openai: 16384,
  // GPT-4/4o standard
  anthropic: 65536,
  // Claude models
...
export { BACKOFF_CONFIG, COOLDOWN_MS, DEFAULT_ERROR_MESSAGES, ERROR_RULES, ERROR_TYPES, TRANSIENT_COOLDOWN_MS, calculateBackoffCooldown, findMatchingErrorRule, getDefaultErrorMessage, getErrorInfo, matchErrorRuleByStatus, matchErrorRuleByText } from "./errorConfig";
...
export const RateLimitReason = {
  ...
  // Daily/monthly quota depleted
  RATE_LIMIT_EXCEEDED: "rate_limit_exceeded",
  // RPM/RPD limits hit
  MODEL_CAPACITY: "model_capacity",
  // Model overloaded (529, 503)
...
export const PROVIDER_PROFILES = {
  oauth: {
    ...
    // 5s (session tokens — short recovery)
    rateLimitCooldown: 60000,
    // 60s default when no retry-after header
    maxBackoffLevel: 8,
    // Higher ceiling (sessions may stay bad longer)
    ...
    // Scaled for 500+ connections (was 3)
    providerFailureWindowMs: 900000,
    // 15min window (was 10min)
    providerCooldownMs: 300000,
    // 5min cooldown when threshold reached
    // Adaptive circuit breaker v2 settings
    degradationThreshold: 5,
    // Enter DEGRADED at this many failures
    maxBackoffMultiplier: 8,
    // Max 8x resetTimeout escalation
    backoffEscalationCount: 2 // Escalate after 2 open cycles
  },
  apikey: {
    transientCooldown: 3000,
    // 3s (API providers recover faster)
    rateLimitCooldown: 0,
    // 0 = respect retry-after header from provider
    maxBackoffLevel: 5,
    // Lower ceiling (API quotas reset at known intervals)
  ...
  local: {
    ...
    // 2s (local — very fast recovery)
    rateLimitCooldown: 5000,
    // 5s (local — no real rate limits)
    maxBackoffLevel: 3,
    // Low ceiling (local either works or doesn't)
    ...
    // 2 failures trigger provider cooldown
    providerFailureWindowMs: 300000,
    // 5min window for counting failures
...
export const DEFAULT_API_LIMITS = {
  ...
  // 60 RPM (reduced from 100 — saves Bottleneck queue memory)
  minTimeBetweenRequests: 350,
  // 350ms minimum gap (increased from 200)
