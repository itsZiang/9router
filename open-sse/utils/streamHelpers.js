...
import { hasAnyReasoningSignal } from "./reasoningFields";
function isRecord(value) {
  return !!value && typeof value === "object" && !Array.isArray(value);
}
export function parseSSEDataPayload(data, options = {}) {
  ...
  try {
    ...
    if (options.logWarning !== false && payload.length > 0) {
      console.log(`[WARN] Failed to parse SSE payload (${payload.length} chars): ${payload.substring(0, 200)}...`);
    }
    return null;
  }
}
export function parseSSEDataLines(dataLines, options = {}) {
...
export function createSSEDataLineNormalizer() {
  ...
  const hasSelfDescribingPendingDataPayload = () => {
    ...
    const parsed = dataLines.length > 0 ? parseSSEDataLines(dataLines, {
      logWarning: false
    }) : null;
  ...
  const flush = output => {
    ...
    if (dataLines.length > 0) {
      const parsed = parseSSEDataLines(dataLines, {
        logWarning: false
      });
      if (parsed) {
...
export function createSSEEventPrefixBuffer() {
  let lines = [];
  let emitted = false;
...
export function isKnownNonClaudeStreamPayload(parsed, eventType = "") {
