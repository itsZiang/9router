...
export function __setProxyFallbackTestHooks(hooks) {
...
export async function getProxyCandidates(targetUrl) {
  ...
  // 3. Top 5 1proxy marketplace proxies
  try {
    const oneproxyProxies = await listOneproxyProxies({
      limit: 5
    });
 ...
 */
export async function testSingleProxy(proxyUrl, targetUrl, timeoutMs = 3000) {
...
export async function testProxiesAgainstTarget(targetUrl, proxyUrls) {
  ...
  const results = await Promise.allSettled(proxyUrls.map(async proxyUrl => {
...
export async function findWorkingProxy(targetHostname, targetUrl) {
  ...
  const results = await Promise.allSettled(candidates.map(async proxyUrl => {
    const {
      ok
    } = await (proxyFallbackTestHooks?.testSingleProxy ?? testSingleProxy)(proxyUrl, targetUrl);
    return {
      proxyUrl,
      ok
    };
 ...
 */
export async function selectWorkingProxyFallback(_connectionId) {
