 ...
 */
export function computeHeapPressureThresholdMb(heapSizeLimitMb, override) {
 ...
 */
export const HEAP_PRESSURE_THRESHOLD_MB = computeHeapPressureThresholdMb(v8.getHeapStatistics().heap_size_limit / (1024 * 1024), process.env.HEAP_PRESSURE_THRESHOLD_MB);
 ...
 */
export function checkHeapPressureGuard(heapUsedMb, thresholdMb = HEAP_PRESSURE_THRESHOLD_MB) {
  if (heapUsedMb <= thresholdMb) return null;
  console.warn(`[chatCore] heap pressure guard tripped: ${Math.round(heapUsedMb)}MB > ${thresholdMb}MB; returning 503`);
  return {
    success: false,
    status: 503,
