...
export function isNoThinkingAlias(modelId) {
...
export function stripNoThinkingAlias(modelId) {
...
export function toNoThinkingAlias(qualifiedModelId) {
 ...
 */
export function applyNoThinkingAlias(body, opts = {}) {
  if (!body || typeof body !== "object") return {
    applied: false
  };
  const model = body.model;
  if (!isNoThinkingAlias(model)) return {
    applied: false
  };
  const realModel = stripNoThinkingAlias(model);
  if (!realModel) return {
    applied: false
  }; // malformed: nothing after the prefix
 ...
 */
export function shouldExposeNoThinkingAlias(model) {
 ...
 */
export function appendNoThinkingVariants(models, aliasToCanonical) {
