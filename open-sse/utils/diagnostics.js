 ...
 */
export function reportMalformed200(opts) {
  const {
    mode,
    provider,
    model,
  ...
  console.log(`[MALFORMED-200] mode=${mode || "?"} provider=${provider || "?"} model=${model || "?"} ` + `conn=${connectionId || "-"} reason=${reason || "empty"} recvBytes=${recvBytes ?? -1} ` + `recvLines=${recvLines ?? -1} emitted=${emitted ?? -1} events=${evtStr} ` + `ttft=${ttftMs ?? -1}ms dur=${elapsedMs ?? -1}ms`);
}
...
export function synthOpenAIErrorChunk(opts) {
  ...
  const safeMessage = sanitizeErrorMessage(`[${providerPart}] returned an empty response (${reasonText}). ` + "Likely quota exhaustion, an overloaded upstream, or a proxy/gateway intercepting the stream.");
  const body = {
...
export function synthResponsesFailure(reason) {
  const safeMessage = sanitizeErrorMessage(`stream closed before response.completed (${describeReason(reason)})`);
  const event = {
 ...
 */
export function detectMalformedNonStream(resp) {
