import { extractUsage } from "./usageTracking";
import { parseSSEDataPayload } from "./streamHelpers";
...
import { getAnyReasoningValue } from "./reasoningFields";
...
function handleResponsesTailPayload(parsed, output, context) {
  ...
  if (parsed.type === "response.function_call_arguments.delta") {
    ...
    const pending = pendingKey ? context.passthroughResponsesPendingFunctionCalls.get(pendingKey) : undefined;
  ...
  if (parsed.type === "response.completed" && Array.isArray(asRecord(parsed.response).output) && asRecord(parsed.response).output.length > 0) {
    pushUniqueResponsesOutputItems(context.passthroughResponsesOutputItems, asRecord(parsed.response).output);
  }
  if (parsed.type === "response.completed" && context.passthroughResponsesPendingFunctionCalls.size > 0) {
    pushUniqueResponsesOutputItems(context.passthroughResponsesOutputItems, [...context.passthroughResponsesPendingFunctionCalls.values()]);
  ...
  const backfilled = backfillResponsesCompletedOutput(outputPayload, context.passthroughResponsesOutputItems);
...
export function processBufferedPassthroughLine(line, context) {
  ...
  if (/^event:/i.test(trimmed)) {
    ...
    if (context.shouldAbortOnClaudeLifecycle({
      type: eventType
    })) {
