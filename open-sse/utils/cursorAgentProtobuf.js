...
import { WT_VARINT, WT_LEN, encodeVarint, encodeTag, encodeBytes, encodeString, encodeMessage, encodeUInt32Field, encodeBoolField, encodeDoubleField, decodeVarint, checkedLen, decodeFields, decodeStringField, decodeVarintField } from "./cursorAgentProtobuf/wire";
...
export function wrapConnectFrame(payload, compressed = false) {
...
export function* iterateConnectFrames(stream) {
  let pos = 0;
 ...
 */
export function resolveRequestedModel(modelId) {
 ...
 */
export function encodeSelectedImageBody(img) {
  const parts = [encodeString(SI_UUID, img.uuid)];
  if (typeof img.width === "number" && typeof img.height === "number" && Number.isFinite(img.width) && Number.isFinite(img.height) && img.width > 0 && img.height > 0) {
    parts.push(encodeMessage(SI_DIMENSION, [encodeUInt32Field(DIM_WIDTH, Math.floor(img.width)), encodeUInt32Field(DIM_HEIGHT, Math.floor(img.height))]));
  }
  if (img.mimeType) {
 ...
 */
export function openAIToolsToMcpDefs(tools) {
  return tools.map(t => {
...
export function encodeAgentRunRequest(input) {
  ...
  const {
    modelId,
    parameters
  } = resolveRequestedModel(input.modelId);
  ...
  // streams a response.
  const userMessage = encodeMessage(UMA_USER_MESSAGE, [encodeString(UM_TEXT, input.userText), encodeString(UM_MESSAGE_ID, messageId), encodeMessage(UM_SELECTED_CONTEXT, selectedContextParts), Buffer.concat([encodeTag(UM_MODE, WT_VARINT), encodeVarint(1)])]);
  ...
  for (const param of parameters) {
    rmParts.push(encodeMessage(RM_PARAMETERS, [encodeString(RMP_ID, param.id), encodeString(RMP_VALUE, param.value)]));
  }
  ...
  const modelDetails = encodeMessage(ARR_MODEL_DETAILS, [encodeString(MD_MODEL_ID, modelId), encodeString(MD_DISPLAY_MODEL_ID, modelId), encodeString(MD_DISPLAY_NAME, modelId)]);
  ...
  const mcpToolsBlock = encodeMessage(ARR_MCP_TOOLS, mcpToolDefs.map(def => encodeMessage(ARR_MCP_TOOLS_INNER, [encodeMcpToolDefinitionBody(def)])));
  ...
  const agentRunRequest = [conversationState, action, modelDetails, mcpToolsBlock, encodeString(ARR_CONVERSATION_ID, conversationId), requestedModel, Buffer.concat([encodeTag(ARR_UNKNOWN_12, WT_VARINT), encodeVarint(0)]), encodeString(ARR_REQUEST_ID, conversationId)];
...
export function buildAgentRequestBody(input) {
      ...
      switch (update.fieldNumber) {
        case IU_TEXT_DELTA:
          if (update.wireType === 2) {
            out.push({
              kind: "text",
              text: decodeStringField(update.bytes, TDU_TEXT)
            });
          }
          break;
        ...
        case IU_TOKEN_DELTA:
          if (update.wireType === 2) {
            out.push({
              kind: "token_delta",
              tokens: decodeVarintField(update.bytes, 1)
            });
          }
          break;
        case IU_HEARTBEAT:
 ...
 */
export function decodeExecRequestContext(payload) {
...
export function decodeKvServerEvent(payload) {
  for (const top of decodeFields(payload)) {
 ...
 * available.
 */
export function encodeRequestContextResponse(id, execId, tools) {
  const rcParts = [];
  ...
  const ecm = encodeMessage(ACM_EXEC_CLIENT_MESSAGE, [encodeUInt32Field(ECM_ID, id), encodeString(ECM_EXEC_ID, execId), encodeMessage(ECM_REQUEST_CONTEXT_RESULT, [success])]);
...
function wrapExecClientMessage(execMsgId, execId, resultFieldNumber, resultPayload) {
  const ecm = encodeMessage(ACM_EXEC_CLIENT_MESSAGE, [encodeUInt32Field(ECM_ID, execMsgId), encodeString(ECM_EXEC_ID, execId), encodeMessage(resultFieldNumber, [resultPayload])]);
  return wrapConnectFrame(ecm);
}
...
function encodeShellRejection(command, workingDir, reason) {
  return Buffer.concat([encodeString(SREJ_COMMAND, command), encodeString(SREJ_WORKING_DIR, workingDir), encodeString(SREJ_REASON, reason)]);
}
export function encodeExecReadRejected(execMsgId, execId, path, reason) {
...
export function encodeExecWriteRejected(execMsgId, execId, path, reason) {
  const rejected = encodeMessage(RES_REJECTED, [encodePathRejection(path, reason)]);
  return wrapExecClientMessage(execMsgId, execId, ECM_WRITE_RESULT, rejected);
}
export function encodeExecDeleteRejected(execMsgId, execId, path, reason) {
  const rejected = encodeMessage(RES_REJECTED, [encodePathRejection(path, reason)]);
  return wrapExecClientMessage(execMsgId, execId, ECM_DELETE_RESULT, rejected);
}
export function encodeExecLsRejected(execMsgId, execId, path, reason) {
  const rejected = encodeMessage(RES_REJECTED, [encodePathRejection(path, reason)]);
  return wrapExecClientMessage(execMsgId, execId, ECM_LS_RESULT, rejected);
}
export function encodeExecShellRejected(execMsgId, execId, command, workingDir, reason) {
...
export function encodeExecBackgroundShellSpawnRejected(execMsgId, execId, command, workingDir, reason) {
  const rejected = encodeMessage(RES_REJECTED, [encodeShellRejection(command, workingDir, reason)]);
  return wrapExecClientMessage(execMsgId, execId, ECM_BACKGROUND_SHELL_SPAWN_RES, rejected);
}
export function encodeExecGrepError(execMsgId, execId, errMsg) {
...
export function encodeExecFetchError(execMsgId, execId, url, errMsg) {
...
export function encodeExecWriteShellStdinError(execMsgId, execId, errMsg) {
...
export function encodeExecDiagnosticsResult(execMsgId, execId) {
...
export function encodeExecMcpResult(execMsgId, execId, content, isError) {
...
export function encodeExecMcpError(execMsgId, execId, errMsg) {
 ...
 * request to response.
 */
export function encodeKvGetBlobResult(kvId, blobData, requestMetadata = null) {
 ...
 */
export function encodeKvSetBlobResult(kvId, requestMetadata = null) {
  const parts = [];
  if (kvId !== 0) parts.push(encodeUInt32Field(KCM_ID, kvId));
 ...
 */
export function encodeMcpToolDefinitionBody(def) {
  const parts = [encodeString(MTD_NAME, def.name), encodeString(MTD_DESCRIPTION, def.description), encodeBytes(MTD_INPUT_SCHEMA, def.inputSchemaBytes)];
 ...
 */
export function jsonSchemaToProtobufValue(json) {
 ...
 */
export function decodeProtobufValue(buf) {
  let pos = 0;
...
function encodeProtobufValue(value) {
  ...
  if (typeof value === "object") {
    ...
    for (const [k, v] of Object.entries(obj)) {
      const entry = Buffer.concat([encodeString(MAP_KEY, k), encodeMessage(MAP_VALUE, [encodeProtobufValue(v)])]);
    ...
    if (m.role === "user") {
      ...
      if (Array.isArray(m.tool_calls)) {
        for (const tc of m.tool_calls) {
          ...
          lines.push(`Assistant called tool ${tc.function?.name ?? "(unknown)"} ` + `(${tc.id}) with arguments: ${args}`);
        }
      }
    } else if (m.role === "tool") {
