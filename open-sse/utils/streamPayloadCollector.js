import { cloneLogPayload } from "../stubs/lib/logPayloads";
...
function mergeUsage(target, incoming) {
  ...
  for (const [key, value] of Object.entries(usage)) {
    if (typeof value === "number" && Number.isFinite(value)) {
      if (target[key] === undefined || value > 0) {
        target[key] = value;
      }
...
function buildOpenAISummary(events, fallbackModel) {
  const payloads = events.map(evt => asRecord(evt.data)).filter(payload => Object.keys(payload).length);
...
export function buildStreamSummaryFromEvents(events, fallbackFormat, fallbackModel) {
...
export function compactStructuredStreamPayload(payload) {
...
export function createStructuredSSECollector(options = {}) {
  const {
    maxEvents = 200,
    maxBytes = 49152,
    stage
  ...
  return {
    ...
    build(summary, buildOptions = {}) {
      ...
      return {
        ...
        ...(summary === undefined ? {} : {
          summary: cloneLogPayload(summary)
        })
      };
    }
  };
