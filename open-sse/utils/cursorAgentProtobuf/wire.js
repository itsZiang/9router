...
export function encodeTag(fieldNumber, wireType) {
  return encodeVarint(fieldNumber << 3 | wireType);
}
export function encodeBytes(fieldNumber, value) {
...
export function encodeMessage(fieldNumber, parts) {
 ...
 */
export function checkedLen(len, pos, buf) {
  if (len < 0n || len > BigInt(buf.length - pos)) {
    throw new Error(`length-delimited field overruns buffer (len=${len}, remaining=${buf.length - pos})`);
  }
  return Number(len);
}
  ...
  while (pos < buf.length) {
    ...
    if (wireType === WT_VARINT) {
      ...
      fields.push({
        fieldNumber,
        wireType: 2,
        bytes: buf.subarray(pos, pos + lenN)
      });
      pos += lenN;
    } else if (wireType === 5) {
      pos += 4;
...
export function decodeStringField(buf, fieldNumber) {
  const fields = decodeFields(buf);
  const f = findField(fields, fieldNumber);
  if (f && f.wireType === 2) return f.bytes.toString("utf8");
  return "";
}
export function decodeVarintField(buf, fieldNumber) {
  const fields = decodeFields(buf);
  const f = findField(fields, fieldNumber);
