...
import { parseAndValidatePublicUrl, isPrivateHost, OutboundUrlGuardError } from "../stubs/shared/network/outboundUrlGuard";
...
function validatePublicImageUrl(url) {
  try {
    ...
    if (err instanceof OutboundUrlGuardError) {
      throw new CursorImageError(err.code === "OUTBOUND_URL_INVALID" ? "Image URL is invalid or uses an unsupported scheme." : "Image URL points to a blocked address.");
    }
 ...
 */
export function assertResolvedAddressesPublic(addresses) {
 ...
 */
export async function resolveCursorImages(imageUrls) {
  ...
  for (const url of imageUrls) {
    ...
    const {
      data,
      mimeType
    } = url.toLowerCase().startsWith("data:") ? decodeDataUrl(url) : await fetchImageBytes(url);
    if (!data.length) {
