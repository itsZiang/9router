import { CODEX_SPARK_DISPLAY_NAME, CODEX_SPARK_QUOTA_SESSION, CODEX_SPARK_QUOTA_WEEKLY, isCodexSparkLimitDescriptor } from "../config/codexQuotaScopes";
export function getFieldValue(record, ...keys) {
...
function findCodexSparkRateLimit(data) {
  ...
  for (const entryValue of additionalRateLimits) {
    ...
    if (isCodexSparkLimitDescriptor(getFieldValue(entry, "limit_name", "limitName"), getFieldValue(entry, "metered_feature", "meteredFeature"), getFieldValue(entry, "limit_id", "limitId"), entry["id"], entry["name"], entry["title"], entry["model"], getFieldValue(entry, "model_id", "modelId"))) {
...
function findCodexReviewRateLimit(data) {
  ...
  for (const entryValue of additionalRateLimits) {
    const entry = toRecord(entryValue);
    if (isCodexReviewLimitDescriptor(getFieldValue(entry, "limit_name", "limitName"), getFieldValue(entry, "metered_feature", "meteredFeature"), getFieldValue(entry, "limit_id", "limitId"), entry["id"], entry["name"])) {
      return toRecord(getFieldValue(entry, "rate_limit", "rateLimit"));
    }
  }
  return {};
}
export function buildCodexUsageQuotas(dataValue) {
  ...
  if (Object.keys(sparkPrimaryWindow).length > 0) {
    quotas[CODEX_SPARK_QUOTA_SESSION] = buildPercentageQuota(sparkPrimaryWindow, CODEX_SPARK_DISPLAY_NAME);
  }
  ...
  if (Object.keys(sparkSecondaryWindow).length > 0) {
    quotas[CODEX_SPARK_QUOTA_WEEKLY] = buildPercentageQuota(sparkSecondaryWindow, `${CODEX_SPARK_DISPLAY_NAME} Weekly`);
  }
  return {
    rateLimit,
    quotas
