...
import { registerQuotaFetcher } from "./quotaPreflight";
...
export function parseCrofUsageResponse(data) {
  const obj = toRecord(data);
  if (Object.keys(obj).length === 0) return null;
  const usableRequestsRaw = obj["usable_requests"];
  const usableRequests = usableRequestsRaw === null || usableRequestsRaw === undefined ? null : toNumber(usableRequestsRaw);
...
export async function fetchCrofUsage(connectionId, connection) {
...
export function invalidateCrofUsageCache(connectionId) {
  quotaCache.delete(connectionId);
}
 ...
 */
export function registerCrofUsageFetcher() {
