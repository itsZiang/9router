...
export function estimateMessageTokens(messages) {
  return messages.reduce((sum, msg) => {
    ...
    if (Array.isArray(msg.content)) {
      return sum + msg.content.reduce((s, part) => s + (typeof part?.text === "string" ? estimateTokens(part.text) : 0), 0);
    }
    return sum;
  }, 0);
}
export function detectCodeComplexity(input) {
  ...
  const langIndicators = [/function\s+\w+\s*\(/gi, /const\s+\w+\s*=/gi, /import\s+.*from/gi, /class\s+\w+/gi, /interface\s+\w+/gi, /async\s+function/gi, /def\s+\w+\s*\(/gi, /SELECT\s+.*FROM/gi, /\$\{.*\}/g];
...
export function detectMathComplexity(input) {
  const allText = input.messages.map(m => typeof m.content === "string" ? m.content : "").join("\n");
  const latexMatches = allText.match(/\$\$[\s\S]*?\$\$|\$[^$]+\$/g);
  const latexCount = latexMatches ? latexMatches.length : 0;
  const mathIndicators = [/[+\-*/^]=/g, /\b(sin|cos|tan|log|sqrt|sum|prod|int|lim)\b/gi, /\b\d+\s*[+\-*/]\s*\d+\s*=/g, /∑|∏|∫|√|∞|π/g, /\bf'(?:x)?\b/g, /\bdx\b/g];
...
export function detectContextSize(input) {
...
export function detectToolCalling(input) {
...
export function detectDomainSpecificity(input) {
  const allText = input.messages.map(m => typeof m.content === "string" ? m.content : "").join("\n");
  const domainTerms = {
...
export function getSpecificityBreakdown(input) {
  return {
...
export function detectConversationDepth(input) {
  const userMessages = input.messages.filter(m => m.role === "user").length;
  const assistantMessages = input.messages.filter(m => m.role === "assistant").length;
...
export function detectFileReferences(input) {
  const allText = input.messages.map(m => typeof m.content === "string" ? m.content : "").join("\n");
  const filePatterns = [/(?:\/[\w.-]+){2,}/g, /\b\w+:\d+:\d+\b/g, /\b(?:diff|patch|merge)\b/gi, /\b(?:README|CHANGELOG|TODO)\b/gi, /@@[\s+-]+\d+,\d+\s+@@/g];
  const matches = filePatterns.reduce((sum, re) => {
    return sum + (allText.match(re)?.length || 0);
  }, 0);
  return Math.min(5, matches * 1);
}
export function detectErrorContext(input) {
  const allText = input.messages.map(m => typeof m.content === "string" ? m.content : "").join("\n");
  const errorPatterns = [/\b(?:Error|Exception|TypeError|ReferenceError|SyntaxError)\b/g, /\bat\s+[\w.]+\s+\([\w./]+:\d+:\d+\)/g, /\b(?:throw|catch|finally)\b/g, /\b(?:ERRO|FATAL|WARN)\b/g, /\b(?:failed|crashed|unexpected)\b/gi, /\bExit code \d+\b/g];
  const matches = errorPatterns.reduce((sum, re) => {
    return sum + (allText.match(re)?.length || 0);
  }, 0);
  return Math.min(5, matches * 0.5);
}
export function detectEnhancedContextSize(input) {
  ...
  const toolTokens = input.tools ? input.tools.reduce((sum, t) => sum + estimateTokens(JSON.stringify(t?.function || t)), 0) : 0;
...
export function getEnhancedSpecificityBreakdown(input) {
  return {
    codeComplexity: detectCodeComplexity(input),
    mathComplexity: detectMathComplexity(input),
    reasoningDepth: detectReasoningDepth(input),
