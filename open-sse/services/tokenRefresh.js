...
import { PROVIDERS, OAUTH_ENDPOINTS } from "../config/constants";
import { getGitHubCopilotRefreshHeaders } from "../config/providerHeaderProfiles";
import { pbkdf2Sync } from "node:crypto";
import { runWithProxyContext } from "../utils/proxyFetch";
import { serializeRefresh, wasRefreshTokenRotated } from "./refreshSerializer";
import { WINDSURF_CONFIG } from "../stubs/lib/oauth/constants/oauth";
import { buildGitLabOAuthEndpoints, resolveGitLabOAuthBaseUrl } from "../stubs/lib/oauth/gitlab";
...
export const REFRESH_LEAD_MS = {
  ...
  // 5 minutes
  openai: 5 * 60 * 1000,
  // same Auth0 backend as codex
  claude: 5 * 60 * 1000,
  // Anthropic OAuth rotates refresh_tokens (user-reported)
  "gitlab-duo": 5 * 60 * 1000,
  // GitLab token family revocation on misuse
  kiro: 5 * 60 * 1000,
  // AWS SSO OIDC issues one-time-use refresh tokens
  "kimi-coding": 5 * 60 * 1000,
  // Moonshot rotates per-refresh
  qwen: 5 * 60 * 1000,
  // Alibaba device-code path also rotates
...
function lookupRotation(provider, refreshToken) {
  cleanupRotationMap();
...
function recordRotation(provider, oldRefreshToken, result) {
...
export function _getTokenRotationMapStats() {
  cleanupRotationMap();
  return {
    size: tokenRotationMap.size,
    entries: tokenRotationMap.size
  };
}
export function _clearTokenRotationMap() {
...
const onPersistStore = new AsyncLocalStorage();
export function runWithOnPersist(onPersist, fn) {
  if (!onPersist) return fn();
  return onPersistStore.run(onPersist, fn);
}
export function getActiveOnPersist() {
...
const casGuardStore = new AsyncLocalStorage();
const casGuardStats = {
  skipped: 0,
  persisted: 0
};
export function runWithCasGuard(guard, fn) {
  if (!guard) return fn();
  return casGuardStore.run(guard, fn);
}
export function getActiveCasGuard() {
  return casGuardStore.getStore();
}

/** Skip/persist counters for observability + tests. */
export function getCasGuardStats() {
  return {
    ...casGuardStats
  };
}
...
export function _resetCasGuardStats() {
 ...
 */
async function casGuardShouldSkipPersist(log) {
  ...
  if (wasRefreshTokenRotated(guard.expectedRefreshToken, current)) {
    ...
    log?.warn?.("TOKEN_REFRESH", "CAS guard: skipping persist — a concurrent writer already rotated the refresh_token (#4038)");
    return true;
  }
  casGuardStats.persisted++;
  return false;
}
function buildFormParams(entries) {
 ...
 */
export function extractOAuthErrorCode(raw, depth = 0) {
  ...
  if (typeof raw === "object") {
    const o = raw;
    return extractOAuthErrorCode(o.error, depth + 1) ?? extractOAuthErrorCode(o.code, depth + 1) ?? extractOAuthErrorCode(o.error_code, depth + 1);
  }
  return null;
}
 ...
 */
async function readRefreshErrorBody(response) {
 ...
 */
export async function refreshAccessToken(provider, refreshToken, credentials, log, proxyConfig = null) {
  ...
  try {
    ...
    const response = await runWithProxyContext(proxyConfig, () => fetch(refreshEndpoint, {
      method: "POST",
      headers: {
 ...
 */
export async function refreshWindsurfToken(refreshToken, providerSpecificData, log, proxyConfig = null) {
  if (!refreshToken) {
    log?.warn?.("TOKEN_REFRESH", "No refresh token stored for Windsurf — token may be a long-lived API key");
    return null;
  }
  const authMethod = providerSpecificData?.authMethod || "import";
  ...
  if (!firebaseApiKey) {
    log?.warn?.("TOKEN_REFRESH", "Windsurf Firebase API key unavailable — skipping Firebase token refresh");
    return null;
  }
  const tokenUrl = `https://securetoken.googleapis.com/v1/token?key=${firebaseApiKey}`;
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch(tokenUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
    ...
    if (!response.ok) {
      ...
      try {
        ...
        const fbCode = typeof fbError?.error?.message === "string" ? fbError.error.message : typeof fbError?.error === "string" ? fbError.error : null;
        if (typeof fbCode === "string" && (fbCode.includes("USER_DISABLED") || fbCode.includes("TOKEN_EXPIRED") || fbCode.includes("INVALID_REFRESH_TOKEN") || fbCode.includes("USER_NOT_FOUND"))) {
          log?.error?.("TOKEN_REFRESH", "Windsurf Firebase token is permanently invalid. Re-authentication required.", {
            fbCode
          });
          return {
            error: "unrecoverable_refresh_error",
    ...
    log?.error?.("TOKEN_REFRESH", `Network error refreshing Windsurf token: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
 ...
 */
export async function refreshCodebuddyCnToken(refreshToken, log, proxyConfig = null) {
  if (!refreshToken) return null;
  const {
    CODEBUDDY_CN_CONFIG
  } = await import("../stubs/lib/oauth/constants/oauth");
  const oauth = CODEBUDDY_CN_CONFIG;
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch(oauth.refreshUrl, {
      method: "POST",
      headers: {
...
export async function refreshClineToken(refreshToken, log, proxyConfig = null) {
  ...
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json"
 ...
 * same token.
 */
export async function refreshKimiCodingToken(refreshToken, providerSpecificData, log, proxyConfig = null) {
  ...
  const stableDeviceId = providerSpecificData?.deviceId || pbkdf2Sync(refreshToken, "kimi-device-id", 1000, 16, "sha256").toString("hex");
  ...
  try {
    ...
    const response = await runWithProxyContext(proxyConfig, () => fetch(endpoint, {
      ...
      headers: {
        ...
        "X-Msh-Device-Model": providerSpecificData?.deviceModel || deviceModel,
        ...
        "X-Msh-Device-Name": providerSpecificData?.deviceName || osTypeStr,
        "X-Msh-Os-Version": providerSpecificData?.osVersion || osTypeStr
      },
      body: params
    }));
    if (!response.ok) {
      ...
      try {
        ...
        if (errorCode === "invalid_grant" || errorCode === "invalid_request") {
          log?.error?.("TOKEN_REFRESH", "Kimi Coding refresh token invalid. Re-authentication required.", {
            errorCode
          });
          return {
            error: "unrecoverable_refresh_error",
    ...
    log?.error?.("TOKEN_REFRESH", `Network error refreshing Kimi Coding token: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
 ...
 */
export async function refreshGitLabDuoToken(refreshToken, providerSpecificData, log, proxyConfig = null) {
  if (!refreshToken) {
  ...
  const clientId = providerSpecificData?.clientId || PROVIDERS["gitlab-duo"]?.clientId || process.env.GITLAB_DUO_OAUTH_CLIENT_ID || process.env.GITLAB_OAUTH_CLIENT_ID || "";
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch(tokenUrl, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorCode === "invalid_grant" || errorCode === "invalid_request") {
          log?.error?.("TOKEN_REFRESH", "GitLab Duo refresh token invalid. Re-authentication required.", {
            errorCode
          });
          return {
            error: "unrecoverable_refresh_error",
    ...
    log?.error?.("TOKEN_REFRESH", `Network error refreshing GitLab Duo token: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
export async function refreshClaudeOAuthToken(refreshToken, log, proxyConfig = null) {
  try {
    ...
    const response = await runWithProxyContext(proxyConfig, () => fetch(OAUTH_ENDPOINTS.anthropic.token, {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        Accept: "application/json",
    ...
    if (!response.ok) {
      ...
      const {
        rawText,
        code
      } = await readRefreshErrorBody(response);
 ...
 */
export async function refreshGoogleToken(refreshToken, clientId, clientSecret, log, proxyConfig = null) {
  const response = await runWithProxyContext(proxyConfig, () => fetch(OAUTH_ENDPOINTS.google.token, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json"
...
export async function refreshCodexToken(refreshToken, log, proxyConfig = null) {
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch(OAUTH_ENDPOINTS.openai.token, {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        Accept: "application/json"
    ...
    if (!response.ok) {
      ...
      if (errorCode === "refresh_token_reused" || errorCode === "invalid_grant" || errorCode === "token_expired" || errorCode === "invalid_token") {
        log?.error?.("TOKEN_REFRESH", "Codex refresh token already used or invalid. Re-authentication required.", {
          status: response.status,
          errorCode
        });
        return {
      ...
      if (response.status === 401) {
        ...
        log?.error?.("TOKEN_REFRESH", "Codex OAuth token endpoint returned 401. Re-authentication required.", {
          status: response.status,
          errorCode: code
        });
        return {
 ...
 */
export async function refreshKiroToken(refreshToken, providerSpecificData, log, proxyConfig = null) {
  try {
    ...
    if (clientId && clientSecret && authMethod !== "imported") {
      ...
      if (!response.ok) {
        ...
        if (awsErrorType === "InvalidGrantException" || awsErrorType === "ExpiredTokenException" || awsErrorType === "invalid_grant") {
        ...
        try {
          ...
          const regRes = await runWithProxyContext(proxyConfig, () => fetch(regEndpoint, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json"
          ...
          if (regRes.ok) {
            ...
            const retryRes = await runWithProxyContext(proxyConfig, () => fetch(endpoint, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Accept: "application/json"
...
export async function refreshQoderToken(refreshToken, log, proxyConfig = null) {
  if (!OAUTH_ENDPOINTS.qoder.token || !PROVIDERS.qoder.clientId || !PROVIDERS.qoder.clientSecret) {
    log?.warn?.("TOKEN_REFRESH", "Qoder OAuth refresh skipped: browser OAuth is not configured in this environment");
    return null;
  }
  const basicAuth = btoa(`${PROVIDERS.qoder.clientId}:${PROVIDERS.qoder.clientSecret}`);
  const response = await runWithProxyContext(proxyConfig, () => fetch(OAUTH_ENDPOINTS.qoder.token, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json",
...
export async function refreshGitHubToken(refreshToken, log, proxyConfig = null) {
  const response = await runWithProxyContext(proxyConfig, () => fetch(OAUTH_ENDPOINTS.github.token, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json"
...
export async function refreshCopilotToken(githubAccessToken, log, proxyConfig = null) {
  try {
    const response = await runWithProxyContext(proxyConfig, () => fetch("https://api.github.com/copilot_internal/v2/token", {
...
async function _getAccessTokenInternal(provider, credentials, log, proxyConfig = null) {
  switch (provider) {
    ...
    case "antigravity":
    case "agy":
      return await refreshGoogleToken(credentials.refreshToken, PROVIDERS[provider].clientId, PROVIDERS[provider].clientSecret, log, proxyConfig);
    case "claude":
    ...
    case "kiro":
    case "amazon-q":
      return await refreshKiroToken(credentials.refreshToken, credentials.providerSpecificData, log, proxyConfig);
    case "cline":
    ...
    case "gitlab-duo":
      return await refreshGitLabDuoToken(credentials.refreshToken, credentials.providerSpecificData, log, proxyConfig);
    case "windsurf":
    case "devin-cli":
      return await refreshWindsurfToken(credentials.refreshToken, credentials.providerSpecificData, log, proxyConfig);
    case "codebuddy-cn":
...
export function supportsTokenRefresh(provider) {
  const explicitlySupported = new Set(["gemini", "antigravity", "agy", "claude", "codex", "qwen", "qoder", "github", "kiro", "amazon-q", "cline", "kimi-coding", "windsurf", "devin-cli", "gitlab-duo", "codebuddy-cn"]);
...
export function isUnrecoverableRefreshError(result) {
  return result && typeof result === "object" && (result.error === "unrecoverable_refresh_error" || result.error === "refresh_token_reused" || result.error === "invalid_request" || result.error === "invalid_grant");
}
 ...
 */
export async function getAccessToken(provider, credentials, log, proxyConfig = null, onPersist) {
  ...
  if (connectionId && typeof connectionId === "string") {
    ...
    entry.promise = (async () => {
      const result = await _getAccessTokenWithStalenessCheck(provider, credentials, log, proxyConfig);
      ...
      if (result?.accessToken && effectiveOnPersist) {
        ...
        try {
          ...
          log?.error?.("TOKEN_REFRESH", `onPersist callback failed for ${provider}/${connectionId}: ${sanitizeErrorMessage(persistErr instanceof Error ? persistErr : new Error(String(persistErr)))}`);
          throw persistErr;
        }
      }
      return result;
  ...
  const refreshPromise = serializeRefresh(provider, () => _getAccessTokenInternal(provider, credentials, log, proxyConfig)).then(async result => {
    if (result?.accessToken && effectiveOnPersist) {
      ...
      try {
        ...
        log?.error?.("TOKEN_REFRESH", `Layer 2 onPersist callback failed for ${provider}: ${sanitizeErrorMessage(persistErr instanceof Error ? persistErr : new Error(String(persistErr)))}`);
        throw persistErr;
      }
...
async function _getAccessTokenWithStalenessCheck(provider, credentials, log, proxyConfig) {
  ...
  if (credentials.connectionId) {
    try {
      ...
      } = await import("../stubs/lib/db/providers");
      ...
      log?.warn?.("TOKEN_REFRESH", `Failed to check DB for stale token: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  ...
  const result = await serializeRefresh(provider, () => _getAccessTokenInternal(provider, credentials, log, proxyConfig));
  ...
  if (result && typeof result === "object" && !("error" in result) && result.accessToken && result.refreshToken) {
    recordRotation(provider, oldRefreshToken, result);
  }
  return result;
}
 ...
 */
export function getConnectionRefreshMutexStatus() {
  const result = {};
 ...
 */
function recordFailure(provider, log = null) {
  if (!_circuitBreaker[provider]) {
    _circuitBreaker[provider] = {
      failures: 0,
      blockedUntil: 0
    };
  }
  ...
  if (_circuitBreaker[provider].failures >= CIRCUIT_BREAKER_THRESHOLD) {
    ...
    log?.error?.("TOKEN_REFRESH", `🔴 Circuit breaker tripped for ${provider}: ${CIRCUIT_BREAKER_THRESHOLD} consecutive failures. ` + `Blocked for ${CIRCUIT_BREAKER_COOLDOWN / 60000}min. Provider needs re-authentication.`);
  }
}
...
export async function refreshWithRetry(refreshFn, maxRetries = 3, log = null, provider = "unknown") {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (isUnrecoverableRefreshError(result)) {
        log?.warn?.("TOKEN_REFRESH", `Unrecoverable refresh error for ${provider}: ${result.error} — skipping retries`);
        return result;
      }
      if (result) {
