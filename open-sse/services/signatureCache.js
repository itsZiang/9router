...
const DEFAULT_SIGNATURES = ["<antThinking>", "</antThinking>", "<thinking>", "</thinking>", "<internal_thought>", "</internal_thought>"];
...
export function getModelFamily(model) {
  ...
  const cleaned = modelName.replace(/-\d{4}-\d{2}-\d{2}$/, "") // Remove YYYY-MM-DD suffix
