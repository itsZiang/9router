...
import { countBatchItemCheckpoints, createFile, deleteFile, ensureBatchItemCheckpoints, getApiKeyById, getBatch, getFileContent, getPendingBatches, getTerminalBatches, listBatchItemCheckpoints, listFiles, markBatchItemError, markBatchItemProcessing, markBatchItemResult, updateBatch } from "@/lib/localDb";
import { dispatch } from "../stubs/lib/batches/dispatch";
import { DEFAULT_BATCH_EXPIRATION_SECONDS } from "../stubs/shared/constants/batch";
let isProcessing = false;
let pollInterval = null;
const activeProcesses = new Set();
const activeBatches = new Set();
const DEFAULT_BATCH_WINDOW_SECONDS = 24 * 60 * 60;
const BATCH_RETRY_DURATION_MS = Number.parseInt(process.env.BATCH_RETRY_DURATION_MS ?? "", 10) || 24 * 60 * 60 * 1_000;
const BATCH_BACKOFF_BASE_MS = Number.parseInt(process.env.BATCH_BACKOFF_BASE_MS ?? "", 10) || 5_000;
const BATCH_BACKOFF_MAX_MS = Number.parseInt(process.env.BATCH_BACKOFF_MAX_MS ?? "", 10) || 3_600_000;
  ...
  for (const batch of remaining) {
    if (batch.status === "validating") {
      if (activeCount >= BATCH_MAX_CONCURRENT) {
        console.log(`[BATCH] Concurrency limit ${BATCH_MAX_CONCURRENT} reached, deferring batch ${batch.id}`);
        continue;
      }
      activeCount++;
...
function recoverStaleBatch(batch) {
  ...
  if (checkpointCount === 0 && hasPotentialExternalEffects) {
    console.warn(`[BATCH] Stale batch ${batch.id} has no item checkpoints; failing instead of replaying provider calls`);
    failBatch(batch.id, "Cannot safely recover stale batch because item checkpoints are unavailable; create a new batch to retry intentionally.");
    return;
  }
...
export function parseBatchItems(content, batchEndpoint) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body)) {
      return {
        items: null,
        error: `Line ${index + 1} must include a JSON object body`
      };
    }
    items.push({
      body: body,
...
async function processBatchItems(batch, items) {
  ...
  const checkpoints = new Map(listBatchItemCheckpoints(batch.id).map(checkpoint => [checkpoint.lineNumber, checkpoint]));
...
function applyRecoveredCheckpoint(batchId, item, checkpoint, state) {
  if (checkpoint.status === "completed" && checkpoint.result) {
    ...
    applyItemResult(state, checkpoint.result.response?.status_code ?? 500, checkpoint.result.response?.body);
    return true;
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if ((response.status === 429 || response.status === 502 || response.status === 504) && attempt < maxRetries) {
      ...
      if (Date.now() - retryStartedAt >= MAX_RETRY_DURATION_MS) {
        console.warn(`[BATCH] Item ${item.customId ?? "(no id)"} exceeded 24h retry window after ${attempt} attempts — giving up`);
        return response;
      }
...
export function buildRequestBody(item) {
  const isChatEndpoint = !["/v1/embeddings", "/v1/moderations", "/v1/images/generations", "/v1/images/edits", "/v1/videos", "/v1/videos/generations"].includes(item.url);
  return {
    ...item.body,
    ...(isChatEndpoint ? {
      stream: false
...
export function maybeThrottle(headers) {
  ...
  if (pressures.length === 0) {
    ...
    console.log(`[BATCH] Throttle check - Request pressure: ${remainingReq ?? "n/a"}/${limitReq ?? "n/a"}, Token pressure: ${remainingTokens ?? "n/a"}/${tokenTotal ?? "n/a"}`);
  }
  const pressureRemaining = Math.min(...pressures);
  const delay = throttleDelay(pressureRemaining);
  if (delay !== null) {
    console.log(`[BATCH] Throttling next request with delay of ${Math.round(delay)}ms (pressure remaining: ${(pressureRemaining * 100).toFixed(2)}%)`);
  }
  return delay;
}
    ...
    updateBatch(batchId, {
      ...
      usage: {
        ...
        output_tokens_details: {
          reasoning_tokens: state.tokens.reasoning
        }
      }
    });
  } catch (err) {
...
async function finalizeBatch(batchId, results, itemsWithErrors) {
  ...
  const successes = results.filter(r => typeof r.response?.status_code === "number" && r.response.status_code < 400 && !r.response.body?.error);
  const failuresFromResults = results.filter(r => typeof r.response?.status_code === "number" && r.response.status_code >= 400 || r.response?.body?.error);
...
export async function waitForAllBatches() {
...
export function getCachedHeaders() {
  return {
    headers: prevHeaders,
    timestamp: prevHeadersTimestamp
  };
}
export function resetCachedHeaders() {
  prevHeaders = null;
  prevHeadersTimestamp = 0;
}
export function resetBatchProcessorState() {
  activeBatches.clear();
