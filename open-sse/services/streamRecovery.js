 ...
 */
import { STREAM_RECOVERY } from "../config/constants";
 ...
 */
export function hasTerminalMarker(bytes) {
 ...
 */
export function scanOpenAiSseText(sse) {
  let text = "";
 ...
 */
export function makeContinuationBody(body, assistantSoFar) {
 ...
 */
export function trimContinuationOverlap(emitted, continuation) {
...
export function createRecoverableStream(initialStream, reopen, options) {
  ...
  const emit = (controller, chunk) => {
    ...
    emittedTail += trackDecoder.decode(chunk, {
      stream: true
    });
  ...
  const tryContinue = async controller => {
    ...
    for (;;) {
      ...
      if (r.done) break;
      if (r.value) raw += contDecoder.decode(r.value, {
        stream: true
      });
    }
    ...
    if (suffix) {
      emit(controller, encoder.encode(`data: ${JSON.stringify({
        choices: [{
          index: 0,
          delta: {
            content: suffix
