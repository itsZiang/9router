...
function buildFallbackDescription(tool) {
  ...
  return [`Search the ${accessMode} for recent, factual information and return cited results.`, "Use this when the answer depends on current events, external documents, or fresh facts.", `If max_results is omitted, prefer about ${defaultMaxResults} results.`].join(" ");
}
...
export function prepareWebSearchFallbackBody(body, options) {
  ...
  if (!toolNames.has(OMNIROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
    preservedTools.unshift(buildFallbackTool(toRecord(builtInSearchTools[0]), options.targetFormat));
  }
  const nextBody = {
    ...body,
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = isResponsesTarget ? {
      type: "function",
