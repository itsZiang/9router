...
function decodeEntities(text) {
  return text.replace(/&nbsp;/gi, " ").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#0*39;|&#x0*27;|&apos;/gi, "'")
 ...
 */
export function parseDuckDuckGoLite(rawHtml) {
  if (!rawHtml) return [];
 ...
 */
export async function freeWebSearch(query, maxResults = 5, timeoutMs = 10_000) {
