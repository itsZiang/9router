...
import { CODEX_SPARK_QUOTA_SESSION, CODEX_SPARK_QUOTA_WEEKLY, getCodexModelScope, isCodexSparkLimitDescriptor } from "../config/codexQuotaScopes";
import { registerQuotaFetcher, registerQuotaWindows } from "./quotaPreflight";
 ...
 */
export function registerCodexConnection(connectionId, meta) {
...
function deleteQuotaCacheForConnection(connectionId) {
  ...
  const scopedKeys = Array.from(quotaCache.keys()).filter(key => key.startsWith(`${connectionId}:`));
  for (const key of scopedKeys) quotaCache.delete(key);
}
export function unregisterCodexConnection(connectionId) {
...
function getCodexConnectionMeta(connectionId, connection) {
  if (connection && typeof connection === "object") {
    ...
    const workspaceId = typeof providerSpecificData.workspaceId === "string" && providerSpecificData.workspaceId.trim().length > 0 ? providerSpecificData.workspaceId : undefined;
    if (accessToken) {
      const meta = {
        accessToken,
        ...(workspaceId ? {
 ...
 */
export async function fetchCodexQuota(connectionId, connection) {
...
function findSparkRateLimit(data) {
  ...
  for (const entryValue of additional) {
    ...
    if (!isCodexSparkLimitDescriptor(entry["limit_name"], entry["limitName"], entry["metered_feature"], entry["meteredFeature"], entry["limit_id"], entry["limitId"], entry["id"], entry["name"], entry["title"], entry["model"], entry["model_id"], entry["modelId"])) {
      continue;
    }
...
function getCodexRateLimitWindows(rateLimit) {
  return {
    ...
    secondary: parseCodexWindow(toRecord(rateLimit["secondary_window"] ?? rateLimit["secondaryWindow"]))
  };
}
function assignCodexWindows(target, rateLimit, names) {
  const {
    primary,
    secondary
  } = getCodexRateLimitWindows(rateLimit);
...
function parseCodexUsageResponse(data, requestedModel) {
  ...
  const selectedRateLimit = getSelectedCodexRateLimit(normalRateLimit, sparkRateLimit, useSparkWindows);
  ...
  const window5h = parsedPrimary ?? {
    percentUsed: 0,
    resetAt: null
  };
  const window7d = parsedSecondary ?? {
    percentUsed: 0,
    resetAt: null
  };
  const worstPercentUsed = Math.max(window5h.percentUsed, window7d.percentUsed);
  const limitReached = Boolean(selectedRateLimit["limit_reached"] ?? selectedRateLimit["limitReached"]);
  const windows = {};
 ...
 */
export function getCodexQuotaCooldownMs(quota, threshold = 0.95) {
  const now = Date.now();
 ...
 */
export function invalidateCodexQuotaCache(connectionId) {
  deleteQuotaCacheForConnection(connectionId);
}
 ...
 */
export function registerCodexQuotaFetcher() {
  registerQuotaFetcher("codex", fetchCodexQuota);
  registerMonitorFetcher("codex", fetchCodexQuota);
  registerQuotaWindows("codex", [CODEX_WINDOW_SESSION, CODEX_WINDOW_WEEKLY, CODEX_SPARK_QUOTA_SESSION, CODEX_SPARK_QUOTA_WEEKLY]);
}