...
function compileReplacement(rule) {
  ...
  const normalizedMap = new Map(Object.entries(rule.replacementMap).map(([key, value]) => [normalizeReplacementKey(key), value]));
...
function getRulesDir() {
  ...
  const candidates = [path.join(root, "open-sse", "services", "compression", "rules"), path.join(root, "app", "open-sse", "services", "compression", "rules")];
...
export function validateRulePack(pack) {
  ...
  if (!pack || typeof pack !== "object") {
    return {
      valid: false,
      errors: ["Rule pack must be an object"]
    };
  }
  const value = pack;
...
function readPack(language, category) {
  ...
  if (!validation.valid) {
    throw new Error(`Invalid Caveman rule pack ${language}/${category}: ${validation.errors.join("; ")}`);
  }
  return parsed;
}
export function loadRulePack(language, category, options = {}) {
...
export function loadAllRulesForLanguage(language, options = {}) {
...
export function getAvailableLanguagePacks() {
  ...
  return fs.readdirSync(root).filter(entry => fs.statSync(path.join(root, entry)).isDirectory()).map(language => {
    ...
    const ruleCount = categories.reduce((count, category) => count + loadRulePack(language, category).length, 0);
    return {
      language,
      categories,
      ruleCount
...
export function loadCavemanFileRules(language, options = {}) {
