...
import { RELEVANCE_SCHEMA, validateRelevanceConfig, resolveRelevanceConfig } from "./configSchema";
import { scoreSentences } from "./scorer";
...
function applyRelevanceToText(text, query, cfg) {
  ...
  if (keepSet.size === sentences.length) return {
    result: text,
    changed: false
  };
  const ordered = indexed.filter(({
    i
  }) => keepSet.has(i)).sort((a, b) => a.i - b.i);
      ...
      const newMessages = messages.map(msg => {
        ...
        if (typeof m.content === "string") {
          const {
            result,
            changed
          } = applyRelevanceToText(m.content, query, cfg);
        ...
        if (Array.isArray(m.content)) {
          ...
          const {
            result,
            changed
          } = applyRelevanceToText(String(textBlocks[0].text), query, cfg);
          if (!changed) return msg;
          anyChanged = true;
          const newContent = m.content.map(block => block === textBlocks[0] ? {
            ...block,
            text: result
          } : block);
          return {
