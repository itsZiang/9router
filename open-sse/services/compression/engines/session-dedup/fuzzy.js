...
export function findNearDuplicates(blocks, minJaccard, maxBlocks, shingleSize = 3) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (best >= 0) {
      out.push({
        block: blocks[i],
        matchedIndex: blocks[best].index,
        similarity: bestSim
      });
    }
  }
...
export function applyFuzzyPass(messages, opts) {
  try {
    ...
    const out = messages.map((m, i) => replacements.has(i) ? {
      ...m,
      content: replacements.get(i)
    } : m);
    return {
      messages: out,
      fuzzyCount: replacements.size
    };
  } catch {
    return {
      messages,
