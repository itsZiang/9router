...
import { runIonizerPass } from "./sample";
...
export const ionizerEngine = {
  ...
  apply(body, options) {
    ...
    const {
      ...
      ionizedCount
    } = runIonizerPass(messages, stepConfig, options?.principalId);
    if (ionizedCount === 0) {
      return {
        body,
        compressed: false,
    ...
    const stats = createCompressionStats(body, newBody, "stacked", ["ionizer"], [`ionizer-${ionizedCount}-arrays-sampled`], durationMs);
    return {
      body: newBody,
      compressed: true,
      stats
