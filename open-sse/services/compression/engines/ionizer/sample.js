...
import { storeBlock } from "../ccr/index";
...
export function schemaUnion(rows) {
  const seen = new Set();
  const out = [];
...
export function seededSample(pool, k, seed) {
 ...
 */
export function ionize(rows, opts) {
  const n = rows.length;
  if (n <= opts.targetRows) return {
    kept: rows,
    keptCount: n,
    totalCount: n
  };
  ...
  return {
    kept: keptIdx.map(i => rows[i]),
    keptCount: keptIdx.length,
    totalCount: n
  };
}
 ...
 */
export function applyIonizerPass(messages, opts) {
  try {
    let ionizedCount = 0;
 ...
 */
export function runIonizerPass(messages, stepConfig, principalId) {
  if (stepConfig["enabled"] === false) return {
    messages,
    ionizedCount: 0
  };
  ...
  return applyIonizerPass(messages, {
    threshold,
    targetRows,
    principalId
  });
