...
const WRITE_NAMES = new Set(["write", "write_file", "writefile", "edit", "edit_file", "multiedit", "str_replace", "str_replace_editor", "str_replace_based_edit_tool", "apply_patch", "create_file", "update_file"]);
 ...
 */
export function extractInvocations(messages) {
  const invocations = [];
 ...
 */
export function findSupersededReadCallIds(invocations) {
...
function replaceResultText(content, stub) {
  ...
  if (Array.isArray(content)) {
    return content.map(part => isRecord(part) && part.type === "text" && typeof part.text === "string" ? {
      ...part,
      text: stub
    } : part);
  }
 ...
 */
export function collapseSupersededReads(messages, superseded, readPathByCallId) {
    ...
    if (Array.isArray(msg?.content)) {
      ...
      const newContent = msg.content.map(block => {
        if (isRecord(block) && block.type === "tool_result" && typeof block.tool_use_id === "string" && superseded.has(block.tool_use_id)) {
          ...
          return {
            ...block,
            content: replaceResultText(block.content, stubFor(path))
          };
        }
        return block;
      });
...
export const readLifecycleEngine = {
  id: ENGINE_ID,
  ...
  apply(body, options) {
    ...
    try {
      ...
      const {
        ...
        collapsedCount
      } = collapseSupersededReads(messages, superseded, readPathByCallId);
      if (collapsedCount === 0) {
        return {
          body,
          compressed: false,
      ...
      const stats = createCompressionStats(body, newBody, "stacked", [ENGINE_ID], [`read-lifecycle-collapsed-${collapsedCount}`], durationMs);
      return {
        body: newBody,
        compressed: true,
        stats
