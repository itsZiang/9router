...
import { ultraCompressHeuristic } from "../ultra";
import { createCompressionStats } from "../stats";
import { adaptBodyForCompression } from "../bodyAdapter";
import { DEFAULT_AGGRESSIVE_CONFIG, DEFAULT_ULTRA_CONFIG } from "../types";
...
const CAVEMAN_SCHEMA = [{
  ...
  label: "Intensity",
  defaultValue: "full",
  options: CAVEMAN_INTENSITIES.map(value => ({
    value,
    label: value
  }))
}, {
...
function validateCavemanLikeConfig(config) {
  ...
  if (config.minMessageLength !== undefined && (typeof config.minMessageLength !== "number" || config.minMessageLength < 0)) {
...
function validateLiteConfig(config) {
  const errors = [];
  if (config.preserveSystemPrompt !== undefined && typeof config.preserveSystemPrompt !== "boolean") {
...
export const aggressiveEngine = {
  ...
  apply(body, options) {
    ...
    return {
      ...
      stats: createCompressionStats(adapter.body, compressedBody, "aggressive", ["aggressive"], result.stats.rulesApplied, result.stats.durationMs)
    };
  },
  compress(body, config) {
    return this.apply(body, {
...
export const ultraEngine = {
  ...
  apply(body, options) {
    ...
    return {
      body: adapter.restore(compressedBody),
      compressed: result.stats.savingsPercent > 0,
      stats: createCompressionStats(adapter.body, compressedBody, "ultra", ["ultra"], result.stats.rulesApplied, result.stats.durationMs)
    };
  },
  compress(body, config) {
    return this.apply(body, {
