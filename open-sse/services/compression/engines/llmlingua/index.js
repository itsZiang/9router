...
import { workerBackend } from "./worker";
 ...
 */
export function setLlmlinguaBackend(b) {
  _backend = b;
}
  ...
  for (const seg of segments) {
    if (seg.kind === "preserved") {
      ...
      const {
        text: out,
        didCompress
      } = await compressProseText(seg.text, backend, opts);
      parts.push(out);
    ...
    try {
      if (typeof msg.content === "string") {
        const {
          text,
          didCompress
        } = await compressMessageText(msg.content, backend, opts);
        if (didCompress) {
          compressedCount++;
          result.push({
            ...msg,
        ...
        for (const part of msg.content) {
          if (part["type"] === "text" && typeof part["text"] === "string") {
            const {
              text,
              didCompress
            } = await compressMessageText(part["text"], backend, opts);
            if (didCompress) {
              changed = true;
              compressedCount++;
              newContent.push({
...
const LLMLINGUA_SCHEMA = [{
  ...
  options: Object.values(LLMLINGUA_MODELS).map(m => ({
    value: m.id,
    label: m.label
  }))
}, {
  ...
  key: "modelPath",
  type: "string",
  label: "Model path (offline override)",
  defaultValue: ""
}];
...
export const llmlinguaEngine = {
  ...
  async applyAsync(body, options) {
    ...
    const minTokens = typeof stepConfig["minTokens"] === "number" ? stepConfig["minTokens"] : 2000;
    if (minTokens > 0) {
    ...
    const backendOpts = {
      ...
      compressionRate: typeof stepConfig["compressionRate"] === "number" ? stepConfig["compressionRate"] : undefined,
      modelPath: typeof stepConfig["modelPath"] === "string" && stepConfig["modelPath"] ? stepConfig["modelPath"] : undefined
    };
    try {
      ...
      const {
        ...
        compressedCount
      } = await processMessages(messages, backend, backendOpts);
      ...
      const stats = createCompressionStats(body, newBody, "stacked", [ENGINE_ID], [`llmlingua-compressed-${compressedCount}-messages`], durationMs);
      return {
        body: newBody,
        compressed: true,
        stats
