...
import { resolveLlmlinguaModel, configureTransformersEnv } from "./modelStore";
/**
...
async function getCompressor(entry, modelPath) {
  ...
  const {
    LLMLingua2
  } = await dynamicImport("@atjsh/llmlingua-2");
  const {
    Tiktoken
  } = await dynamicImport("js-tiktoken/lite");
  const o200k_base = (await dynamicImport("js-tiktoken/ranks/o200k_base")).default;
  const oai = new Tiktoken(o200k_base);

  const {
    promptCompressor
  } = await LLMLingua2[entry.factory](entry.hfRepo, {
    transformerJSConfig: {
      device: "cpu",
      dtype: entry.dtype
    },
...
if (parentPort) {
  parentPort.on("message", async msg => {
    ...
    try {
      ...
      const out = await compressor.compress(text, {
        rate
      });
