...
import { workerBackend, depsAvailable } from "./worker";
import { DEFAULT_LLMLINGUA_MODEL } from "./constants";
...
export function __setUltraSlmTestHooks(hooks) {
  _testHooks = hooks;
}
 ...
 */
export async function runLlmlinguaUltra(text, opts) {
  if (_testHooks?.run) {
 ...
 */
export async function prewarmLlmlinguaUltra(opts) {
  ...
  try {
    ...
    await runLlmlinguaUltra("The quick brown fox jumps over the lazy dog while the sun sets behind the hills.", {
      model: opts?.model ?? DEFAULT_LLMLINGUA_MODEL,
      compressionRate: opts?.compressionRate
    });
  } catch {
...
export function __resetUltraEntryForTests() {
  _slmAvailable = null;
  _testHooks = null;
}