...
import { LLMLINGUA_WORKER_TIMEOUT_MS, LLMLINGUA_WORKER_IDLE_MS } from "./constants";
import { resolveLlmlinguaModel } from "./modelStore";
...
const WORKER_JS_REL = path.join("open-sse", "services", "compression", "engines", "llmlingua", "onnxWorker.js");
const WORKER_TS_REL = path.join("open-sse", "services", "compression", "engines", "llmlingua", "onnxWorker.ts");
const MAX_WALK_UP = 8;
 ...
 */
export function firstAncestorWith(anchors, relPath) {
...
export function resolveWorkerFile() {
  ...
  if (jsRoot) return {
    workerFile: path.join(jsRoot, WORKER_JS_REL),
    execArgv: []
  };
  ...
  if (tsRoot) return {
    workerFile: path.join(tsRoot, WORKER_TS_REL),
    execArgv: ["--import", "tsx/esm"]
  };

  // Nothing found — return a cwd-relative .js path; the spawn will fail-open.
  return {
    workerFile: path.join(process.cwd(), WORKER_JS_REL),
    execArgv: []
  };
}
 ...
 */
export const workerBackend = async (text, opts) => {
 ...
 */
export function __resetLlmlinguaWorkerForTests() {
