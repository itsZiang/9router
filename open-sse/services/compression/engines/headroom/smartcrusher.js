...
import { encodeTabularBlock, wrapTabular, kindOf } from "./tabular";
 ...
 */
export function detectHomogeneous(arr) {
 ...
 */
export function tryCompactJson(jsonStr, minRows = DEFAULT_MIN_ROWS) {
  let parsed;
  try {
 ...
 */
export function pickSmallestEncoding(arr) {
 ...
 */
export function collectCompactableArrays(messages, minRows = DEFAULT_MIN_ROWS) {
  const out = [];
  ...
  for (const msg of messages) {
    ...
    if (typeof msg.content === "string") scanText(msg.content);else if (Array.isArray(msg.content)) {
      for (const part of msg.content) {
        if (part["type"] === "text" && typeof part["text"] === "string") scanText(part["text"]);
      }
    }
  }
  return out;
 ...
 */
export function crushText(text, minRows = DEFAULT_MIN_ROWS) {
 ...
 */
export function crushMessages(messages, minRows = DEFAULT_MIN_ROWS) {
  let changed = false;
