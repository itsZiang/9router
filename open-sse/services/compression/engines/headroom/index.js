...
import { crushMessages, DEFAULT_MIN_ROWS } from "./smartcrusher";
import { TABULAR_FENCE_OPEN, TABULAR_FENCE_CLOSE, GCF_FENCE_OPEN, GCF_FENCE_CLOSE, decodeTabular } from "./tabular";
import { TOON_FENCE_OPEN, TOON_FENCE_CLOSE } from "./toon";
...
export const headroomEngine = {
  ...
  apply(body, options) {
    ...
    const {
      ...
      changed
    } = crushMessages(messages, minRows);
    if (!changed) {
      return {
        body,
        compressed: false,
    ...
    const stats = createCompressionStats(body, newBody, "stacked", ["headroom-smartcrusher"], ["tabular-compaction"], durationMs);
    return {
      body: newBody,
      compressed: true,
      stats
 ...
 */
export function reconstructHeadroom(body) {
  const messages = body["messages"];
...
function restoreText(text) {
  ...
  if (!text.includes(TABULAR_FENCE_OPEN) && !text.includes(GCF_FENCE_OPEN) && !text.includes(TOON_FENCE_OPEN)) return text;
  let result = text;
