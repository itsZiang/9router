...
export function needsQuote(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c === 0x22 || c === 0x5c || c < 0x20 || c === 0x0a || c === 0x0d || c === 0x7c || c === 0x2c) return true; // " \ control \n \r | ,
    // C1 control characters
    if (c >= 0x80 && c <= 0x9f) return true;
    // Unicode whitespace beyond ASCII
    if (c > 0x7f && (c === 0xa0 || c === 0x2028 || c === 0x2029 || c === 0xfeff || c === 0x1680 || c >= 0x2000 && c <= 0x200a || c === 0x202f || c === 0x205f || c === 0x3000)) return true;
  }
  return false;
}
...
export function formatScalar(v, delimiter = 0) {
...
export function parseScalar(s, tabularContext) {
  if (s === "") return "";
...
export function parseQuotedString(s) {
...
export function splitRespectingQuotes(s, delim) {
  const parts = [];
  let current = "";
  let inQuote = false;
  let escaped = false;
...
export function splitFieldDecl(s) {
