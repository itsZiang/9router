...
export function setLlmCompressorBackend(b) {
  _backend = b;
}
...
function splitProseAndPreserved(text) {
  ...
  for (const part of withPlaceholders.split(splitRe)) {
    ...
    segments.push(original !== undefined ? {
      kind: "preserved",
      text: original
    } : {
      kind: "prose",
...
const ENGINE_ID = "llm";
export const llmCompressorEngine = {
  id: ENGINE_ID,
  ...
  async applyAsync(body, options) {
    ...
    try {
      ...
      const stats = createCompressionStats(body, newBody, "stacked", [ENGINE_ID], [`llm-compressed-${compressedCount}-messages`], durationMs);
      return {
        body: newBody,
        compressed: true,
        stats
