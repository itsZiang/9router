import { MCP_ACCESSIBILITY_DEFAULTS } from "./constants";
...
export function extractRefs(text) {
  ...
  const pattern = new RegExp(MCP_ACCESSIBILITY_DEFAULTS.preserveRefPattern.source, MCP_ACCESSIBILITY_DEFAULTS.preserveRefPattern.flags);
...
export function findNthSiblingEnd(lines, start, indent, role, n) {
  let count = 0;
...
export function findLastNSiblingStart(lines, end, indent, role, n) {
  const positions = [];
  for (let k = 0; k < end; k++) {
    const mm = lines[k].match(SIBLING_PATTERN);
...
export function collapseRepeated(text, threshold, keepHead, keepTail) {
  ...
  while (i < lines.length) {
    ...
    if (groupLen >= threshold) {
      ...
      out.push(`${indent}... [${groupLen - keepHead - keepTail} similar "${role}" items omitted by OmniRoute MCP filter]`);
      ...
      if (omittedRefs.length > 0) {
        out.push(`${indent}  [refs of omitted "${role}" items (clickable): ${omittedRefs.join(" ")}]`);
      }
