...
const SECRET_PATTERNS = [[/\b(sk-[A-Za-z0-9_-]{16,})\b/g, "[REDACTED_OPENAI_KEY]"], [/\b(xox[baprs]-[A-Za-z0-9-]{16,})\b/g, "[REDACTED_SLACK_TOKEN]"], [/\b(AKIA[0-9A-Z]{16})\b/g, "[REDACTED_AWS_KEY]"],
...
[/((?:api[_-]?key|api[_-]?token|access[_-]?key|access[_-]?token|client[_-]?secret|auth[_-]?token|private[_-]?key|secret[_-]?key|credentials?|token|secret|password)\s*[:=]\s*)("[^"]+"|'[^']+'|[^\s]+)/gi, "$1[REDACTED]"],
...
export function redactRtkRawOutput(value) {
  let redacted = false;
  let text = value;
...
export function isLikelyFailureOutput(value) {
  return /\b(error|failed|failure|exception|traceback|panic|fatal|critical|TS\d{4}|FAIL)\b/i.test(value);
}
export function maybePersistRtkRawOutput(raw, options) {
...
export function readRtkRawOutput(pointerId) {
  const dir = path.join(dataDir(), "rtk", "raw-output");
  if (!fs.existsSync(dir)) return null;
  const entry = fs.readdirSync(dir).find(file => file.endsWith(".log") && file.includes(pointerId));
 ...
 */
export function listRtkCommandSamples(opts = {}) {
  const dir = path.join(dataDir(), "rtk", "raw-output");
