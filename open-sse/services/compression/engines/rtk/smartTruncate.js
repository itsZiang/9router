export function smartTruncate(text, options = {}) {
  ...
  const priorityLines = priorityPatterns.length ? lines.filter(line => priorityPatterns.some(pattern => pattern.test(line))) : [];
  ...
  let result = [...selected.slice(0, head.length), `[rtk:truncated ${droppedLines} lines]`, ...selected.slice(head.length)].join("\n");
