...
import { DEFAULT_RTK_CONFIG } from "../../types";
import { detectCommandType } from "./commandDetector";
import { RTK_SCHEMA, validateRtkEngineConfig } from "./configSchema";
import { deduplicateRepeatedLines } from "./deduplicator";
import { groupSimilarLines } from "./grouper";
import { matchRtkFilter } from "./filterLoader";
import { applyLineFilter } from "./lineFilter";
import { smartTruncate } from "./smartTruncate";
import { normalizeCodeLanguage, stripCode } from "./codeStripper";
import { maybePersistRtkRawOutput } from "./rawOutput";
import { applyRenderer } from "./renderers/index";
import { isTextBlock } from "../../messageContent";
import { adaptBodyForCompression } from "../../bodyAdapter";
import { isAnthropicToolResultBlock } from "../../toolResultCompressor";
...
function hasCacheControlMarker(part) {
  return !!part && typeof part === "object" && part.cache_control !== undefined && part.cache_control !== null;
}
...
function shouldCompressMessage(message, config) {
  ...
  if (config.applyToToolResults && Array.isArray(message.content) && message.content.some(isAnthropicToolResultBlock)) return true;
  if (message.role === "tool") return config.applyToToolResults || config.applyToCodeBlocks && hasCodeFence(message.content);
  if (message.role === "assistant") return config.applyToAssistantMessages || config.applyToCodeBlocks && hasCodeFence(message.content);
  return false;
}
...
function processRtkCodeBlocksOnly(content, config) {
  ...
  const processText = text => {
    ...
    const nextText = text.replace(/```([\s\S]*?)```/g, match => {
      const processed = processRtkText(match, {
        config
      });
...
export function processRtkText(text, options = {}) {
  ...
  const hasGenericErrorMarkers = /Error:|Exception:|Traceback \(most recent call last\):/.test(text);
  ...
  const deduped = deduplicateRepeatedLines(result, {
...
function processToolResultBlocks(content, config, toolCallLookup) {
  ...
  const nextContent = content.map(part => {
    ...
    const {
      command,
      skipFilters
    } = resolveToolMeta(toolUseId, toolCallLookup);
    const inner = part.content;
    if (typeof inner === "string") {
      if (!inner) return part;
      const processed = processRtkText(inner, {
        config,
        command,
        skipFilters
      });
    ...
    if (Array.isArray(inner)) {
      ...
      const nextInner = inner.map(sub => {
        ...
        const processed = processRtkText(sub.text, {
          config,
          command,
          skipFilters
        });
 ...
 */
export function effectiveMaxLines(base, intensity) {
  const factor = intensity === "aggressive" ? 0.5 : intensity === "minimal" ? 1.5 : 1;
  return Math.max(1, Math.round(base * factor));
}
export function applyRtkCompression(body, options = {}) {
  const start = performance.now();
  const stepConfig = options.stepConfig && options.stepConfig.enabled === undefined ? {
    enabled: true,
    ...options.stepConfig
  } : options.stepConfig;
  const explicitConfig = options.config && Object.keys(options.config).length > 0;
  const baseConfig = !explicitConfig && !stepConfig ? {
    enabled: true
  } : options.config ?? {};
  ...
  const compressedMessages = messages.map(message => {
    ...
    if (message.role === "tool") {
      ...
      ({
        command,
        skipFilters
      } = resolveToolMeta(callId, toolCallLookup));
    }
  ...
  const stats = createCompressionStats(adapter.body, compressedBody, "rtk", [...new Set(allTechniques)], allRules.length > 0 ? [...new Set(allRules)] : undefined, Math.round((performance.now() - start) * 100) / 100);
  stats.engine = "rtk";
...
export { detectCommandFromText, detectCommandOutput, detectCommandType } from "./commandDetector";
export { runRtkFilterTests } from "./verify";
export { maybePersistRtkRawOutput, readRtkRawOutput, redactRtkRawOutput, listRtkCommandSamples } from "./rawOutput";
// RTK learn/discover: the sample-source adapter (rawOutput) feeds these pure miners.
export { discoverRepeatedNoise } from "./discover";
export { suggestFilter, commandToId } from "./learn";