...
export function isAnthropicToolResultBlock(value) {
 ...
 */
export function compressAnthropicToolResultBlock(block, opts) {
  const content = block.content;
  if (typeof content === "string") {
    if (!content || COMPRESSED_MARKER_RE.test(content)) return {
      block,
      saved: 0
    };
    const result = compressToolResult(content, opts);
    if (result.strategy === "none" || result.saved <= 0) return {
      block,
      saved: 0
    };
    return {
...
export function compressToolResult(content, opts) {
  if (opts.fileContent) {
