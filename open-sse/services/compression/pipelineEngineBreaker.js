...
export const DEFAULT_PIPELINE_BREAKER = {
  enabled: false,
  failureThreshold: 3,
  cooldownMs: 30_000
};
 ...
 */
export function resolvePipelineBreakerConfig(partial, env = process.env) {
  const enabled = partial?.enabled ?? env.COMPRESSION_PIPELINE_BREAKER_ENABLED === "true";
  const failureThreshold = partial?.failureThreshold ?? toNonNegativeInt(env.COMPRESSION_PIPELINE_BREAKER_THRESHOLD, DEFAULT_PIPELINE_BREAKER.failureThreshold);
  const cooldownMs = partial?.cooldownMs ?? toNonNegativeInt(env.COMPRESSION_PIPELINE_BREAKER_COOLDOWN_MS, DEFAULT_PIPELINE_BREAKER.cooldownMs);
  return {
 ...
 */
export function canRunEngine(engine, config, now = Date.now()) {
...
export function recordEngineFailure(engine, config, now = Date.now()) {
...
export function recordEngineSuccess(engine, config) {
  if (!config.enabled) return;
  const s = get(engine);
  s.failures = 0;
  s.openedUntil = null;
...
export function getEngineBreakerState(engine) {
  const s = _state.get(engine);
  return {
    failures: s?.failures ?? 0,
    open: s?.openedUntil != null
  };
}

/** Clear all breaker state (tests + operator reset). */
export function resetPipelineEngineBreakers() {
  _state.clear();
}