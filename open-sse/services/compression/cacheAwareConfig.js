...
import { normalizePreserveSystemPromptMode, resolvePreserveSystemPrompt } from "./preserveSystemPromptMode";
import { resolvePrefixFreezeConfig, extractStablePrefixHash, observePrefix, isPrefixFrozen } from "./prefixFreeze";
 ...
 */
export function resolveCacheAwareConfig(config, body, context) {
  ...
  const effective = resolvePreserveSystemPrompt(mode, {
    hasCache
  });
  if (effective === config.preserveSystemPrompt) return config;
  return {
    ...config,
    preserveSystemPrompt: effective
  };
}