...
const CRITICAL_KINDS = new Set(["url", "const_case", "env_var", "version", "dotted_identifier", "function_call", "file_path", "inline_code"]);
 ...
 */
export function checkFidelity(inputText, outputText, cfg) {
  try {
    const tokens = uniq(extractPreservedBlocks(inputText).blocks.filter(b => CRITICAL_KINDS.has(b.kind)).map(b => b.content.trim()).filter(c => c.length > 0));
    ...
    if (keys.length > 0) {
      ...
      const keyRatio = survivalRatio(keys.map(k => `"${k}"`), outputText);
      if (keyRatio < minKey) {
        return {
          passed: false,
