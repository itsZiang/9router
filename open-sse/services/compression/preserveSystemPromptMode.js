 ...
 */
export const PRESERVE_SYSTEM_PROMPT_MODES = ["always", "whenNoCache", "never"];
export function isPreserveSystemPromptMode(value) {
 ...
 */
export function normalizePreserveSystemPromptMode(config) {
 ...
 */
export function resolvePreserveSystemPrompt(mode, {
  hasCache
}) {
  switch (mode) {
    case "always":
 ...
 */
export function modeToBaselineBoolean(mode) {
  return resolvePreserveSystemPrompt(mode, {
    hasCache: false
  });
}