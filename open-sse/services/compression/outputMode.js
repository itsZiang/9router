import { DEFAULT_CAVEMAN_OUTPUT_MODE_CONFIG } from "./types";
 ...
 */
export const SHARED_BOUNDARIES = "Code blocks, file paths, commands, errors, URLs: keep exact. Security warnings, irreversible action confirmations, multi-step ordered sequences: write normal. Resume terse style after. Active every response until user asks for normal mode.";
...
export function shouldBypassCavemanOutputMode(messages) {
  ...
  if (/\b(clarify|explain in detail|more detail|step by step|why exactly|what do you mean)\b/.test(text)) {
    return "clarification_requested";
  }
  if (/\b(first|then|after that|before|rollback|backup)\b[\s\S]{0,240}\b(delete|drop|migrate|deploy|release)\b/.test(text)) {
    return "order_sensitive_sequence";
  }
  return null;
}
export function buildCavemanOutputInstruction(config, language = "en") {
...
export function applyCavemanOutputMode(body, options, language = "en") {
  const config = {
  ...
  const alreadyApplied = messages.some(message => message.role === "system" && typeof message.content === "string" && message.content.includes(CAVEMAN_OUTPUT_MARKER));
