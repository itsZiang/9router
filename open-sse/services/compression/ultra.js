import { pruneByScore } from "./ultraHeuristic";
import { extractPreservedBlocks } from "./preservation";
import { DEFAULT_ULTRA_CONFIG } from "./types";
import { extractTextContent, mapTextContent } from "./messageContent";
import { slmAvailable, runLlmlinguaUltra, prewarmLlmlinguaUltra } from "./engines/llmlingua/ultraEntry";
 ...
 */
export function ultraCompressHeuristic(messages, config = {}, tier = "heuristic") {
 ...
 */
export async function ultraCompress(messages, config = {}) {
    ...
    for (const msg of messages) {
      ...
      const next = await mapTextContentAsync(msg, async textPart => {
        ...
        const {
          text: out,
          usedSlm
        } = await compressProseSlm(textPart, effectiveConfig);
