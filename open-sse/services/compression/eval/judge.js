 ...
 */
export function buildJudgePrompt(fullAnswer, compressedAnswer) {
  return [{
    role: "system",
 ...
 */
export async function runSelfTest(client, judgeModel) {
  const goodVerdict = parseJudgeVerdict((await client.complete(judgeModel, buildJudgePrompt(CONTROL_PAIR.reference, CONTROL_PAIR.good))).text);
  const degradedVerdict = parseJudgeVerdict((await client.complete(judgeModel, buildJudgePrompt(CONTROL_PAIR.reference, CONTROL_PAIR.degraded))).text);
  ...
  return {
    passed: true,
    detail: "control pair ranked correctly"
  };
}