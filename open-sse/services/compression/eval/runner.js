import { selectCompressionPlan, applyCompressionAsync } from "../strategySelector";
import { estimateCompressionTokens } from "../stats";
import { loadCorpus, hashCorpus } from "./corpus";
import { buildJudgePrompt, parseJudgeVerdict, runSelfTest } from "./judge";
import { buildGradePrompt, parseGradeVerdict } from "./grader";
...
export async function runEval(opts) {
  ...
  for (const c of cases) {
    ...
    try {
      ...
      const compressed = await opts.client.complete(opts.answerModel, [{
        role: "user",
        content: answerText(compressedBody)
      }]);
  ...
  const report = aggregateRecords(records, stamps, {
    partial,
    totalCostUsd: meter.spent
  });
  return {
    aborted: false,
    report
