...
const RETENTION_KINDS = new Set(["url", "markdown_link", "const_case", "env_var", "version", "dotted_identifier", "function_call", "file_path", "error_message", "inline_code"]);
...
export function computeRetention(original, compressed) {
  ...
  for (const entity of entities) {
    if (compressed.includes(entity)) survived++;else lost.push(entity);
  }
  return {
    total: entities.length,
    survived,
...
export function measureCompression(original, compressed) {
  ...
  const savingsPercent = originalTokens > 0 ? Math.round((originalTokens - compressedTokens) / originalTokens * 1000) / 10 : 0;
  return {
    originalTokens,
    compressedTokens,
    savingsPercent,
