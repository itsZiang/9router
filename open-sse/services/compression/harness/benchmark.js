...
import { getCompressionEngine } from "../engines/registry";
import { runCompressionEval } from "./runner";
import { checkTokensPerTaskGate } from "./budgetGate";
...
// ── Prose ────────────────────────────────────────────────────────────────
{
  id: "prose-1",
  task: "prose",
  input: ["Actually, I think what you basically want to do is essentially iterate over the list", "of files and then kind of process each one in turn. So basically what I mean is that", "you should loop through them one by one. In other words, just go through each file", "in the directory and perform the operation. Does that make sense? Let me know if you", "need any clarification on this matter whatsoever.", "", "So anyway, the main takeaway here is that you need to call processFile() for each", "item in the files array. That is essentially the core of what I am trying to convey."].join("\n")
}, {
  id: "prose-2",
  task: "prose",
  input: ["I would like to add that, at the end of the day, the solution involves calling", "https://api.example.com/v2/process with the API_KEY environment variable set.", "The endpoint version is v2.3.1 and you must pass X-Request-Id in the headers.", "", "Please note that basically every request needs authentication via the Bearer token.", "So in other words you should include Authorization: Bearer <token> in every call.", "The error.message field will contain details when a 401 or 403 occurs."].join("\n")
},
// ── Tool output ──────────────────────────────────────────────────────────
{
  id: "tool-output-1",
  task: "tool-output",
  input: ["$ npm install", "npm warn deprecated inflight@1.0.6: This module is not supported", "npm warn deprecated inflight@1.0.6: This module is not supported", "npm warn deprecated inflight@1.0.6: This module is not supported", "npm warn deprecated glob@7.2.3: Glob versions prior to v9 are no longer supported", "npm warn deprecated glob@7.2.3: Glob versions prior to v9 are no longer supported", "npm warn deprecated glob@7.2.3: Glob versions prior to v9 are no longer supported", "npm warn deprecated rimraf@3.0.2: Rimraf versions prior to v4 are no longer supported", "", "added 1234 packages, and audited 1234 packages in 42s", "", "found 0 vulnerabilities"].join("\n")
}, {
  id: "tool-output-2",
  task: "tool-output",
  input: ["tests/unit/foo.test.ts ..........", "tests/unit/bar.test.ts ..........", "tests/unit/baz.test.ts ..........", "tests/unit/qux.test.ts ..........", "tests/unit/foo.test.ts ..........", "tests/unit/bar.test.ts ..........", "tests/unit/baz.test.ts ..........", "tests/unit/qux.test.ts ..........", "  ✓ all 80 tests passed", "  coverage: 87.3% statements", "Error: ENOENT: no such file or directory '/tmp/cache/build.lock'"].join("\n")
},
// ── JSON / structured ────────────────────────────────────────────────────
{
  id: "json-1",
 ...
 */
export function engineToCompressFn(engineId) {
 ...
 */
export async function benchmarkEngines(corpus, engineIds) {
  const reports = {};
 ...
 */
export function runBenchmarkGate(reports, baselines, tolerancePercent = 2) {
 ...
 */
export const DEFAULT_BENCHMARK_ENGINES = ["lite", "caveman", "aggressive", "ultra", "rtk", "session-dedup", "headroom", "ccr"];
