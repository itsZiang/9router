...
export function withSource(plan, source) {
  return {
    ...plan,
    source
  };
 ...
 */
export function planFromHeader(config, header, combos) {
  const h = header.trim();
  if (!h) return null;
  const lower = h.toLowerCase();
  if (lower === "off") return withSource({
    mode: "off",
    stackedPipeline: []
  }, "request-header");
  if (lower === "default") {
  ...
  if (lower.startsWith("engine:")) {
    ...
    return engine?.enabled ? withSource(deriveDefaultPlan({
      [id]: engine
    }, true), "request-header") : null;
  }
  const combo = combos[lower] ?? combos[h];
  return combo ? withSource({
    mode: "stacked",
    stackedPipeline: combo
  }, "request-header") : null;
}

/** Renders the X-OmniRoute-Compression response header value. */
export function formatCompressionMeta(plan) {
 ...
 */
export function formatCompressionAnnotation(stats) {
 ...
 */
export function buildNamedComboLookup(combos) {
  const map = {};
 ...
 */
export function deriveDefaultPlanFromConfig(config, comboId, combos = {}) {
  if (config.enginesExplicit) {
    ...
    return resolveCompressionPlan(config, {
      comboId,
      combos
    });
  }
