...
import { countTextTokens } from "../../stubs/shared/utils/tiktokenCounter";
...
export function applyHardBudget(body, opts) {
  const {
    targetTokens,
    targetRatio
  } = opts;
  ...
  const resultTokens = countTextTokens(usedMessages.map(m => typeof m.content === "string" ? m.content : JSON.stringify(m.content)).join(" "));
