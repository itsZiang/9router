import { RISK_PATTERNS } from "./riskPatterns";
 ...
 */
export function detectRiskSpans(text, cfg) {
  try {
    if (!cfg.enabled) return [];
    if (!text) return [];
    const enabled = new Set(cfg.categories?.length ? cfg.categories : ["stack_trace", "private_key", "secret_assignment", "k8s_secret", "db_migration", "legal"]);
    ...
    const ddlPromoted = ddl.length >= MIN_DDL ? [{
      start: ddl[0].start,
      end: ddl[ddl.length - 1].end,
      category: "db_migration"
    }] : [];

    // Guarded categories: secret_assignment, stack_trace, legal.
    const guarded = regexHits.filter(h => h.category === "secret_assignment" || h.category === "stack_trace" || h.category === "legal");
    ...
    const promoted = [...selfEvident.map(h => ({
      start: h.start,
      end: h.end,
      category: h.category
    })), ...k8sHits.map(h => ({
      ...
      category: h.category
    })), ...ddlPromoted, ...guardedPromoted.map(h => ({
      start: h.start,
      end: h.end,
      category: h.category
    }))];
