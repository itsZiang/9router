...
import { applyCcBridgeTransformPipeline, CLAUDE_AGENT_SDK_IDENTITY, DEFAULT_CC_BRIDGE_PIPELINE, DEFAULT_IDENTITY_PREFIXES, DEFAULT_PARAGRAPH_REMOVAL_ANCHORS, DEFAULT_TEXT_REPLACEMENTS } from "./ccBridgeTransforms";
// Re-export base DSL types so external callers depend only on systemTransforms.
export { CLAUDE_AGENT_SDK_IDENTITY, DEFAULT_CC_BRIDGE_PIPELINE, DEFAULT_IDENTITY_PREFIXES, DEFAULT_PARAGRAPH_REMOVAL_ANCHORS, DEFAULT_TEXT_REPLACEMENTS };
...
"opencode", "open-code", "cline", "roo-cline", "roo_cline", "cursor", "windsurf", "aider", "continue.dev", "copilot", "avante", "codecompanion",
 ...
 * default pipeline.
 */
export const OPENWEBUI_PARAGRAPH_ANCHORS = ["github.com/open-webui/open-webui", "openwebui.com", "docs.openwebui.com"];

/** Open WebUI identity paragraph prefixes. */
export const OPENWEBUI_IDENTITY_PREFIXES = ["You are Open WebUI"];
export const PI_PARAGRAPH_ANCHORS = ["@earendil-works/pi-coding-agent", "/.pi/", "Pi documentation (read only when the user asks about pi itself"];
 ...
 */
export const DEFAULT_CLAUDE_PIPELINE = [
...
// opencode.ai/docs, cline, getcursor/cursor, continue.dev, Open WebUI, Pi docs).
{
  ...
  needles: [...DEFAULT_PARAGRAPH_REMOVAL_ANCHORS, ...OPENWEBUI_PARAGRAPH_ANCHORS, ...PI_PARAGRAPH_ANCHORS]
},
 ...
 */
export const DEFAULT_CC_BRIDGE_PROVIDER_PIPELINE = [
...
export const DEFAULT_SYSTEM_TRANSFORMS_CONFIG = {
  providers: {
    [PROVIDER_CLAUDE]: {
 ...
 */
export function applyTransformPipeline(body, pipeline) {
 ...
 */
export function applySystemTransformPipeline(providerId, body, config = getSystemTransformsConfig()) {
  if (!body || typeof body !== "object") return {
    body,
    appliedOpKinds: []
  };
 ...
 */
export function setSystemTransformsConfig(input) {
  ...
  if ("providers" in candidate && candidate.providers && typeof candidate.providers === "object") {
    ...
    for (const [providerId, providerDefault] of Object.entries(DEFAULT_SYSTEM_TRANSFORMS_CONFIG.providers)) {
...
export function resetSystemTransformsConfig() {
  _store[GLOBAL_KEY] = DEFAULT_SYSTEM_TRANSFORMS_CONFIG;
}