 ...
 */
export function injectCfClearance(existingCookie, cfClearanceToken) {
 ...
 */
export async function refreshCookie(existingCookie, options) {
  const {
    force = false,
    log
  } = options || {};
  try {
    ...
    const cfClearanceToken = await getCfClearanceToken({
      force
    });
 ...
 */
export async function fetchWithAutoRefresh(fetchFn, initialCookie, options) {
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      options?.log?.warn?.("CLAUDE-WEB-AUTO-REFRESH", `Auth error detected (attempt ${attempt}/${maxRetries}), refreshing cf_clearance...`);
      try {
 ...
 */
export function createAutoRefreshMiddleware(options) {
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        if ((response.status === 403 || response.status === 401) && attempt < maxRetries) {
          log?.warn?.("CLAUDE-WEB-AUTO-REFRESH", `HTTP ${response.status} - refreshing cf_clearance (attempt ${attempt}/${maxRetries})`);
          try {
            const refresh = await refreshCookie(currentCookie, {
              ...options,
              force: attempt > 1,
        ...
        log?.error?.("CLAUDE-WEB-AUTO-REFRESH", `Fetch failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
      }
    }
