...
export function stripGpt5SamplingWhenReasoning(body, provider, model, log) {
  ...
  log?.warn?.("PARAMS", `Stripped ${stripped.join(", ")} for reasoning-active ${model} ` + `(GPT-5 rejects sampling params unless reasoning_effort=none)`);
  return next;
}