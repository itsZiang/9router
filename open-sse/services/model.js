...
const CROSS_PROXY_MODEL_ALIASES_LOWER = Object.fromEntries(Object.entries(CROSS_PROXY_MODEL_ALIASES).map(([alias, canonical]) => [alias.toLowerCase(), canonical]));
...
const CODEX_PREFERRED_UNPREFIXED_MODELS = new Set(["gpt-5.5", "gpt-5.5-xhigh", "gpt-5.5-high", "gpt-5.5-medium", "gpt-5.5-low"]);
...
const CODEX_PREFERRED_UNPREFIXED_MODEL_ALIASES = new Map([]);
 ...
 */
export function resolveBareModelToConnectionDefault(requestedModelStr, resolvedModel, connectionDefaultModel) {
...
export function normalizeCrossProxyModelId(modelId) {
  ...
  const normalized = CROSS_PROXY_MODEL_ALIASES[modelId] || CROSS_PROXY_MODEL_ALIASES_LOWER[modelId.toLowerCase()];
...
function getInferredProvidersForModel(modelId) {
  const providers = [...(MODEL_TO_PROVIDERS.get(modelId) || [])];
  if (CODEX_PREFERRED_UNPREFIXED_MODELS.has(modelId) && hasCodexPreferredUnprefixedModel(modelId) && !providers.includes("codex")) {
...
async function getActiveProviderSet() {
  try {
    ...
    const providers = conns.map(getProviderIdFromConnection).filter(provider => Boolean(provider));
...
function shouldPreferClaudeCodeForUnprefixedClaudeModel(modelId, activeProviders, preferClaudeCode) {
...
function resolveModelAliasTarget(alias, aliases) {
  ...
  if (resolved && typeof resolved === "object" && typeof resolved.provider === "string" && typeof resolved.model === "string") {
    const normalizedPair = normalizeCrossProxyModelId(`${resolved.provider}/${resolved.model}`).modelId;
...
async function resolveModelByProviderInference(modelId, extendedContext) {
  ...
  const [activeProviders, preferClaudeCodeForUnprefixedClaudeModels] = await Promise.all([getActiveProviderSet(), getPreferClaudeCodeForUnprefixedClaudeModels()]);
  ...
  if (activeProviders?.has("codex") && !activeProviders.has("openai") && providers.includes("codex") && CODEX_PREFERRED_UNPREFIXED_MODELS.has(modelId)) {
    return {
      provider: "codex",
  ...
  if (candidatesToUse.includes("claude") && shouldPreferClaudeCodeForUnprefixedClaudeModel(modelId, activeProviders, preferClaudeCodeForUnprefixedClaudeModels)) {
    return {
      provider: "claude",
  ...
  if (/^claude-/i.test(modelId)) {
    if (shouldPreferClaudeCodeForUnprefixedClaudeModel(modelId, activeProviders, preferClaudeCodeForUnprefixedClaudeModels)) {
      return {
        provider: "claude",
        model: modelId,
        extendedContext
...
export async function getModelInfoCore(modelStr, aliasesOrGetter) {
  ...
  if (aliases && typeof aliases === "object") {
    ...
    if (wildcardMatch) {
      const target = wildcardMatch.target;
      if (target.includes("/")) {
