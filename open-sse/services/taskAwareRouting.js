...
export const TASK_TARGET_POWER = {
  light: 35,
  standard: 65,
  heavy: 95,
  critical: 120
...
export const LIGHT_TASK_RE = /\b(hi|hello|thanks|thank you|ping|format|rewrite|grammar|translate|summari[sz]e|short|quick|one[- ]?liner|explain briefly)\b/i;
export const HEAVY_TASK_RE = /\b(debug|root cause|architecture|architectural|refactor|migrate|implementation|implement|design|analy[sz]e|investigate|compare|benchmark|whitebox|codebase|end[- ]?to[- ]?end|e2e)\b/i;
export const CRITICAL_TASK_RE = /\b(critical|security|vulnerability|exploit|rce|remote code execution|supply chain|account takeover|auth bypass|privilege escalation|tenant|cross[- ]tenant|sandbox escape|ssrf|deserialization|prod incident|data exfiltration|bug bounty)\b/i;

// ── Conversation affinity state ───────────────────────────────────────────────

/** @internal exported only for testing */
export const comboConversationAffinity = new Map();
 ...
 */
export function isTaskRoutingStrategy(strategy) {
...
function collectText(value, out = []) {
  ...
  if (typeof rec["content"] === "string") out.push(rec["content"]);else if (Array.isArray(rec["content"])) collectText(rec["content"], out);
...
function estimatePromptChars(body) {
  ...
  const parts = [body["system"], body["instructions"], body["messages"], body["input"], contents, body["query"], body["url"]];
...
function maxRequestedOutput(body) {
  ...
  const candidates = [body["max_tokens"], body["max_output_tokens"], body["max_completion_tokens"], genConf?.["maxOutputTokens"]].map(v => Number.parseInt(String(v ?? ""), 10)).filter(v => Number.isFinite(v));
  return candidates.length > 0 ? Math.max(...candidates) : 0;
}
function getTaskText(body) {
  const contents = body?.["contents"] ?? body?.["request"]?.["contents"];
  return collectText([body?.["system"], body?.["instructions"], body?.["messages"], body?.["input"], contents, body?.["query"], body?.["url"]]).join("\n");
}
...
export function getTaskSignals(body) {
...
export function classifyTask(body) {
  ...
  const critical = add(s.promptChars >= 100000, "huge-context") || add(s.outputTokens >= 32768, "huge-output") || add(s.toolCount >= 8 && s.promptChars >= 16000, "many-tools-large-context") || add(s.criticalKeyword && (effortIsHigh || s.toolCount >= 3 || s.promptChars >= 8000), "critical-domain");
  if (critical) {
    return {
      level: "critical",
      weight: taskWeight("critical"),
      ...s,
      reasons
    };
  }
  const heavySignalCount = [add(s.promptChars >= 50000, "large-context"), add(s.promptChars >= 24000, "medium-large-context"), add(s.messageCount >= 16, "long-conversation"), add(s.toolCount >= 4, "many-tools"), add(s.outputTokens >= 8192, "large-output"), add(effortIsHigh, "high-reasoning-effort"), add(s.criticalKeyword, "security-sensitive"), add(s.heavyKeyword && s.promptChars >= 4000, "complex-task")].filter(Boolean).length;
  ...
  const light = s.promptChars <= 2000 && s.messageCount <= 3 && s.toolCount === 0 && s.outputTokens <= 1500 && effortIsLight && !s.criticalKeyword && !s.heavyKeyword;
 ...
 */
export function modelPowerScore(modelStr) {
  ...
  if (ctx >= 1_000_000) score += 22;else if (ctx >= 400_000) score += 15;else if (ctx >= 200_000) score += 9;else if (ctx > 0 && ctx <= 32_000) score -= 10;
  const maxOut = caps.maxOutputTokens ?? 0;
  if (maxOut >= 128_000) score += 12;else if (maxOut >= 64_000) score += 8;else if (maxOut > 0 && maxOut <= 8_192) score -= 8;
 ...
 */
export function reorderByTaskWeight(targets, task = classifyTask({}), required = new Set()) {
  if (!Array.isArray(targets) || targets.length <= 1) return targets;
  const reordered = targets.map((t, i) => ({
    t,
    i,
    score: scoreModelForTask(t.modelStr, task, required)
  })).sort((a, b) => b.score - a.score || a.i - b.i).map(x => x.t);
...
function allRoleText(items, roles, contentKey = "content") {
  if (!Array.isArray(items)) return "";
  return normalizeFingerprintText(items.filter(item => !!item && typeof item === "object" && roles.has(String(item["role"] ?? ""))).map(item => collectText(contentKey === "parts" ? item["parts"] : item["content"]).join("\n")).filter(Boolean).join("\n"));
}
...
export function getConversationCacheKey(body) {
  ...
  const explicitCandidates = [body["conversation_id"], body["conversationId"], body["thread_id"], body["threadId"], body["session_id"], body["sessionId"], meta?.["conversation_id"], meta?.["conversationId"], meta?.["thread_id"], meta?.["threadId"], meta?.["session_id"], meta?.["sessionId"]];
  ...
  const contents = body["contents"] ?? body["request"]?.["contents"];
  const seedParts = [collectText(body["system"]).join("\n"), collectText(body["instructions"]).join("\n"), allRoleText(body["messages"] ?? [], systemRoles), allRoleText(body["input"] ?? [], systemRoles), allRoleText(contents ?? [], systemRoles, "parts"), firstRoleText(body["messages"] ?? [], userRoles), typeof body["input"] === "string" ? body["input"] : firstRoleText(body["input"] ?? [], userRoles), firstRoleText(contents ?? [], userRoles, "parts"), body["query"], body["url"]].filter(Boolean);
...
export function pruneConversationAffinity(now = Date.now()) {
 ...
 */
export function getOrSetConversationAffinityIndex(rotationKey, conversationCacheKey, currentIndex) {
  const now = Date.now();
  ...
  if (existing) {
    ...
    comboConversationAffinity.set(affinityKey, {
      index: pinnedIndex,
      lastUsed: now
    });
    return pinnedIndex;
 ...
 */
export function clearConversationAffinity(comboName) {
  if (comboName) {
