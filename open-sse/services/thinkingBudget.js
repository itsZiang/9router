...
import { capThinkingBudget, getDefaultThinkingBudget, getResolvedModelCapabilities, supportsReasoning } from "../stubs/lib/modelCapabilities";
 ...
 */
export function hydrateThinkingBudgetConfig(settings) {
...
export function normalizeThinkingLevel(body) {
  ...
  if (typeof geminiLevel === "string" && THINKING_LEVEL_MAP[geminiLevel.toLowerCase()] !== undefined) {
...
function stripThinkingConfig(body) {
  ...
  if (result.generationConfig) {
    const generationConfig = {
      ...toRecord(result.generationConfig)
    };
...
function applyAdaptiveBudget(body, cfg) {
  ...
  const budget = capThinkingBudget(getStringField(bodyRecord, "model"), Math.ceil(baseBudget * multiplier));
