 ...
 */
export function extractCodeAssistSubscriptionTier(subscriptionInfo) {
  ...
  if (!tier) {
    if (!isIneligible(subscription)) {
      ...
      if (defaultTier) {
        ...
        if (name) tier = `${name} (Restricted)`;else if (id) tier = `${id} (Restricted)`;
      }
    }
  }
  return tier;
 ...
 */
export function extractCodeAssistOnboardTierId(subscriptionInfo) {
  const subscription = toRecord(subscriptionInfo);
