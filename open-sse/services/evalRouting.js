import { listModelEvalRunsForRouting } from "../stubs/lib/db/evals";
...
const evalRoutingCache = new Map();
...
function toStringList(value, maxItems) {
  ...
  return [...new Set(value.map(entry => String(entry || "").trim()).filter(Boolean))].slice(0, maxItems);
}
...
function normalizeEvalRoutingConfig(rawConfig) {
  ...
  const qualityWeight = clampNumber(raw.qualityWeight, DEFAULT_EVAL_ROUTING_CONFIG.qualityWeight, 0, 1);
  const latencyWeight = clampNumber(raw.latencyWeight, DEFAULT_EVAL_ROUTING_CONFIG.latencyWeight, 0, 1);
  ...
  return {
    ...
    minCases: Math.floor(clampNumber(raw.minCases, DEFAULT_EVAL_ROUTING_CONFIG.minCases, 1, 100_000)),
    qualityWeight: weightTotal > 0 ? qualityWeight / weightTotal : 1,
    latencyWeight: weightTotal > 0 ? latencyWeight / weightTotal : 0,
    cacheTtlMs: Math.floor(clampNumber(raw.cacheTtlMs, DEFAULT_EVAL_ROUTING_CONFIG.cacheTtlMs, 1_000, 300_000))
  };
}
...
function calculateTargetScore(runs, config, bestLatencyMs) {
  const validRuns = dedupeLatestRunsBySuite(runs).filter(run => run.summary.total >= config.minCases);
  ...
  const latencyWeight = validRuns.reduce((sum, run) => sum + Math.max(0, run.avgLatencyMs) * run.summary.total, 0);
...
export function orderTargetsByEvalScores(targets, rawConfig, log = {}) {
  ...
  log.info?.("COMBO", `Eval-driven routing: ranked ${scoredCount}/${targets.length} targets by eval history`);
  log.debug?.("COMBO", `Eval-driven routing scores: ${entries.filter(entry => entry.score).map(entry => `${entry.target.modelStr}=${entry.score?.score.toFixed(3)} pass=${entry.score?.passRate.toFixed(1)} cases=${entry.score?.totalCases}`).join(", ")}`);
  return entries.map(entry => entry.target);
}
export function resetEvalRoutingCache() {
