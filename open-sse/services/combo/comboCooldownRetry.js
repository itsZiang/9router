 ...
 */
export const COMBO_COOLDOWN_NON_RETRYABLE_REASONS = new Set(["quota_exhausted", "auth_error", "not_found", "not_found_local"]);
 ...
 */
export const COMBO_COOLDOWN_RETRYABLE_REASONS = new Set(["rate_limit", "rate_limited", "transient", "overloaded", "server_error"]);
function toFiniteWaitMs(value) {
  if (typeof value === "number" && Number.isFinite(value) && value > 0) {
    return value;
  }
  return 0;
}
function isRetryableComboCooldownReason(reason) {
 ...
 *   - budgetLeftMs >= waitMs
 */
export function shouldWaitForComboCooldown(input) {
  const {
    reason,
    attempt,
    budgetLeftMs,
  ...
  const wait = settings.enabled === true && isRetryableComboCooldownReason(reason) && waitMs > 0 && waitMs <= settings.maxWaitMs && attempt < settings.maxAttempts && budgetLeftMs >= waitMs;
  return {
    wait,
    waitMs
  };
 ...
 */
export function resolveComboCooldownWaitDecision(input) {
  const {
    targets,
    earliestRetryAfter,
    attempt,
  ...
  for (const target of targets) {
    ...
    if (!info) continue;
    const remainingMs = typeof info.remainingMs === "number" && Number.isFinite(info.remainingMs) ? info.remainingMs : 0;
  ...
  const hintWaitMs = typeof rawHintWaitMs === "number" && Number.isFinite(rawHintWaitMs) && rawHintWaitMs > 0 ? rawHintWaitMs : 0;
