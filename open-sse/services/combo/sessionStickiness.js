...
import { computeHeadroom } from "./headroomRanking";
// ─── Constants ────────────────────────────────────────────────────────────────
...
export function __setStickinessHeadroomFetcherForTests(fetcher) {
...
async function resolveSaturation(connectionId, provider) {
  ...
  try {
    ...
    const [util5h, util7d] = await Promise.all([getSaturation(connectionId, provider, {
      unit: "percent",
      window: "5h"
    }), getSaturation(connectionId, provider, {
      unit: "percent",
      window: "weekly"
    })]);
    return {
 ...
 */
export function deriveMessageHash(messages) {
...
export function recordStickyBinding(messageHash, connectionId) {
...
export function clearStickyBinding(messageHash) {
  stickyMap.delete(messageHash);
}

/** Reset the entire store (for testing). */
export function clearAllStickyBindings() {
  stickyMap.clear();
}
 ...
 */
export async function applySessionStickiness(orderedTargets, messages) {
  const noOp = {
  ...
  try {
    ...
    const reordered = [orderedTargets[stickyIdx], ...orderedTargets.slice(0, stickyIdx), ...orderedTargets.slice(stickyIdx + 1)];
