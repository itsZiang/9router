 ...
 * @param nowMs         Current epoch ms; defaults to Date.now() (off-path only).
 */
export function decrementInflight(connectionId, nowMs = Date.now()) {
 ...
 * @param nowMs         Current epoch ms; defaults to Date.now() (off-path only).
 */
export function getInflight(connectionId, nowMs = Date.now()) {
  if (!connectionId) return 0;
  const slot = _inflightMap.get(connectionId);
...
export function _clearInflightForTest() {
  _inflightMap.clear();
}
