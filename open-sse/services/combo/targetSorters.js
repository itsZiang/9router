...
import { secureRandomFloat, secureRandomInt } from "../../stubs/shared/utils/secureRandom";
import { getComboStepTarget, getComboStepWeight } from "../../stubs/lib/combos/steps";
import { getComboMetrics } from "../comboMetrics";
...
export function selectWeightedTarget(targets) {
...
export function orderTargetsForWeightedFallback(targets, selectedExecutionKey, preserveExistingOrder = false) {
...
export async function sortModelsByCost(models) {
  try {
    ...
    const withCost = await Promise.all(models.map(async modelStr => {
...
export async function sortTargetsByCost(targets) {
 ...
 */
export function sortModelsByUsage(models, comboName) {
...
export function sortTargetsByUsage(targets, comboName) {
  const orderedModels = sortModelsByUsage(targets.map(target => target.modelStr), comboName);
  const byModel = new Map();
  for (const target of targets) {
    const queue = byModel.get(target.modelStr) || [];
    queue.push(target);
...
export function orderTargetsByPowerOfTwoChoices(targets, comboName) {
