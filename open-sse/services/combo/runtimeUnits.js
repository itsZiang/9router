...
import { resolveDelayMs } from "./comboPredicates";
import { validateResponseQuality } from "./validateQuality";
function isRecord(value) {
  return !!value && typeof value === "object" && !Array.isArray(value);
}
function getCombosList(allCombos) {
  const combos = Array.isArray(allCombos) ? allCombos : allCombos?.combos || [];
  return combos.filter(combo => isRecord(combo) && typeof combo.name === "string");
}
...
export async function executeRuntimeUnitCombo(args) {
  ...
  for (const unit of orderedUnits) {
    for (let retry = 0; retry <= maxRetries; retry += 1) {
      ...
      args.log.info("COMBO", `Trying ${unit.kind} ${unitDisplayName(unit)}${retry > 0 ? ` (retry ${retry})` : ""}`);
      ...
      if (response.ok) {
        ...
        const quality = await validateResponseQuality(response, clientRequestedStream, args.log, args.config.responseValidation);
        if (quality.valid) {
          ...
          return {
            response: quality.clonedResponse ?? response,
            unit
          };
        }
      }
