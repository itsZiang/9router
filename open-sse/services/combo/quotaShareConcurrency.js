...
export function quotaShareConcurrencyKey(connectionId) {
 ...
 */
export async function acquireQuotaShareConcurrencySlot(target, cap, opts, log) {
  ...
  try {
    ...
    log.warn("COMBO", `Quota-share concurrency: connection ${connectionId} gate saturated (cap=${cap}) — proceeding without a slot`);
    return null;
  }
}