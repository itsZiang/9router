...
import { getProviderConnectionById } from "../../stubs/lib/db/providers";
import { effectiveMaxConcurrency } from "./comboPredicates";
/** Read a connection's positive `maxConcurrent`, or null when unset / <= 0 / on error. */
export async function lookupPositiveCap(connectionId) {
  try {
    const conn = await getProviderConnectionById(connectionId);
    const raw = conn?.maxConcurrent;
 ...
 */
export async function resolveMaxConcurrentByConnection(targets) {
  const caps = new Map();
 ...
 */
export function makeConnectionConcurrencyResolver(fallbackConcurrency) {
  const cache = new Map();
