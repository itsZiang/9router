 ...
 */
import { classifyErrorText, hasPerModelQuota, isProviderExhaustedReason } from "../accountFallback";
import { RateLimitReason } from "../../config/constants";
import { isProviderCircuitOpenResult } from "./comboPredicates";
 ...
 */
export function applyComboTargetExhaustion(target, opts) {
  const {
    result,
    fallbackResult,
    errorText,
...
function markConnectionLevelExhaustion(target, opts) {
  ...
  const provider = target.provider;
  if (!provider || provider === "unknown" || !CONNECTION_LEVEL_ERROR_STATUSES.includes(result.status) || isProviderCircuitOpenResult(result, errorText) ||
  ...
  if (connId) {
    ...
    log.info(tag, `Provider ${provider} connection ${connId} error (${result.status}) — marking for skip on remaining targets (#1731v2)`);
  } else {
    sets.exhaustedProviders.add(provider);
    log.info(tag, `Provider ${provider} connection error (${result.status}) — marking for skip on remaining targets (#1731)`);
  }
}