...
import { isProviderFailureCode } from "../accountFallback";
...
export const ALL_ACCOUNTS_RATE_LIMITED_PATTERNS = [/unavailable/i, /service temporarily unavailable/i];
export function isAllAccountsRateLimitedResponse(status, contentType, errorText) {
...
export function isProviderCircuitOpenResult(result, errorText) {
 ...
 */
export function getExhaustedTargetSkipReason(target, exhaustedProviders, exhaustedConnections) {
  const {
    provider,
    modelStr,
    connectionId
 ...
 */
export function clampComboDepth(value) {
 ...
 */
export function shouldSkipForPredictedTtft(metric, predictiveTtftMs) {
  if (!metric || !(predictiveTtftMs > 0)) return false;
  return (metric.requests ?? 0) >= PREDICTIVE_TTFT_MIN_SAMPLES && (metric.avgLatencyMs ?? 0) > predictiveTtftMs;
}
...
export function shouldRecordProviderBreakerFailure(args) {
  return !args.isStreamReadinessFailure && isProviderFailureCode(args.status) && !args.sameProviderNext && !args.skipProviderBreaker;
}
export function resolveDelayMs(value, fallback) {
 ...
 */
export function effectiveMaxConcurrency(cap, fallback) {
  if (typeof cap === "number" && Number.isFinite(cap) && cap > 0) {
    return Math.floor(cap);
  }
  return fallback;
}
export function comboModelNotFoundResponse(message) {
  return errorResponse(404, message);
}
export function getTargetProvider(modelStr, providerId) {
  const parsed = parseModel(modelStr);
  return providerId || parsed.provider || parsed.providerAlias || "unknown";
}
export function isStreamReadinessFailureErrorBody(errorBody) {
 ...
 */
export function isTokenLimitBreachErrorBody(errorBody) {
  if (!errorBody || typeof errorBody !== "object") return false;
  const error = errorBody.error;
  if (!error || typeof error !== "object") return false;
  return error.code === "TOKEN_LIMIT_EXCEEDED";
}
export function toRecordedTarget(target) {
  return {
