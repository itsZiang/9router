...
import { getAntigravityFetchAvailableModelsUrls, ANTIGRAVITY_BASE_URLS } from "../../config/antigravityUpstream";
import { isUserCallableAntigravityModelId, toClientAntigravityQuotaModelId } from "../../config/antigravityModelAliases";
import { isUserCallableAgyModelId } from "../../config/agyModels";
import { getDbInstance } from "../../stubs/lib/db/core";
import { applyAntigravityClientProfileHeaders, getAntigravityBootstrapHeaders, getAntigravityClientProfile } from "../antigravityClientProfile";
import { antigravityUserAgent, getAntigravityHeaders, getAntigravityLoadCodeAssistMetadata } from "../antigravityHeaders";
import { getAntigravityRemainingCredits, updateAntigravityRemainingCredits } from "../../executors/antigravity";
import { getCreditsMode } from "../antigravityCredits";
import { generateAntigravityRequestId, getAntigravitySessionId } from "../antigravityIdentity";
import { extractCodeAssistOnboardTierId, extractCodeAssistSubscriptionTier } from "../codeAssistSubscription";
import { toRecord, toNumber, getFieldValue } from "./scalars";
...
const _antigravityAvailableModelsCache = new Map();
const _antigravityAvailableModelsInflight = new Map();
const _antigravityUserQuotaCache = new Map();
const _antigravityUserQuotaInflight = new Map();
const _antigravityCreditProbeCache = new Map();
const _antigravityCreditProbeInflight = new Map();
...
const _antigravityCacheCleanupTimer = setInterval(() => {
  ...
  for (const [key, entry] of _antigravityUserQuotaCache) {
    if (now - entry.fetchedAt > ANTIGRAVITY_MODELS_CACHE_TTL_MS) _antigravityUserQuotaCache.delete(key);
  }
...
function getAntigravityLocalUsageUnits(provider, connectionId, modelId, resetAt) {
  ...
  try {
    ...
    const row = db.prepare(`SELECT COALESCE(SUM(
         ...
         WHERE provider = ?
           ...
           AND success = 1
           AND timestamp >= ?
           AND timestamp < ?`).get(provider, connectionId, modelId, windowStart, windowEnd);
...
function applyLocalUsageFallback(quota, provider, connectionId, modelId) {
...
function buildAntigravityUsageCacheKey(accessToken, projectId) {
  return `${accessToken.substring(0, 16)}:${projectId || "default"}`;
}
async function fetchAntigravityAvailableModelsCached(accessToken, projectId, options = {}) {
  ...
  const promise = (async () => {
    ...
    _antigravityAvailableModelsCache.set(cacheKey, {
      data,
      fetchedAt: Date.now()
    });
    return data;
...
async function fetchAntigravityUserQuotaCached(accessToken, projectId, options = {}) {
  ...
  const promise = (async () => {
    try {
      ...
      const data = await response.json();
      _antigravityUserQuotaCache.set(cacheKey, {
        data,
        fetchedAt: Date.now()
      });
      return data;
...
function extractCodeAssistTierId(subscription) {
...
export function mapCodeAssistTierIdToLabel(tierId) {
  const upper = tierId.toUpperCase();
  if (upper.includes("ULTRA")) return "Ultra";
  if (upper.includes("PRO") || upper.includes("PREMIUM") || upper.includes("GOOGLE_ONE") || upper.includes("ONE_AI")) return "Pro";
  ...
  if (upper.includes("FREE") || upper.includes("INDIVIDUAL") || upper.includes("LEGACY")) return "Free";
  return null;
}
export function mapSubscriptionTierStringToPlanLabel(tierText) {
  const upper = tierText.toUpperCase();
  if (upper.includes("ULTRA")) return "Ultra";
  if (upper.includes("PRO") || upper.includes("PREMIUM") || upper.includes("GOOGLE ONE")) return "Pro";
  if (upper.includes("ENTERPRISE")) return "Enterprise";
...
export function mapCodeAssistSubscriptionToPlanLabel(subscriptionInfo) {
  ...
  const tierName = String(getFieldValue(currentTier, "name", "displayName") || subscription.subscriptionType || subscription.tier || "");
...
const KNOWN_ANTIGRAVITY_PLAN_LABELS = new Set(["Ultra", "Pro", "Enterprise", "Business", "Plus", "Lite"]);

/**
 * Map raw loadCodeAssist tier data to short display labels (Antigravity Manager parity).
 */
export function getAntigravityPlanLabel(subscriptionInfo, fallbackInfo) {
 ...
 */
async function probeAntigravityCreditBalance(accessToken, accountId, projectId, options = {}, providerSpecificData = {}) {
  ...
  if (inflight) return inflight;
  const promise = probeAntigravityCreditBalanceUncached(accessToken, accountId, projectId, providerSpecificData).then(data => {
    _antigravityCreditProbeCache.set(cacheKey, {
      data,
      fetchedAt: Date.now()
    });
    return data;
...
async function probeAntigravityCreditBalanceUncached(accessToken, accountId, projectId, providerSpecificData = {}) {
  try {
    ...
    for (const baseUrl of ANTIGRAVITY_BASE_URLS) {
      ...
      const sessionId = getAntigravitySessionId({
      ...
      applyAntigravityClientProfileHeaders(headers, {
        connectionId: accountId,
        projectId,
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (Array.isArray(parsed?.remainingCredits)) {
              const googleCredit = parsed.remainingCredits.find(c => c?.creditType === "GOOGLE_ONE_AI");
              if (googleCredit) {
 ...
 */
export async function getAntigravityUsage(provider, accessToken, providerSpecificData, connectionProjectId, connectionId, options = {}) {
  if (!accessToken) {
    return {
      plan: "Free",
  ...
  try {
    subscriptionInfo = await getAntigravitySubscriptionInfoCached(accessToken, providerSpecificData, options);
    const savedProjectId = typeof providerSpecificData?.projectId === "string" && providerSpecificData.projectId.trim() ? providerSpecificData.projectId.trim() : null;
    const subscriptionProject = toRecord(subscriptionInfo).cloudaicompanionProject;
    const projectId = savedProjectId || connectionProjectId || (typeof subscriptionProject === "string" ? subscriptionProject : typeof toRecord(subscriptionProject).id === "string" ? toRecord(subscriptionProject).id : null);

    // Derive accountId for credit balance cache.
    // Must match executor key: credentials.connectionId
    const accountId = connectionId || "unknown";
    ...
    if ((options.forceRefresh || creditBalance === null) && creditsMode !== "off") {
      creditBalance = await probeAntigravityCreditBalance(accessToken, accountId, projectId, options, providerSpecificData || {});
    }
    const [data, userQuotaData] = await Promise.all([fetchAntigravityAvailableModelsCached(accessToken, projectId, options), fetchAntigravityUserQuotaCached(accessToken, projectId, options)]);
    const dataObj = toRecord(data);
    if (dataObj.__antigravityForbidden === true) {
      return {
        message: "Antigravity access forbidden. Check subscription."
      };
    }
    const modelEntries = toRecord(dataObj.models);
    const userQuotaEntries = new Map();
    ...
    for (const [rawModelKey, infoValue] of Object.entries(modelEntries)) {
      ...
      if (!modelKey || info.isInternal === true || !(provider === "agy" ? isUserCallableAgyModelId(modelKey) : isUserCallableAntigravityModelId(modelKey)) || Object.keys(quotaInfo).length === 0) {
        continue;
      }
      ...
      if (!fractionReported) {
        console.warn(`[Antigravity] model ${modelKey} returned no remainingFraction — quota unknown`);
      }
      ...
      }, provider, connectionId, modelKey);
    }
    ...
    for (const [modelKey, bucket] of userQuotaEntries) {
      if (quotas[modelKey] || !(provider === "agy" ? isUserCallableAgyModelId(modelKey) : isUserCallableAntigravityModelId(modelKey))) {
        continue;
      }
 ...
 */
async function getAntigravitySubscriptionInfoCached(accessToken, providerSpecificData, options = {}) {
  const profile = getAntigravityClientProfile({
    providerSpecificData
  });
  ...
  if (data != null) {
    _antigravitySubCache.set(cacheKey, {
      data,
      fetchedAt: Date.now()
    });
  }
 ...
 */
async function getAntigravitySubscriptionInfo(accessToken, providerSpecificData) {
  try {
    const profile = getAntigravityClientProfile({
      providerSpecificData
    });
    const response = await fetch(ANTIGRAVITY_CONFIG.loadProjectApiUrl, {
      method: "POST",
      headers: profile === "harness" ? getAntigravityBootstrapHeaders(profile, accessToken) : getAntigravityHeaders("loadCodeAssist", accessToken),
      body: JSON.stringify({
