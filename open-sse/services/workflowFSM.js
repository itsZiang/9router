...
export function classifyRisk(desc) {
  const l = desc.toLowerCase();
  if (HIGH_KEYWORDS.some(k => l.includes(k))) return "high";
  if (MED_KEYWORDS.some(k => l.includes(k))) return "medium";
  return "low";
}
const PHASE_ORDER = ["classify", "plan", "plan_review", "execute", "code_review", "quality_review", "security", "test", "output_review"];
const T = [{
  from: "classify",
  to: "plan",
...
export function createWorkflow(id, description, opts) {
...
export function advance(ctx, result) {
...
export function pause(ctx, reason) {
...
export function resume(ctx, phase) {
...
export function isTerminated(ctx) {
  return ctx.currentPhase === "done" || ctx.currentPhase === "failed";
}
export function getPhaseSequence(ctx) {
  return ctx.history.map(r => r.phase);
}
export function getLLMCallCount(ctx) {
