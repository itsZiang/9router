  ...
  async startLogin(providerId, options) {
    ...
    if (!config) {
      this.emit("status", {
        providerId,
        status: "error",
        message: "No extraction config found"
      });
      return {
        success: false,
      ...
      for (let i = 0; i < maxPolls; i++) {
        ...
        for (const source of tokenSources) {
          if (source.type === "cookie") {
            ...
            const matched = cookies.find(c => c.name === source.name && (!domain || c.domain.includes(domain.replace(/^\./, ""))));
        ...
        for (const source of tokenSources) {
          if (source.type === "localStorage" && !credentials[source.key]) {
            try {
              const value = await page.evaluate(key => localStorage.getItem(key), source.key);
          ...
          if (source.type === "sessionStorage" && !credentials[source.key]) {
            try {
              const value = await page.evaluate(key => sessionStorage.getItem(key), source.key);
              if (value && typeof value === "string") {
                credentials[source.key] = value;
              }
            } catch {
        ...
        const requiredKeys = tokenSources.map(s => s.type === "cookie" ? s.name : s.type === "localStorage" || s.type === "sessionStorage" ? s.key : s.name);
