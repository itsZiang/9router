...
import { getGitHubCopilotInternalUserHeaders } from "../config/providerHeaderProfiles";
import { fetchBailianQuota } from "./bailianQuotaFetcher";
import { fetchDeepseekQuota } from "./deepseekQuotaFetcher";
import { fetchOpencodeQuota } from "./opencodeQuotaFetcher";
import { getOllamaCloudUsage, getOpenCodeGoUsage } from "./opencodeOllamaUsage";
...
import { toRecord, toNumber, getFieldValue, clampPercentage, toDisplayLabel } from "./usage/scalars";
import { parseResetTime, createQuotaFromUsage } from "./usage/quota";
import { getMiniMaxUsage, getMiniMaxPlanLabel, getMiniMaxSessionTotal, inferMiniMaxPlanLabelFromTotals, getMiniMaxQuotaResetAt, isMiniMaxTextQuotaModel, getMiniMaxWeeklyTotal, createMiniMaxQuotaFromCount, createMiniMaxQuotaFromPercent, getMiniMaxRemainingPercent, getMiniMaxAuthErrorMessage, getMiniMaxErrorSummary } from "./usage/minimax";
import { getGlmUsage } from "./usage/glm";
// Re-exported para o teste glm-coding-plan-monthly (importa de services/usage).
export { glmMonthlyRemainingPercentage } from "./usage/glm";
import { getAntigravityUsage, getAntigravityPlanLabel, mapCodeAssistSubscriptionToPlanLabel, mapCodeAssistTierIdToLabel, mapSubscriptionTierStringToPlanLabel } from "./usage/antigravity";
import { getCursorUsage } from "./usage/cursor";
import { getKimiUsage } from "./usage/kimi";
import { getCodexUsage, consumeCodexRateLimitResetCredit } from "./usage/codex";
import { getClaudeUsage, getClaudePlanLabel } from "./usage/claude";
import { getKiroUsage } from "./usage/kiro";
// Re-exported para os testes kiro-* (importam de services/usage).
export { buildKiroUsageResult, discoverKiroProfileArn } from "./usage/kiro";
export { consumeCodexRateLimitResetCredit } from "./usage/codex";
...
function shouldDisplayGitHubQuota(quota) {
...
async function getCrofUsage(apiKey) {
  if (!apiKey) {
    return {
      message: "CrofAI API key not available. Add a key to view usage."
    };
  }
  let response;
  try {
    ...
    return {
      message: `CrofAI connected. Unable to fetch usage: ${error.message}`
    };
  }
  const rawText = await response.text();
  if (response.status === 401 || response.status === 403) {
    return {
      message: "CrofAI connected. The API key was rejected by /usage_api/."
    };
  }
  if (!response.ok) {
    return {
      message: `CrofAI connected. /usage_api/ returned HTTP ${response.status}.`
    };
  }
  let payload = {};
  if (rawText) {
    try {
      ...
      return {
        message: "CrofAI connected. Unable to parse /usage_api/ response."
      };
    }
  }
  const usableRequestsRaw = payload["usable_requests"];
  const usableRequests = usableRequestsRaw === null || usableRequestsRaw === undefined ? null : toNumber(usableRequestsRaw, 0);
  ...
  if (usableRequests !== null) {
    ...
    const todayResetMs = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), RESET_HOUR_UTC);
    const nextResetMs = todayResetMs > now.getTime() ? todayResetMs : todayResetMs + 24 * 60 * 60 * 1000;
 ...
 */
async function getBailianCodingPlanUsage(connectionId, apiKey, providerSpecificData) {
  try {
    const connection = {
      apiKey,
    ...
    if (!quota) {
      return {
        message: "Bailian Coding Plan connected. Unable to fetch quota."
      };
    }
    const bailianQuota = quota;
    ...
    return {
      message: `Bailian Coding Plan error: ${error.message}`
    };
  }
}
 ...
 */
async function getDeepseekUsage(connectionId, apiKey) {
  try {
    const connection = {
      apiKey
    };
 ...
 */
async function getXiaomiMimoUsage(connectionId) {
  if (!connectionId) {
    return {
      message: "Xiaomi MiMo: connection id unavailable for self-tracked quota."
    };
  }
  try {
    const {
      getMonthlyProviderTokensForConnection
    } = await import("../stubs/lib/usage/usageStats");
    ...
    return {
      message: `Xiaomi MiMo self-tracked usage error: ${error.message}`
    };
  }
}
 ...
 */
async function getOpencodeUsage(connectionId, apiKey) {
  if (!apiKey) {
    return {
      message: "OpenCode API key not available. Add a key to view usage."
    };
  }
  try {
    ...
    if (!quota) {
      return {
        message: "OpenCode connected. Unable to fetch quota data."
      };
    }
    const {
      window5h,
    ...
    return {
      message: `OpenCode error: ${sanitizeErrorMessage(error)}`
    };
  }
}
 ...
 */
async function getNanoGptUsage(apiKey) {
  if (!apiKey) {
    return {
      message: "NanoGPT API key not available. Add a key to view usage."
    };
  }
  try {
    ...
    if (!res.ok) {
      ...
      return {
        message: `NanoGPT quota API error (${res.status})`
      };
    }
    ...
    if (data.active) {
      ...
      if (Object.keys(quotas).length === 0) {
        return {
          plan,
          message: "NanoGPT connected, but no active limits found."
        };
      }
    }
    return {
    ...
    return {
      message: `NanoGPT connected. Unable to fetch usage: ${error.message}`
    };
  }
}
 ...
 */
export const USAGE_FETCHER_PROVIDERS = ["github", "antigravity", "agy", "claude", "codex", "cursor", "kiro", "amazon-q", "kimi-coding", "kimi-coding-apikey", "qwen", "qoder", "glm", "glm-cn", "zai", "glmt", "opencode-go", "minimax", "minimax-cn", "crof", "bailian-coding-plan", "nanogpt", "deepseek", "opencode", "opencode-zen", "xiaomi-mimo", "vertex", "vertex-partner", "codebuddy-cn"];
/**
...
export async function getUsageForProvider(connection, options = {}) {
  ...
  switch (provider) {
    ...
    case "antigravity":
    case "agy":
      return await getAntigravityUsage(provider, accessToken, providerSpecificData, projectId, id, options);
    case "claude":
...
function formatGitHubQuotaSnapshot(quota, resetAt = null) {
  ...
  const percentRemainingValue = toNumber(getFieldValue(source, "percent_remaining", "percentRemaining"), Number.NaN);
  let total = Number.isFinite(totalValue) ? Math.max(0, totalValue) : Number.isFinite(entitlement) ? Math.max(0, entitlement) : 0;
  ...
  let remainingPercentage = Number.isFinite(percentRemainingValue) ? clampPercentage(percentRemainingValue) : undefined;
...
function inferGitHubPlanName(data, premiumQuota) {
  ...
  const premiumTotal = premiumQuota?.total || toNumber(getFieldValue(monthlyQuotas, "premium_interactions", "premiumInteractions"), 0);
 ...
 */
async function getVertexUsage(connectionId, provider) {
  if (!connectionId) {
    return {
      message: "Vertex connected. Connection id unavailable for usage tracking."
    };
  }
  try {
    const {
    ...
    const {
      costUsd,
      requests
    } = await getConnectionSpendUsdSinceAdded(provider, connectionId);
    const spend = {
    ...
    return {
      plan: "Vertex AI",
      message: `$${costStr} used since this account was added \u00b7 ${requests} request${requests === 1 ? "" : "s"}`,
      quotas: {
        spend
      }
    };
    ...
    return {
      message: `Vertex usage tracking error: ${error.message}`
    };
  }
}
...
async function getQwenUsage(accessToken, providerSpecificData) {
  ...
  try {
    ...
    if (!resourceUrl) {
      return {
        message: "Qwen connected. No resource URL available."
      };
    }

    // Qwen may have usage endpoint at resource URL
    return {
      message: "Qwen connected. Usage tracked per request."
    };
  } catch (error) {
    return {
      message: "Unable to fetch Qwen usage."
    };
  }
}
...
async function getQoderUsage(accessToken) {
  ...
  try {
    ...
    return {
      message: "Qoder connected. Usage tracked per request."
    };
  } catch (error) {
    return {
      message: "Unable to fetch Qoder usage."
    };
  }
}
