...
import { resetWindowIfElapsed, getWindowUsage, incrementWindowTokens, getTokenLimitsForRequest, logTokenLimitReset } from "@/lib/localDb";
 ...
 */
export function seedWindowUsageFromHistory(limit, now = Date.now()) {
  const {
    periodStartAt
  } = resetWindowIfElapsed(limit, now);
  ...
  if (limit.scopeType === "model") {
    row = db.prepare(`SELECT ${tokenSum} FROM usage_history
         WHERE api_key_id = ? AND model = ? AND timestamp >= ?`).get(limit.apiKeyId, limit.scopeValue, lowerBound);
  } else if (limit.scopeType === "provider") {
    row = db.prepare(`SELECT ${tokenSum} FROM usage_history
         WHERE api_key_id = ? AND provider = ? AND timestamp >= ?`).get(limit.apiKeyId, limit.scopeValue, lowerBound);
  } else {
    row = db.prepare(`SELECT ${tokenSum} FROM usage_history
         WHERE api_key_id = ? AND timestamp >= ?`).get(limit.apiKeyId, lowerBound);
  }
 ...
 */
export function getCurrentWindowUsage(limit, now = Date.now(), forceFresh = false) {
  const {
    windowStart
  } = resetWindowIfElapsed(limit, now);
 ...
 */
export function addWindowTokens(limit, tokens, now = Date.now()) {
  const {
    windowStart
  } = resetWindowIfElapsed(limit, now);
...
export function syncCache(limitId, windowStart, tokensUsed) {
  cache.set(limitId, {
    windowStart,
    tokensUsed,
    syncedAt: Date.now()
...
export function invalidateLimit(limitId) {
 ...
 */
export function checkTokenLimits(apiKeyId, provider = "", model = "", now = Date.now()) {
 ...
 */
export function recordTokenUsage(apiKeyId, provider, model, tokens) {
  if (!apiKeyId) return;
  ...
  Promise.resolve().then(() => {
    ...
    const tx = db.transaction(() => {
      for (const limit of limits) {
        ...
        const currentRow = db.prepare("SELECT tokens_used FROM api_key_token_counters WHERE limit_id = ? AND window_start = ?").get(limit.id, windowStart);

        // First write to a new window? Detect rollover from the prior window.
        if (!currentRow) {
          const priorRow = db.prepare(`SELECT window_start, tokens_used FROM api_key_token_counters
                 WHERE limit_id = ? AND window_start < ?
                 ORDER BY window_start DESC LIMIT 1`).get(limit.id, windowStart);
