...
export function generateRoutingHints(targets, input) {
  ...
  const strategyModifier = determineStrategyModifier(specificityLevel, eligibleTargets.length, underqualifiedTargets.length);
  return {
    tierAssignments,
    specificity,
    specificityLevel,
...
export function getTargetTier(target) {
  return classifyTier(target.provider, target.modelStr);
}
export function estimateRequestCost(target, inputTokens, estimatedOutputTokens) {
...
export function compareByCostEffectiveness(a, b, hint) {
