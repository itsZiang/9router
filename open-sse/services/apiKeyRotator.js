...
const _connectionExtraKeys = new Map();
 ...
 */
export function getValidApiKey(connectionId, primaryKey, extraKeys = [], health) {
  const validExtras = extraKeys.filter(k => typeof k === "string" && k.trim().length > 0);
  ...
  if (primaryKey) {
    ...
    if (primaryHealth.status !== "invalid") {
      ...
      console.warn(`[KeyRotator] Skipping invalid primary key for connection ${connectionId.slice(0, 8)}`);
    }
  }
 ...
 * @returns Updated health status
 */
export function recordKeyTerminal(connectionId, keyId) {
  const health = getOrCreateHealth(connectionId, keyId);
 ...
 */
export function getInvalidKeyCount(health) {
  if (!health) return 0;
 ...
 */
export function getKeyHealthStats(connectionId, primaryKey, extraKeys = [], health) {
  ...
  for (const keyId of keys) {
    ...
    if (h.status === "active") active++;else if (h.status === "warning") warning++;else if (h.status === "invalid") invalid++;
  }
  return {
    total,
    active,
 ...
 */
export function resetKeyStatus(connectionId, keyId) {
  const health = getOrCreateHealth(connectionId, keyId);
  health.failures = 0;
  health.status = "active";
 ...
 */
export function getAllKeyHealth() {
  const result = {};
 ...
 */
export function syncHealthFromDB(connectionId, health) {
  if (!health) return;
 ...
 */
export function resetRotationIndex(connectionId) {
 ...
 */
export function getApiKeyCount(primaryKey, extraKeys = []) {
  const validExtras = extraKeys.filter(k => typeof k === "string" && k.trim().length > 0);
 ...
 */
export function resolveKeyForRequest(connectionId, primaryKey, extraKeys, selectedKeyId) {
  if (selectedKeyId) {
...
export function removeConnectionHealth(connectionId) {
...
export function removeConnectionIndex(connectionId) {
  _keyIndexes.delete(connectionId);
