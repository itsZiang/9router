import { isAccountDeactivated, isCreditsExhausted, isDailyQuotaExhausted, isOAuthInvalidToken } from "./accountFallback";
...
export function isEmptyContentResponse(responseBody) {
  ...
  if (Array.isArray(body.choices)) {
    ...
    // Cline gateway (muse-spark, OpenRouter-served) can also use
    // `reasoning_text`/`thinking`/`thought`. Honor them so a reasoning-only
    // completion is not misread as empty content.
    const reasoningTextAlias = message?.reasoning_text ?? delta?.reasoning_text ?? message?.thinking ?? delta?.thinking ?? message?.thought ?? delta?.thought;
    ...
    })) || (Array.isArray(delta?.reasoning_details) && delta.reasoning_details.some(block => {
      const b = block;
      return !!b && typeof b === "object" && (typeof b.text === "string" && b.text.length > 0 || typeof b.content === "string" && b.content.length > 0);
    }));
    ...
    const hasReasoning = (reasoningContent !== null && reasoningContent !== undefined && reasoningContent !== "")
      ...
      || (reasoningTextAlias !== null && reasoningTextAlias !== undefined && reasoningTextAlias !== "")
      || reasoningDetails;
...
export const CONTEXT_OVERFLOW_SIGNALS = ["context overflow", "prompt too large", "context window", "maximum context", "exceeds context", "input too long", "token limit", "too many tokens", "context length", "exceed.*context", "messages exceed"];
