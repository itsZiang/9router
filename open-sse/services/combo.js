...
import { checkFallbackError, classifyLockoutReason, decayModelFailureCount, formatRetryAfter, getModelLockoutInfo, getRuntimeProviderProfile, isModelLocked, recordModelLockoutFailure, recordProviderFailure, selectLockoutCooldownMs } from "./accountFallback";
import { errorResponse, unavailableResponse } from "../utils/error";
import { recordComboIntent, recordComboRequest, getComboMetrics } from "./comboMetrics";
import { resolveComboConfig, getDefaultComboConfig, resolveComboQueueDepth } from "./comboConfig";
import { maybeGenerateHandoff, maybeGenerateUniversalHandoff, injectUniversalHandoffBody, SKIP_UNIVERSAL_HANDOFF_FLAG } from "./contextHandoff";
import { recordSessionModelUsage, getLastSessionModel, getHandoff } from "../stubs/lib/db/contextHandoffs";
import { getHiddenModelsByProvider } from "../stubs/models";
import { resolveModelLockoutSettings } from "../stubs/lib/resilience/modelLockoutSettings";
import { fetchCodexQuota } from "./codexQuotaFetcher";
import { evaluateQuotaCutoff, getQuotaFetcher } from "./quotaPreflight";
import * as semaphore from "./rateLimitSemaphore";
import { getCircuitBreaker } from "../stubs/shared/utils/circuitBreaker";
import { fisherYatesShuffle, getNextFromDeck } from "../stubs/shared/utils/shuffleDeck";
import { parseModel } from "./model";
import { createComboContext } from "./combo/context";
import { phaseComboSetup } from "./combo/comboSetup";
import { checkCredentialGate, logCredentialSkip } from "./credentialGate";
import { emit } from "../stubs/lib/events/eventBus";
import { notifyWebhookEvent } from "../stubs/lib/webhookDispatcher";
import { classifyWithConfig } from "./intentClassifier";
import { selectProvider as selectAutoProvider } from "./autoCombo/engine";
import { extractTextContent } from "../translator/formats/gemini.js";
import { selectWithStrategy } from "./autoCombo/routerStrategy";
import { parseAutoPrefix } from "./autoCombo/autoPrefix";
import { handlePipelineCombo, buildPipelineResponse } from "./autoCombo/pipelineRouter";
import { DEFAULT_WEIGHTS } from "./autoCombo/scoring";
import { supportsToolCalling } from "./modelCapabilities";
import { estimateTokens } from "./contextManager";
import { getSessionConnection } from "./sessionManager";
import { applySessionStickiness, recordStickyBinding } from "./combo/sessionStickiness";
import { selectQuotaShareTarget } from "./combo/quotaShareStrategy";
import { resolveMaxConcurrentByConnection, makeConnectionConcurrencyResolver, lookupPositiveCap } from "./combo/concurrencyCaps";
import { acquireQuotaShareConcurrencySlot } from "./combo/quotaShareConcurrency";
import { orderTargetsByEvalScores } from "./evalRouting";
...
import { isProviderInCooldown, recordProviderCooldown, recordProviderSuccess } from "./providerCooldownTracker";
import { resolveResilienceSettings } from "../stubs/lib/resilience/settings";
import { resolveReasoningBufferedMaxTokens, toPositiveInteger } from "./reasoningTokenBuffer";
import { RESET_WINDOW_NAMES } from "./combo/types";
import { MAX_RR_COUNTERS, rrCounters, rrStickyTargets, weightedStickyTargets, clampStickyRoundRobinTargetLimit, clampStickyWeightedTargetLimit, getStickyRoundRobinStartIndex, recordStickyRoundRobinSuccess, getStickyWeightedExecutionKey, recordStickyWeightedSuccess } from "./combo/rrState";
import { validateResponseQuality, toRetryAfterDisplayValue } from "./combo/validateQuality";
import { resolveComboCooldownWaitDecision } from "./combo/comboCooldownRetry";
import { computeClosestRetryAfter, waitForCooldownAwareRetry } from "../stubs/sse/services/cooldownAwareRetry";
import { handleFusionChat } from "./fusion";
import { TRANSIENT_FOR_SEMAPHORE, MAX_FALLBACK_WAIT_MS, MAX_GLOBAL_ATTEMPTS, isAllAccountsRateLimitedResponse, clampComboDepth, shouldSkipForPredictedTtft, shouldRecordProviderBreakerFailure, resolveDelayMs, comboModelNotFoundResponse, isStreamReadinessFailureErrorBody, isTokenLimitBreachErrorBody, toRecordedTarget, getExhaustedTargetSkipReason } from "./combo/comboPredicates";
import { applyComboTargetExhaustion } from "./combo/targetExhaustion";
import { executeRuntimeUnitCombo } from "./combo/runtimeUnits";
import { dedupeTargetsByExecutionKey, isRecord } from "./combo/comboData";
import { expandProviderWildcardsInCombo, expandProviderWildcardsInCollection } from "./combo/providerWildcard";
import { resolveShadowTargets, scheduleShadowRouting } from "./combo/shadowRouting";
import { sortTargetsByCost, sortTargetsByUsage, orderTargetsByPowerOfTwoChoices } from "./combo/targetSorters";
import { filterTargetsByRequestCompatibility, getModelContextLimitForModelString, resolveComboRuntimeUnits, resolveComboTargets, resolveWeightedTargets, resolveWeightedStepGroups, sortTargetsByContextSize } from "./combo/comboStructure";
import { QUOTA_SOFT_DEPRIORITIZE_FACTOR, setCandidateQuotaSoftPenalty, _registerExecutionCandidates, _unregisterExecutionCandidates, extractPromptForIntent, mapIntentToTaskType, getIntentConfig, applyRequestTagRouting, scoreAutoTargets, expandAutoComboCandidatePool } from "./combo/autoStrategy";
import { resolveResetWindowConfig, resolveSlaRoutingPolicy, calculateResetWindowAffinity } from "./combo/quotaScoring";
import { fetchResetAwareQuotaWithCache, preScreenTargets, orderTargetsByResetAwareQuota, orderTargetsByResetWindow, orderTargetsByHeadroom } from "./combo/quotaStrategies";
import { classifyTask, getConversationCacheKey, isTaskRoutingStrategy, reorderByTaskWeight } from "./taskAwareRouting";
...
export { getComboFromData, getComboModelsFromData, resolveNestedComboModels, resolveNestedComboTargets, validateComboDAG } from "./combo/comboStructure";
...
function normalizeNestedComboMode(value) {
  return value === "execute" ? "execute" : "flatten";
}
function calculateTargetContextAffinity(target, sessionId) {
...
function quotaRemainingPercentFromQuota(quota) {
  ...
  if (windows && typeof windows === "object" && !Array.isArray(windows)) {
    ...
    for (const windowInfo of Object.values(windows)) {
      if (!windowInfo || typeof windowInfo !== "object") continue;
      const percentUsed = Number(windowInfo.percentUsed);
...
const QUOTA_BLOCKING_CONNECTION_STATUSES = new Set(["banned", "credits_exhausted", "deactivated", "expired", "rate_limited"]);
function normalizeConnectionStatus(value) {
  return typeof value === "string" ? value.trim().toLowerCase() : "";
}
function hasFutureRateLimitUntil(value) {
  if (value == null || value === "") return false;
  const time = new Date(String(value)).getTime();
  return Number.isFinite(time) && time > Date.now();
}
export function getConnectionStatusQuotaCutoffReason(connection) {
...
export async function buildAutoCandidates(targets, comboName, sessionId = null, resetWindowConfig = resolveResetWindowConfig(null), resilienceSettings = null) {
  ...
  const uniqueProviders = Array.from(new Set(targets.map(target => target.provider || parseModel(target.modelStr).provider || "unknown")));
  const connectionPoolCounts = new Map();
  ...
  for (const target of targets) {
    ...
    const connectionIds = providerConnections.map(c => c && typeof c === "object" && typeof c.id === "string" ? c.id : null).filter(id => id !== null);
    const allowedConnectionIds = Array.isArray(target.allowedConnectionIds) ? new Set(target.allowedConnectionIds.filter(connectionId => typeof connectionId === "string" && connectionId.trim().length > 0)) : null;
    const scopedConnectionIds = allowedConnectionIds ? connectionIds.filter(connectionId => allowedConnectionIds.has(connectionId)) : connectionIds;
  ...
  const candidates = await Promise.all(expandedTargets.map(async target => {
    ...
    const hasHistoricalSignal = Number.isFinite(historicalTotal) && historicalTotal >= MIN_HISTORY_SAMPLES;
    ...
    const p95LatencyMs = hasHistoricalSignal ? Number.isFinite(historicalP95Latency) && historicalP95Latency > 0 ? historicalP95Latency : getBootstrapLatencyMs(model) : Number.isFinite(avgLatency) && avgLatency > 0 ? avgLatency : getBootstrapLatencyMs(model);
    const errorRate = hasHistoricalSignal ? Number.isFinite(historicalSuccessRate) && historicalSuccessRate >= 0 && historicalSuccessRate <= 1 ? 1 - historicalSuccessRate : 0.05 : Number.isFinite(successRate) && successRate >= 0 && successRate <= 100 ? 1 - successRate / 100 : 0.05;
    const latencyStdDev = hasHistoricalSignal && Number.isFinite(historicalStdDev) && historicalStdDev > 0 ? Math.max(10, historicalStdDev) : Math.max(10, p95LatencyMs * 0.1);
    const breakerStateRaw = getCircuitBreaker(provider)?.getStatus?.()?.state;
    const circuitBreakerState = breakerStateRaw === "OPEN" || breakerStateRaw === "HALF_OPEN" ? breakerStateRaw : "CLOSED";
    ...
    if (fetcher && target.connectionId) {
      ...
      if (!quotaCutoffBlocked && quotaCutoffEnabled) {
        const cutoffDecision = evaluateQuotaCutoff(quota, buildAutoQuotaThresholds(provider, connection, resilienceSettings));
 ...
 */
export function pinIsDurablyUnhealthy(circuitState, connections, now, opts = {}) {
 ...
 */
async function isPinnedModelDurablyUnhealthy(pinnedModel) {
  try {
...
export function isContextOverflow400(errorText) {
  return /\bcontext.*(?:length_exceeded|too long|overflow|exceeded|window|limit)\b/i.test(errorText) || /exceeds.*context/i.test(errorText) || /your input exceeds/i.test(errorText);
}
/** @param {string} errorText */
export function isParamValidation400(errorText) {
  return /\bmax_tokens\b.*(?:illegal|must|range|invalid)/i.test(errorText) || /\bparameter is illegal\b/i.test(errorText) || /\bis illegal.*range\b/i.test(errorText);
}
...
export async function handleComboChat({
  ...
  const handleSingleModelWithTimeout = async (b, modelStr, target) => {
    if (comboTargetTimeoutMs <= 0) {
      return handleSingleModel(b, modelStr, target).catch(err => errorResponse(502, err?.message ?? "Upstream model error"));
    }
    ...
    const timeoutPromise = new Promise(resolve => {
      timeoutId = setTimeout(() => {
        timedOut = true;
        log.warn("COMBO", `Model ${modelStr} exceeded ${comboTargetTimeoutMs}ms timeout — falling back`);
    ...
    if (parentHedgeSignal) {
      if (parentHedgeSignal.aborted) {
        ...
        parentHedgeSignal.addEventListener("abort", onParentHedgeAbort, {
          once: true
        });
      }
    }
    try {
      return await Promise.race([handleSingleModel(b, modelStr, targetWithSignal).catch(err => {
        if (timedOut) {
  ...
  if (pinnedModel) {
    ...
    const pinInCombo = !haveFullCombos || resolveComboTargets(combo, allCombos, clampComboDepth(config.maxComboDepth)).some(t => t.modelStr === pinnedModel);
    ...
    if (pinInCombo && !pinDurablyDown) {
      log.info("COMBO", `Bypassing strategy — routing directly to pinned context model: ${pinnedModel}`);
      return handleSingleModelWithTimeout(body, pinnedModel);
    }
    log.warn("COMBO", pinInCombo ? `Context-cache pin "${pinnedModel}" provider durably unhealthy — dropping pin, using strategy` : `Stale context-cache pin "${pinnedModel}" not in combo "${combo.name}" targets — dropping pin, using strategy`);
    return handleSingleModelWithTimeout(body, pinnedModel);
  }
  ...
  if (strategy === "fusion") {
    const fusionModels = (combo.models || []).map(m => {
    ...
    const tuning = cfg.fusionTuning && typeof cfg.fusionTuning === "object" ? cfg.fusionTuning : undefined;
  ...
  const executeModeUnits = nestedComboMode === "execute" && allCombos ? resolveComboRuntimeUnits(combo, allCombos, "execute", nestingContext.maxDepth) : [];
  const hasExecutableComboRef = executeModeUnits.some(unit => unit.kind === "combo-ref");
  const simpleExecuteStrategies = new Set(["priority", "round-robin", "random", "strict-random", "weighted", "fill-first"]);
  if (hasExecutableComboRef && simpleExecuteStrategies.has(strategy)) {
    ...
    if (strategy === "weighted") {
      ...
      const stickyUnit = stickyKey ? runtimeUnits.find(unit => unit.executionKey === stickyKey) : null;
      if (stickyUnit) {
        runtimeUnits = [stickyUnit, ...runtimeUnits.filter(unit => unit.executionKey !== stickyUnit.executionKey)];
        unitExecutionStrategy = "priority";
      }
    }
    if (strategy === "random") runtimeUnits = fisherYatesShuffle([...runtimeUnits]);
    if (strategy === "strict-random") {
      const key = await getNextFromDeck(`combo:${combo.name}`, runtimeUnits.map(unit => unit.executionKey));
      const selected = runtimeUnits.find(unit => unit.executionKey === key) || runtimeUnits[0];
      runtimeUnits = [selected, ...runtimeUnits.filter(unit => unit.executionKey !== selected.executionKey)];
    }
    let runtimeStickyLimit = null;
    let runtimeStickyTargets = runtimeUnits;
    if (strategy === "round-robin") {
      const perComboStickyLimit = config.stickyRoundRobinLimit;
      runtimeStickyLimit = clampStickyRoundRobinTargetLimit(perComboStickyLimit !== undefined && perComboStickyLimit !== null ? perComboStickyLimit : settings?.stickyRoundRobinLimit);
      const {
        startIndex,
        counter
      } = getStickyRoundRobinStartIndex(combo.name, runtimeUnits, runtimeStickyLimit);
      if (runtimeStickyLimit <= 1) rrCounters.set(combo.name, counter + 1);
      runtimeUnits = runtimeUnits.map((_, offset) => runtimeUnits[(startIndex + offset) % runtimeUnits.length]);
    ...
    if (strategy === "weighted" && execution.response.ok && execution.unit) {
      const stickyLimit = clampStickyWeightedTargetLimit(config.stickyWeightedLimit);
      if (stickyLimit > 1) recordStickyWeightedSuccess(combo.name, execution.unit.executionKey, stickyLimit);
    }
    if (strategy === "round-robin" && execution.response.ok && execution.unit && runtimeStickyLimit && runtimeStickyLimit > 1) {
      recordStickyRoundRobinSuccess(combo.name, execution.unit, runtimeStickyLimit, runtimeStickyTargets);
    }
  ...
  const isTargetSelectableForWeighted = async target => {
    ...
    if (resilienceSettings.providerCooldown.enabled && Boolean(target.provider && target.provider !== "unknown") && isProviderInCooldown(target.provider, target.connectionId ?? undefined, resilienceSettings)) {
      return false;
    }
    if (target.provider && rawModel && isModelLocked(target.provider, target.connectionId || "", rawModel)) {
      return false;
    }
  ...
  const expandedAllCombos = allCombos ? Array.isArray(allCombos) ? await expandProviderWildcardsInCollection(allCombos) : {
    ...allCombos,
    combos: await expandProviderWildcardsInCollection(allCombos.combos || [])
  } : allCombos;
  const stickyWeightedLimit = clampStickyWeightedTargetLimit(config.stickyWeightedLimit);
  if (strategy === "weighted" && !weightedStickyTargets.has(combo.name) && weightedStickyTargets.size >= MAX_RR_COUNTERS) {
    const oldest = weightedStickyTargets.keys().next().value;
    if (oldest !== undefined) weightedStickyTargets.delete(oldest);
  }
  let stepGroups;
  const weightedEligibleKeys = new Set();
  if (strategy === "weighted") {
  ...
  const rawStickyWeightedKey = strategy === "weighted" ? getStickyWeightedExecutionKey(combo.name, stickyWeightedLimit) : null;
  const stickyWeightedKey = rawStickyWeightedKey && weightedEligibleKeys.has(rawStickyWeightedKey) ? rawStickyWeightedKey : null;
  ...
  const weightedResolution = strategy === "weighted" ? resolveWeightedTargets(expandedCombo, expandedAllCombos, stickyWeightedKey, weightedEligibleKeys, stepGroups) : null;
  const getWeightedStepKeyForTarget = target => {
    if (!weightedResolution?.orderedSteps) return null;
    const step = weightedResolution.orderedSteps.find(entry => target.executionKey === entry.executionKey || target.executionKey.startsWith(entry.executionKey + ">"));
    return step?.executionKey || null;
  };
  let orderedTargets = strategy === "weighted" ? weightedResolution?.orderedTargets || [] : resolveComboTargets(expandedCombo, expandedAllCombos, clampComboDepth(config.maxComboDepth));
  orderedTargets = await applyRequestTagRouting(orderedTargets, body, log);
  if (strategy === "weighted") {
    log.info("COMBO", `Weighted selection${stickyWeightedKey ? " (sticky)" : ""}${allCombos ? " with nested resolution" : ""}: ${orderedTargets.length} total targets`);
  } else if (allCombos) {
    ...
    if (autoVariant === "smart" || config.pipeline_enabled) {
      try {
        ...
        if (pipelineMsg === "PIPELINE_DISABLED") {
          ...
          log.info("COMBO", "Pipeline skipped (prompt below token threshold), falling through to standard auto routing");
        } else {
  ...
  if (strategy === "auto") {
    ...
    if (requestHasTools) {
      ...
      if (filtered.length > 0) {
        ...
        log.warn("COMBO", "Auto strategy: all candidates filtered by tool-calling policy, falling back to full pool");
      }
    }
    ...
    const estimatedInputTokens = estimateTokens(typeof requestMessages === "string" || requestMessages !== null && typeof requestMessages === "object" ? requestMessages : []);
    if (estimatedInputTokens > 0) {
      ...
      if (filteredByContext.length > 0) {
        log.debug?.("COMBO", `Auto strategy: context-window filter kept ${filteredByContext.length}/${eligibleTargets.length} candidates (est. ${estimatedInputTokens} tokens)`);
        eligibleTargets = filteredByContext;
      } else {
        log.warn("COMBO", `Auto strategy: all candidates filtered by context-window policy (est. ${estimatedInputTokens} tokens), falling back to full pool`);
    ...
    const rawAutoConfigSource = combo?.autoConfig || (isRecord(combo?.config?.auto) ? combo.config.auto : null) || combo?.config || {};
    const autoConfigSource = isRecord(rawAutoConfigSource) ? rawAutoConfigSource : {};
    const routingStrategy = typeof autoConfigSource.routerStrategy === "string" ? autoConfigSource.routerStrategy : typeof autoConfigSource.routingStrategy === "string" ? autoConfigSource.routingStrategy : typeof autoConfigSource.strategyName === "string" ? autoConfigSource.strategyName : "rules";
    const candidatePool = Array.isArray(autoConfigSource.candidatePool) ? autoConfigSource.candidatePool : [...new Set(eligibleTargets.map(target => target.provider))];
    const weights = autoConfigSource.weights && typeof autoConfigSource.weights === "object" ? autoConfigSource.weights : DEFAULT_WEIGHTS;
    const explorationRate = Number.isFinite(Number(autoConfigSource.explorationRate)) ? Number(autoConfigSource.explorationRate) : 0.05;
    const budgetCap = Number.isFinite(Number(autoConfigSource.budgetCap)) ? Number(autoConfigSource.budgetCap) : undefined;
    const modePack = typeof autoConfigSource.modePack === "string" ? autoConfigSource.modePack : undefined;
    ...
    try {
      ...
      log.warn("COMBO", "Failed to retrieve Last Known Good Provider. This is non-fatal.", {
        err
      });
    }
    ...
    const candidates = await buildAutoCandidates(eligibleTargets, combo.name, relayOptions?.sessionId, resetWindowConfig, autoCandidateResilienceSettings);
    const routableCandidates = candidates.filter(candidate => candidate.quotaCutoffBlocked !== true);
    const quotaBlockedCount = candidates.length - routableCandidates.length;
    if (quotaBlockedCount > 0) {
      log.info("COMBO", `Auto strategy: quota cutoff skipped ${quotaBlockedCount}/${candidates.length} account candidates`);
    }
    ...
    if (candidates.length > 0 && routableCandidates.length === 0) {
      return unavailableResponse(429, "All auto strategy candidates are below configured quota cutoffs");
    }
    if (routableCandidates.length > 0) {
      ...
      if (routingStrategy !== "rules") {
        try {
          const decision = selectWithStrategy(routableCandidates, {
            taskType,
            requestHasTools,
          ...
          log.warn("COMBO", `Auto strategy '${routingStrategy}' failed (${err?.message || "unknown"}), falling back to rules`);
        }
      }
      ...
      const autoManifestHint = config.complexityAwareRouting === true ? buildComplexityRoutingHint(eligibleTargets.filter(t => t.kind === "model"), body, log) : null;
      const scoredTargets = scoreAutoTargets(eligibleTargets, routableCandidates, taskType, weights, autoManifestHint);
      ...
      if (!selectedTarget) {
        return unavailableResponse(429, "No auto strategy targets remained after quota cutoff filtering");
      }
      ...
      orderedTargets = dedupeTargetsByExecutionKey([selectedTarget, ...rankedTargets, ...eligibleTargets].filter(entry => entry !== undefined && entry !== null));
      log.info("COMBO", `Auto selection: ${selectedTarget?.modelStr || `${selectedProvider}/${selectedModel}`} | intent=${intent} task=${taskType} | strategy=${routingStrategy} | ${selectionReason}`);
    } else {
    ...
    try {
      ...
      if (lkgpProvider) {
        ...
        let lkgpIndex = -1;
        if (connId) {
          lkgpIndex = orderedTargets.findIndex(target => target.provider === providerName && target.connectionId === connId);
        }
        if (lkgpIndex < 0) {
          lkgpIndex = orderedTargets.findIndex(target => target.provider === providerName ||
        ...
        if (lkgpIndex > 0) {
          ...
          log.info("COMBO", `[LKGP] Prioritizing last known good provider ${providerName}${connId ? ` (account ${connId})` : ""} for combo "${combo.name}"`);
        } else if (lkgpIndex === 0) {
          log.debug?.("COMBO", `[LKGP] Last known good provider ${providerName}${connId ? ` (account ${connId})` : ""} already first for combo "${combo.name}"`);
        }
      }
    } catch (err) {
      log.warn("COMBO", "Failed to retrieve Last Known Good Provider. This is non-fatal.", {
    ...
    const selectedExecutionKey = await getNextFromDeck(`combo:${combo.name}`, orderedTargets.map(target => target.executionKey));
    const selectedTarget = orderedTargets.find(target => target.executionKey === selectedExecutionKey) || null;
    ...
    const rest = fisherYatesShuffle(orderedTargets.filter(target => target.executionKey !== selectedExecutionKey));
    orderedTargets = [selectedTarget, ...rest].filter(target => target !== null);
    log.info("COMBO", `Strict-random deck: ${selectedExecutionKey} selected (${orderedTargets.length} targets)`);
    ...
    if (config.manifestRouting === true) {
      try {
        const manifestHint = generateRoutingHints(orderedTargets.filter(t => t.kind === "model"), {
          messages: Array.isArray(body?.messages) ? body.messages : [],
          tools: Array.isArray(body?.tools) ? body.tools : undefined,
          model: typeof body?.model === "string" ? body.model : undefined
        });
        if (manifestHint.strategyModifier === "require-premium") {
          const eligible = orderedTargets.filter(t => t.kind !== "model" || manifestHint.eligibleTargets.some(e => e.provider === t.provider && e.modelStr === t.modelStr));
    ...
    orderedTargets = await orderTargetsByResetAwareQuota(orderedTargets, combo.name, config, log, apiKeyAllowedConnections);
    log.info("COMBO", `Reset-aware ordering: ${orderedTargets[0]?.modelStr}${orderedTargets[0]?.connectionId ? ` (${orderedTargets[0].connectionId})` : ""} first`);
    ...
    log.info("COMBO", `Reset-window ordering: ${orderedTargets[0]?.modelStr}${orderedTargets[0]?.connectionId ? ` (${orderedTargets[0].connectionId})` : ""} first`);
    ...
    orderedTargets = await orderTargetsByHeadroom(orderedTargets, combo.name, log, apiKeyAllowedConnections);
    log.info("COMBO", `Headroom ordering: ${orderedTargets[0]?.modelStr}${orderedTargets[0]?.connectionId ? ` (${orderedTargets[0].connectionId})` : ""} has most free capacity`);
    ...
    log.info("COMBO", `Quota-share ordering: ${orderedTargets[0]?.modelStr}${orderedTargets[0]?.connectionId ? ` (${orderedTargets[0].connectionId})` : ""} selected (DRR+P2C)`);
  }
  const _sticky = await applySessionStickiness(orderedTargets, body.messages);
  ...
  if (isTaskRoutingStrategy(strategy)) {
    ...
    const nextOrder = pinnedFirst ? [pinnedFirst, ...taskReordered.filter(t => t !== pinnedFirst)] : taskReordered;
    if (nextOrder[0]?.modelStr !== orderedTargets[0]?.modelStr) {
      ...
      log.info("COMBO", `task-route task=${task.level}${reasons} cacheKey=${conversationCacheKey ?? "none"} → ${nextOrder[0]?.modelStr}`);
    }
  ...
  const preScreenMap = strategy === "priority" ? await preScreenTargets(orderedTargets, isModelAvailable).catch(() => new Map()) : new Map();
  if (orderedTargets.length === 0) {
    return comboModelNotFoundResponse("Combo has no executable targets");
  }
  scheduleShadowRouting(combo, config, body, resolveShadowTargets(combo, config, allCombos), handleSingleModel, isModelAvailable, strategy, log);
  ...
  // semantics exactly.
  const comboCooldownWaitEnabled = strategy === "quota-share" && resilienceSettings.comboCooldownWait.enabled;
  ...
  const quotaShareConcurrencyEnabled = strategy === "quota-share" && resilienceSettings.quotaShareConcurrencyLimit.enabled;
  const dispatchWithCooldownRetry = async () => {
    for (let setTry = 0; setTry <= maxSetRetries; setTry++) {
      ...
      const exhaustedConnections = new Set();
      ...
      const executeTarget = async i => {
        ...
        if (resilienceSettings.providerCooldown.enabled && Boolean(provider && provider !== "unknown") && isProviderInCooldown(provider, target.connectionId ?? undefined, resilienceSettings)) {
        ...
        const allowRateLimitedConnection = Boolean(provider && provider !== "unknown") && transientRateLimitedProviders.has(provider);
        ...
        const exhaustedSkip = getExhaustedTargetSkipReason(target, exhaustedProviders, exhaustedConnections);
        if (exhaustedSkip) {
        ...
        for (let retry = 0; retry <= maxRetries; retry++) {
          ...
          if (signal?.aborted) {
            ...
            return {
              ok: false,
              response: errorResponse(499, "Client disconnected")
            };
          }
          globalAttempts++;
          if (globalAttempts > MAX_GLOBAL_ATTEMPTS) {
            log.warn("COMBO", `Maximum combo attempts (${MAX_GLOBAL_ATTEMPTS}) exceeded across all targets and fallbacks. Terminating loop to prevent runaway background requests.`);
            return {
              ok: false,
              response: errorResponse(503, "Maximum combo retry limit reached")
            };
          }

          // Predictive TTFT Circuit Breaker (skip slow models)
          if (zeroLatencyOptimizationsEnabled && config.predictiveTtftMs && config.predictiveTtftMs > 0 && retry === 0) {
          ...
          log.info("COMBO", `Trying model ${i + 1}/${orderedTargets.length}: ${modelStr}${retry > 0 ? ` (retry ${retry})` : ""}`);
          ...
          if (zeroLatencyOptimizationsEnabled && i > 0 && config.fallbackCompressionMode && config.fallbackCompressionMode !== "off") {
            const {
              estimateTokens
            ...
            if (estimatedTokens > (config.fallbackCompressionThreshold ?? 1000)) {
              const {
                applyCompression
              } = await import("./compression/strategySelector");
              const compressionResult = applyCompression(attemptBody, config.fallbackCompressionMode,
          ...
          if (universalHandoffConfig.enabled && relayOptions?.sessionId && !body?.[SKIP_UNIVERSAL_HANDOFF_FLAG]) {
            ...
            if (lastModel && lastModel !== modelStr) {
              ...
              attemptBody = injectUniversalHandoffBody(attemptBody,
              // Use the cloned body to maintain isolation
              lastModel, modelStr, `Model routing: ${lastModel} → ${modelStr}`, existingHandoff);
            }
          }
          ...
          // model's known output cap; otherwise preserve the client's explicit limit.
          {
            ...
            if (currentMaxTokens !== null && bufferedMaxTokens !== null) {
              ...
              if (bufferedMaxTokens !== currentMaxTokens) {
                log.info("COMBO", `Reasoning model ${modelStr}: adjusted max_tokens ${currentMaxTokens} -> ${bufferedMaxTokens}`);
              }
            }
          }
          ...
          if (result.ok) {
            const selectedConnectionId = result.headers?.get("X-OmniRoute-Selected-Connection-Id") || result.headers?.get("x-omniroute-selected-connection-id") || undefined;
            const effectiveConnectionId = selectedConnectionId || target.connectionId || "";
            const quality = await validateResponseQuality(result, clientRequestedStream, log, config.responseValidation);
            if (!quality.valid) {
              ...
              if (provider && rawModel) {
                ...
                if (mlSettings.enabled && mlSettings.errorCodes.includes(502)) {
                  recordModelLockoutFailure(provider, target.connectionId || "", rawModel, "quality_failure", 502, mlSettings.baseCooldownMs, profile, {
            ...
            notifyWebhookEvent("request.completed", {
              ...
              provider,
              model: modelStr,
              account: typeof target.label === "string" && target.label.trim().length > 0 ? target.label.trim() : "",
            ...
            if (combo.context_cache_protection && effectiveSessionId && !body?.[SKIP_UNIVERSAL_HANDOFF_FLAG]) {
              recordSessionModelUsage(effectiveSessionId, combo.name, modelStr, provider, target.connectionId ?? undefined);
            }

            // Universal handoff: record model usage for session
            if (universalHandoffConfig.enabled && relayOptions?.sessionId && !body?.[SKIP_UNIVERSAL_HANDOFF_FLAG]) {
              const prevModel = getLastSessionModel(relayOptions.sessionId, combo.name);
              recordSessionModelUsage(relayOptions.sessionId, combo.name, modelStr, provider, target.connectionId ?? undefined);
              if (prevModel && prevModel !== modelStr) {
                const handoffSourceMessages = Array.isArray(body?.messages) && body.messages.length > 0 ? body.messages : Array.isArray(body?.input) ? body.input : [];
            ...
            if (strategy === "context-relay" && relayOptions?.sessionId && relayConfig && relayConfig.handoffProviders.includes(provider) && provider === "codex") {
              ...
              if (connectionId) {
                ...
                if (quotaInfo) {
                  const resetCandidates = [quotaInfo.windows?.session?.resetAt, quotaInfo.windows?.weekly?.resetAt, quotaInfo.resetAt].filter(value => typeof value === "string" && value.length > 0).sort((a, b) => a.localeCompare(b));
                  const handoffSourceMessages = Array.isArray(body?.messages) && body.messages.length > 0 ? body.messages : Array.isArray(body?.input) ? body.input : [];
            ...
            if (provider) {
              ...
              void (async () => {
                try {
                  ...
                  await Promise.all([setLKGP(combo.name, target.executionKey, provider, connId), setLKGP(combo.name, combo.id || combo.name, provider, connId)]);
                } catch (err) {
            ...
            return {
              ok: true,
              response: quality.clonedResponse ?? result
            };
          }
          ...
          try {
            ...
            try {
              ...
              if (text) {
                ...
                errorText = typeof parsedError === "object" && parsedError?.message || (typeof parsedError === "string" ? parsedError : null) || errorBody?.message || errorText;
          ...
          const isStreamReadinessFailure = (result.status === 502 || result.status === 504) && isStreamReadinessFailureErrorBody(errorBody);
          ...
          const structuredError = rawError && typeof rawError === "object" ? {
          ...
          const fallbackResult = checkFallbackError(result.status, errorText, 0, null, provider, result.headers, profile, structuredError);
          const {
            cooldownMs
          } = fallbackResult;
          const selectedConnectionId = result.headers?.get("X-OmniRoute-Selected-Connection-Id") || result.headers?.get("x-omniroute-selected-connection-id") || undefined;
          ...
          if (result.status === 400 && fallbackResult.shouldFallback && !isContextOverflow400(errorText) && !isParamValidation400(errorText) && (errorText.toLowerCase().includes("context") || errorText.toLowerCase().includes("prompt") || errorText.toLowerCase().includes("token") || errorText.toLowerCase().includes("malformed") || errorText.toLowerCase().includes("invalid") || errorText.toLowerCase().includes("bad request"))) {
            log.warn("COMBO", `400 Bad Request with body-specific error detected on ${modelStr} — skipping fallback to other targets to prevent infinite loop`);
          ...
          const sameProviderNext = typeof nextTarget?.provider === "string" && nextTarget.provider === provider;
          if (shouldRecordProviderBreakerFailure({
          ...
          const isTransient = !isStreamReadinessFailure && !isTokenLimitBreach && [408, 429, 500, 502, 503, 504].includes(result.status);
          if (retry < maxRetries && isTransient && !providerExhausted) {
            ...
            if (provider && rawModel && retry === 0) {
              ...
              if (mlSettings.enabled && mlSettings.errorCodes.includes(result.status)) {
                recordModelLockoutFailure(provider, targetWithConnection.connectionId || "", rawModel, classifyLockoutReason(result.status), result.status, mlSettings.baseCooldownMs, profile, {
          ...
          if (resilienceSettings.providerCooldown.enabled && provider && provider !== "unknown") {
            recordProviderCooldown(provider, targetWithConnection.connectionId ?? undefined, resilienceSettings);
          }
          const fallbackWaitMs = fallbackDelayMs > 0 && cooldownMs > 0 && cooldownMs <= MAX_FALLBACK_WAIT_MS ? Math.min(cooldownMs, fallbackDelayMs) : 0;
      ...
      if (earliestRetryAfter) {
        ...
        if (comboCooldownWaitEnabled && status === 429) {
          ...
          if (decision.wait) {
            log.info("COMBO", `Quota-share cooldown wait: ${msg} — waiting ${Math.ceil(decision.waitMs / 1000)}s (reason=${decision.reason ?? "?"}) then retrying (attempt ${comboCooldownAttempt + 1}/${resilienceSettings.comboCooldownWait.maxAttempts})`);
  ...
  let quotaShareConcurrencyRelease = null;
  ...
  if (quotaShareConcurrencyEnabled && qsConnectionId) {
    ...
    quotaShareConcurrencyRelease = await acquireQuotaShareConcurrencySlot(orderedTargets[0], qsCap, {
...
async function handleRoundRobinCombo({
  ...
  const config = settings ? resolveComboConfig(combo, settings) : {
  ...
  const rrExpandedAllCombos = allCombos ? Array.isArray(allCombos) ? await expandProviderWildcardsInCollection(allCombos) : {
    ...allCombos,
    combos: await expandProviderWildcardsInCollection(allCombos.combos || [])
  } : allCombos;
  const orderedTargets = resolveComboTargets(rrExpandedCombo, rrExpandedAllCombos, clampComboDepth(config.maxComboDepth));
  const tagFilteredTargets = await applyRequestTagRouting(orderedTargets, body, log);
  const evalRankedTargets = orderTargetsByEvalScores(tagFilteredTargets, config.evalRouting, log);
  const filteredTargets = filterTargetsByRequestCompatibility(evalRankedTargets, body, log, "Context-aware round-robin fallback");
  const modelCount = filteredTargets.length;
  if (modelCount === 0) {
    return comboModelNotFoundResponse("Round-robin combo has no executable targets");
  }
  scheduleShadowRouting(combo, config, body, resolveShadowTargets(combo, config, allCombos), handleSingleModel, isModelAvailable, "round-robin", log);
  ...
  const perComboStickyLimit = config.stickyRoundRobinLimit;
  const stickyLimit = clampStickyRoundRobinTargetLimit(perComboStickyLimit !== undefined && perComboStickyLimit !== null ? perComboStickyLimit : settings?.stickyRoundRobinLimit);
  ...
  if (stickyRoundRobinEnabled) {
    ...
    if (sticky) {
      const stickyTarget = filteredTargets.find(target => target.executionKey === sticky.executionKey);
      if (stickyTarget) {
        ...
        if (!stickyAvailable) {
          log.info("COMBO-RR", `Clearing stale sticky target ${stickyTarget.modelStr} — unavailable`);
          rrStickyTargets.delete(combo.name);
        }
      }
    }
  }
  if (!rrCounters.has(combo.name) && !rrStickyTargets.has(combo.name) && rrCounters.size >= MAX_RR_COUNTERS) {
  ...
  const {
    startIndex,
    counter
  } = getStickyRoundRobinStartIndex(combo.name, filteredTargets, stickyLimit);
  if (!stickyRoundRobinEnabled) {
  ...
  const _rrSessionSticky = await applySessionStickiness(filteredTargets, body?.messages);
  let rrStartIndex = startIndex;
  if (_rrSessionSticky.stuck) {
    const stickyIdx = filteredTargets.findIndex(t => t.connectionId === _rrSessionSticky.targets[0]?.connectionId);
  ...
  for (let offset = 0; offset < modelCount; offset++) {
    ...
    if (resilienceSettings.providerCooldown.enabled && Boolean(provider && provider !== "unknown") && isProviderInCooldown(provider, target.connectionId, resilienceSettings)) {
    ...
    try {
      ...
      if (errCode === "SEMAPHORE_TIMEOUT" || errCode === "SEMAPHORE_QUEUE_FULL") {
        log.warn("COMBO-RR", `Semaphore ${errCode === "SEMAPHORE_QUEUE_FULL" ? "queue full" : "timeout"} for ${modelStr}, trying next model`);
        if (offset > 0) fallbackCount++;
        continue;
      }
      throw err;
    ...
    try {
      for (let retry = 0; retry <= maxRetries; retry++) {
        globalAttempts++;
        if (globalAttempts > MAX_GLOBAL_ATTEMPTS) {
          log.warn("COMBO-RR", `Maximum combo attempts (${MAX_GLOBAL_ATTEMPTS}) exceeded. Terminating loop to prevent runaway requests.`);
        ...
        let attemptBody = body;
        {
          ...
          if (currentMaxTokens !== null && bufferedMaxTokens !== null && bufferedMaxTokens !== currentMaxTokens) {
            attemptBody = {
              ...bodyRecord,
              max_tokens: bufferedMaxTokens
            };
            log.info("COMBO-RR", `Reasoning model ${modelStr}: adjusted max_tokens ${currentMaxTokens} -> ${bufferedMaxTokens}`);
          }
        }
        ...
        if (result.ok) {
          ...
          if (provider && rawModel) {
            ...
            if (dcResult.cleared) {
              ...
              log.debug?.("COMBO-RR", `Model ${modelStr} decayed to failureCount=${dcResult.newFailureCount}`);
            }
          }
          if (provider && provider !== "unknown") {
            recordProviderSuccess(provider, effectiveConnectionId || undefined);
          ...
          if (provider) {
            ...
            void (async () => {
              try {
                ...
                log.warn("COMBO-RR", "Failed to record Last Known Good Provider. This is non-fatal.", {
                  err
                });
              }
            })();
        ...
        if (result.status === 499) {
          log.info("COMBO-RR", `Client disconnected (499) during ${modelStr} — stopping combo loop`);
          recordComboRequest(combo.name, modelStr, {
            success: false,
            latencyMs: Date.now() - startTime,
            fallbackCount,
        ...
        const isAllAccountsRateLimited = isAllAccountsRateLimitedResponse(result.status, result.headers?.get("content-type") ?? null, errorText);
        ...
        if (!isStreamReadinessFailure && !isTokenLimitBreach && TRANSIENT_FOR_SEMAPHORE.includes(result.status) && cooldownMs > 0) {
...
// 9router re-exports
export { handleFusionChat };

export function resetComboRotation(comboName) {
  // Use the comboRotationState declared above (line ~2457)
  const state = comboRotationState;
  if (comboName) state.delete(comboName);
  else state.clear();
}
