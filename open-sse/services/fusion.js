 ...
 */
import { errorResponse, sanitizeErrorMessage } from "../utils/error";
import { extractTextContent } from "../translator/helpers/geminiHelper";
...
export function extractPanelText(json) {
  ...
  if (Array.isArray(output)) {
    const t = output.flatMap(o => Array.isArray(o.content) ? o.content.map(c => typeof c?.text === "string" ? c.text : "") : []).join("");
    if (t.trim()) return t;
  }
  return "";
}
...
export function appendUserTurn(body, text) {
  ...
  if (Array.isArray(body.messages)) {
    ...
    next.input = [...body.input, {
      role: "user",
      content: text
    }];
  } else if (Array.isArray(body.contents)) {
    next.contents = [...body.contents, {
      role: "user",
      parts: [{
        text
      }]
 ...
 */
export function collectPanel(calls, cfg) {
  return new Promise(resolve => {
...
export async function handleFusionChat({
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    const resp = res;
    if (!resp.ok) {
      log.warn("FUSION", `Panel ${model} failed`, {
        status: resp.status
      });
      continue;
    }
