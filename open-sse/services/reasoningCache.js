...
import { clearAllReasoningCache, cleanupExpiredReasoning, deleteReasoningCache, getReasoningCache, getReasoningCacheEntries, getReasoningCacheStats, setReasoningCache } from "../stubs/lib/db/reasoningCache";

// ──────────────── Provider/Model Detection ────────────────

const REASONING_REPLAY_PROVIDERS = new Set(["deepseek", "opencode-go", "siliconflow", "nebius", "deepinfra", "sambanova", "fireworks", "together",
...
"xiaomi-mimo"]);
const REASONING_REPLAY_MODEL_PATTERNS = [/deepseek-r1/i, /deepseek-reasoner/i, /deepseek-chat/i, /deepseek[-/]v4[-.](flash|pro)(-free)?/i, /zen\/deepseek-v4/i, /kimi-k2/i, /qwq/i, /qwen.*think/i, /glm.*think/i,
...
function buildAssistantMessageCacheKey(requestId, messageIndex) {
 ...
 */
export function cacheReasoningBatch(toolCallIds, provider, model, reasoning) {
 ...
 */
export function cacheReasoningFromAssistantMessage(message, provider, model, context) {
  ...
  if (toolCallIds.length === 0) {
    ...
    cacheReasoningByKey(buildAssistantMessageCacheKey(requestId, messageIndex), provider, model, reasoning);
    return 1;
  }
 ...
 */
export function clearReasoningCacheAll(provider) {
  // Clear memory
  if (provider) {
    for (const [key, entry] of memoryCache) {
 ...
 */
export function deleteReasoningCacheEntry(toolCallId) {
 ...
 */
export function cleanupReasoningCache() {
  purgeExpiredMemory();
  try {
