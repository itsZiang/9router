import { BACKOFF_STEPS_MS, PROVIDER_PROFILES, RateLimitReason, HTTP_STATUS } from "../config/constants";
import { BACKOFF_CONFIG, COOLDOWN_MS, calculateBackoffCooldown, findMatchingErrorRule, matchErrorRuleByText, matchErrorRuleByStatus } from "../config/errorConfig";
import { getProviderErrorRuleMatch } from "../config/providerErrorRules";
import { getPassthroughProviders, getProviderCategory } from "../config/providerRegistry";
import { DEFAULT_RESILIENCE_SETTINGS, resolveResilienceSettings } from "../stubs/lib/resilience/settings";
import { resolveModelLockoutSettings } from "../stubs/lib/resilience/modelLockoutSettings";
import { getAllCircuitBreakerStatuses, getCircuitBreaker } from "../stubs/shared/utils/circuitBreaker";
import { classify429FromError, looksLikeQuotaExhausted } from "../stubs/shared/utils/classify429";
import { resolveProviderId } from "../../src/shared/constants/providers";
import { resolveUseUpstream429BreakerHints } from "../stubs/shared/utils/providerHints";
import { getCodexModelScope } from "../config/codexQuotaScopes";
import { getQuotaScopedModelForProvider } from "./antigravityQuotaFamily";
...
const lastConnectionFailure = new Map();
function pruneConnectionFailureDedupeEntries() {
...
export const ACCOUNT_DEACTIVATED_SIGNALS = ["account_deactivated", "account has been deactivated", "account has been disabled", "your account has been suspended", "this account is deactivated",
// AG (Antigravity/Google Cloud Code) permanent ban signals
"verify your account to continue", "this service has been disabled in this account for violation", "this service has been disabled in this account"];
...
let _customBannedSignals = [];
export function setCustomBannedSignals(signals) {
  _customBannedSignals = signals;
}
export function getMergedBannedSignals() {
...
export const CREDITS_EXHAUSTED_SIGNALS = ["insufficient_quota", "billing_hard_limit_reached", "exceeded your current quota", "exceeded your current usage quota", "credit_balance_too_low", "your credit balance is too low", "credits exhausted", "out of credits", "payment required", "free tier of the model has been exhausted",
...
"insufficient balance", "insufficient_balance", "insufficient account balance"];

// T11: Signals that indicate OAuth token is invalid/expired (not permanent deactivation)
export const OAUTH_INVALID_TOKEN_SIGNALS = ["invalid authentication credentials", "oauth 2", "login cookie", "valid authentication credential", "invalid credentials"];
...
const CONTEXT_OVERFLOW_PATTERNS = [/\binput is too long\b/i, /\binput too long\b/i, /\bcontext.*(too long|exceeded|overflow|limit)/i, /\btoo many tokens\b/i, /\bprompt is too long\b/i, /\bcontext window/i, /\bmaximum context/i, /\bmax.*token/i, /\btoken limit/i, /\brequest too large\b/i];
...
const MODEL_ACCESS_DENIED_CODES = new Set(["model_not_found",
// OpenAI, OpenAI-compatible (Kiro, Together, Fireworks, etc.)
"deployment_not_found" // Azure OpenAI
]);
const MODEL_ACCESS_DENIED_TYPES = new Set(["not_found_error" // Anthropic: model doesn't exist — reliably model-scoped
]);
...
const MODEL_ACCESS_AMBIGUOUS_TYPES = new Set(["permission_error" // Anthropic: could be model access OR key/org/feature scope
]);
...
const MODEL_ACCESS_DENIED_PATTERNS = [/\binvalid model\b/i, /\bmodel.*not.*(?:available|found|supported|accessible)\b/i, /\bmodel.*(?:does not exist|doesn't exist)\b/i, /\baccess.*denied.*model\b/i, /\bmodel.*access.*denied\b/i, /\bplease select a different model\b/i,
...
/\b(?:access|permission)[\s\S]{0,60}?\bmodel\b/i, /\bmodel[\s\S]{0,60}?\b(?:access|permission)\b/i];
...
const AUTH_CREDENTIAL_ERROR_PATTERNS = [/\b(?:invalid|incorrect|expired|missing|revoked)\s+api[\s_-]?key\b/i, /\bapi[\s_-]?key\s+(?:is\s+)?(?:invalid|incorrect|expired|missing|revoked|not\s+valid)\b/i, /\bauthentication\s+(?:failed|error|required)\b/i, /\b(?:invalid|expired|missing|revoked)\s+(?:token|credentials?|bearer)\b/i, /\bunauthorized\b/i, /\bnot\s+authenticated\b/i];
...
const MALFORMED_REQUEST_PATTERNS = [/\bimproperly formed request\b/i, /\binvalid.*message.*format/i, /\bmessages must alternate\b/i, /\bempty (message|content)\b/i,
// Tool call function name errors
/\bfunction'?s? name (?:can't|can not|is|has) (?:blank|empty|missing)/i, /function.*name.*(?:blank|empty|missing)/i, /tool_call.*name.*(?:blank|empty|missing)/i];
...
const RATE_LIMIT_TEXT_PATTERNS = [/high.?frequency/i, /non-compliant/i, /too many requests/i, /rate.?limit/i, /频繁/,
// "frequent" (zh) — high-frequency request throttling
...
const PARAM_VALIDATION_PATTERNS = [/max_tokens.*illegal/i, /max_tokens.*must be/i, /max_tokens.*range/i, /parameter is illegal/i, /is illegal.*range/i];
...
function isCompatibleProvider(provider) {
  return typeof provider === "string" && (provider.startsWith("openai-compatible-") || provider.startsWith("anthropic-compatible-"));
}
function buildProviderProfile(category, settings) {
  ...
  return {
    ...
    rateLimitCooldown: connectionCooldown.useUpstreamRetryHints ? 0 : connectionCooldown.baseCooldownMs,
...
export async function getRuntimeProviderProfile(provider) {
  try {
    const {
      getCachedSettings
    } = await import("../stubs/lib/db/readCache");
...
const canonicalProviderCache = new Map();
function getCanonicalLockProvider(provider) {
...
function getModelLockKey(provider, connectionId, model) {
  const canonicalProvider = getCanonicalLockProvider(provider);
  const lockModel = canonicalProvider === "codex" ? getCodexModelScope(model) : getQuotaScopedModelForProvider(canonicalProvider, model) || model;
  return `${canonicalProvider}:${connectionId}:${lockModel}`;
}
function getFailureWindowMs(profile = null, fallbackMs = 30 * 60 * 1000) {
  const configured = profile?.resetTimeoutMs;
  return typeof configured === "number" && configured > 0 ? configured : fallbackMs;
}
function cleanupModelLockKey(key, now = Date.now()) {
...
function getModelLockBaseCooldown(status, fallbackCooldownMs, profile = null) {
...
function getScaledCooldown(baseCooldownMs, failureCount, maxBackoffLevel = BACKOFF_CONFIG.maxLevel) {
...
function ensureCleanupTimer() {
  ...
  try {
    ...
    if (typeof _cleanupTimer === "object" && "unref" in _cleanupTimer) {
      _cleanupTimer.unref?.(); // Don't prevent process exit (Node.js only)
    }
  } catch {
 ...
 * backoff is disabled.
 */
export function selectLockoutCooldownMs(parsedCooldownMs, settings) {
...
export function recordModelLockoutFailure(provider, connectionId, model, reason, status, fallbackCooldownMs, profile = null, options = {}) {
  ensureCleanupTimer();
  const key = getModelLockKey(provider, connectionId, model);
  const now = Date.now();
  ...
  if (reason === "quota_exhausted" && typeof options.exactCooldownMs !== "number") {
    options = {
      ...options,
      exactCooldownMs: getMsUntilTomorrow()
    };
  }
  ...
  const withinWindow = previous && now - previous.lastFailureAt <= previous.resetAfterMs + (previous.lastCooldownMs ?? 0);
  ...
  const maxCooldownMs = typeof options.maxCooldownMs === "number" && options.maxCooldownMs > 0 ? options.maxCooldownMs : BACKOFF_CONFIG.max;
  const cooldownMs = typeof options.exactCooldownMs === "number" && options.exactCooldownMs > 0 ? options.exactCooldownMs : Math.min(getScaledCooldown(baseCooldownMs, failureCount, profile?.maxBackoffSteps ?? BACKOFF_CONFIG.maxLevel), maxCooldownMs);
 ...
 */
export function hasPerModelQuota(provider, _model = null, connectionPassthroughModels) {
 ...
 */
export function lockModelIfPerModelQuota(provider, connectionId, model, reason, cooldownMs, connectionPassthroughModels) {
...
export function shouldMarkAccountExhaustedFrom429(provider, model = null, connectionPassthroughModels, failureKind) {
  ...
  return shouldPreserveQuotaSignalsFor429(provider) && !hasPerModelQuota(provider, model, connectionPassthroughModels);
}
export function classifyLockoutReason(status) {
  if (status === 429) return "rate_limit";
  if (status === 403) return "quota_exhausted";
  return "unknown";
}
export function decayModelFailureCount(provider, connectionId, model) {
  const key = getModelLockKey(provider, connectionId, model);
  const failure = modelFailureState.get(key);
  if (!failure) return {
    cleared: false,
 ...
 */
export function clearAllModelLockouts() {
  modelLockouts.clear();
...
function getProviderBreaker(provider) {
  return provider ? getCircuitBreaker(provider) : null;
}
function configureProviderBreaker(provider, profile) {
...
export function getProviderBreakerState(provider) {
  const breaker = getProviderBreaker(provider);
 ...
 */
export function recordProviderFailure(provider, log, connectionId, profile) {
  if (!provider) return;
...
export function getProvidersInCooldown() {
  return getAllCircuitBreakerStatuses().filter(status => {
...
export function parseRetryFromErrorText(errorText) {
  ...
  const isoMatch = /\b(?:try again at|wait until|reset(?:s)? at|available at|retry after)\s+(\d{4}-\d{2}-\d{2}[Tt ]\d{2}:\d{2}(?::\d{2})?(?:\.\d+)?(?:Z|[+-]\d{2}:?\d{2})?)/i.exec(msg);
  if (isoMatch) {
...
function isSubscriptionQuotaText(lower) {
  return lower.includes("usage limit reached") || lower.includes("usage limit has been") || lower.includes("claude pro usage limit") || lower.includes("you've reached your usage limit") || lower.includes("you have reached your usage limit");
}
...
export function classifyErrorText(errorText) {
  ...
  const lower = String(errorText).toLowerCase();
  if (lower.includes("quota exceeded") || lower.includes("quota depleted") || lower.includes("quota will reset") || lower.includes("your quota will reset") || lower.includes("quota has been exceeded") || lower.includes("hour quota") || lower.includes("billing") || looksLikeQuotaExhausted(lower) ||
 ...
 */
export function classifyError(status, errorText, context) {
  ...
  if (context?.provider) {
    const match = getProviderErrorRuleMatch(context.provider, status, context.headers ?? null, context.body);
 ...
 */
export function getMsUntilTomorrow() {
...
export function isDailyQuotaExhausted(errorText) {
  ...
  return lower.includes("today's quota") || lower.includes("daily quota") || lower.includes("try again tomorrow");
}
 ...
 */
export function checkFallbackError(status, errorText, backoffLevel = 0, _model = null, provider = null, headers = null, profileOverride = null, structuredError) {
  ...
  if (status === 503 && headers) {
    const hintValue = typeof headers.get === "function" ? headers.get("x-omni-fallback-hint") : headers["x-omni-fallback-hint"] || headers["X-Omni-Fallback-Hint"];
  ...
  const retryableStatuses = new Set([HTTP_STATUS.REQUEST_TIMEOUT, HTTP_STATUS.RATE_LIMITED, HTTP_STATUS.SERVER_ERROR, HTTP_STATUS.BAD_GATEWAY, HTTP_STATUS.SERVICE_UNAVAILABLE, HTTP_STATUS.GATEWAY_TIMEOUT]);
  function parseResetFromHeaders(headers) {
    if (!headers) return null;
    const recordHeaders = headers;

    // Retry-After header
    const retryAfter = typeof headers.get === "function" ? headers.get("retry-after") : recordHeaders["retry-after"] || recordHeaders["Retry-After"];
    if (retryAfter) {
    ...
    // X-RateLimit-Reset
    const rlReset = typeof headers.get === "function" ? headers.get("x-ratelimit-reset") : recordHeaders["x-ratelimit-reset"] || recordHeaders["X-RateLimit-Reset"];
    if (rlReset) {
  ...
  function getScaledBaseCooldown(reason, level = backoffLevel) {
    void reason;
    const baseCooldownMs = typeof profile?.baseCooldownMs === "number" && profile.baseCooldownMs >= 0 ? profile.baseCooldownMs : COOLDOWN_MS.transientInitial;
    return {
      baseCooldownMs,
  ...
  if (errorText) {
    ...
    if (shouldUseQuotaSignal && isCreditsExhausted(errorStr)) {
      return {
        shouldFallback: true,
        cooldownMs: COOLDOWN_MS.paymentRequired ?? 3600 * 1000,
        // 1h cooldown
    ...
    // subscription quota.)
    if (shouldUseQuotaSignal && !isCreditsExhausted(errorStr) && !isDailyQuotaExhausted(errorStr) && isSubscriptionQuotaText(errorStr.toLowerCase())) {
    ...
    if (shouldUseQuotaSignal && quotaResetHintMs && classifyErrorText(errorStr) === RateLimitReason.QUOTA_EXHAUSTED) {
    ...
    if (status === HTTP_STATUS.FORBIDDEN && errorStr.toLowerCase().includes("not authorized for this route")) {
      return {
    ...
    if (status === HTTP_STATUS.FORBIDDEN && provider && getProviderCategory(provider) === "apikey" && !errorStr.toLowerCase().includes("has not been used in project") && !errorStr.toLowerCase().includes("hour quota") && !errorStr.toLowerCase().includes("quota has been exceeded")) {
  ...
  if (configuredRule) {
    if (configuredRule.backoff) {
      ...
      const providerMatch = provider ? getProviderErrorRuleMatch(provider, status, headers, structuredError ?? null) : null;
      const reason = providerMatch ? providerMatch.reason : configuredRule.reason ?? RateLimitReason.UNKNOWN;
  ...
  if (status === HTTP_STATUS.BAD_REQUEST) {
    ...
    const structuredCode = typeof structuredError?.code === "string" ? structuredError.code.toLowerCase() : "";
    const structuredType = typeof structuredError?.type === "string" ? structuredError.type.toLowerCase() : "";
    ...
    const matchesModelAccessPattern = !looksLikeAuthCredentialError && MODEL_ACCESS_DENIED_PATTERNS.some(p => p.test(errorStr));
    const isModelAccessDeniedStructured = !!structuredError && (MODEL_ACCESS_DENIED_CODES.has(structuredCode) || MODEL_ACCESS_DENIED_TYPES.has(structuredType) ||
 ...
 */
export function cooldownUntilMs(value) {
...
export function applyErrorState(account, status, errorText, provider = null) {
  ...
  const newBackoffLevel = "newBackoffLevel" in fallbackDecision ? fallbackDecision.newBackoffLevel : undefined;
  return {
    ...account,
