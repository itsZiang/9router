...
export function isClaudeOauthUsageCoolingDown(accessToken, now = Date.now()) {
...
export function markClaudeOauthUsage429(accessToken, now = Date.now(), cooldownMs = OAUTH_USAGE_429_COOLDOWN_MS) {
  if (!accessToken) return;
  oauthCooldown.set(accessToken, now + cooldownMs);
}

/** Test-only: clear all entries. */
export function _resetClaudeOauthUsageCooldown() {
  oauthCooldown.clear();
}