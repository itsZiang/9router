...
export function cloakThirdPartyToolNames(body, options) {
  ...
  const assigned = new Map(); // original -> alias
  ...
  if (Array.isArray(messages)) {
    body.messages = messages.map(message => {
      ...
      const newContent = content.map(block => {
        if (block?.type === "tool_use" && typeof block.name === "string" && shouldCloak(block.name)) {
          changed = true;
          return {
            ...block,
            name: aliasFor(block.name)
          };
        }
        return block;
      });
  ...
  const toolChoice = body.tool_choice;
  if (toolChoice?.type === "tool" && typeof toolChoice.name === "string" && shouldCloak(toolChoice.name)) {
    body.tool_choice = {
      ...toolChoice,
      name: aliasFor(toolChoice.name)
    };
  }
