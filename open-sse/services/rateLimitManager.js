...
import { parseRetryAfterFromBody } from "./accountFallback";
import { getProviderCategory } from "../config/providerRegistry";
import { getCodexRateLimitKey } from "../executors/codex";
import { awaitProviderDefaultSlot } from "./providerDefaultRateLimit";
import { DEFAULT_RESILIENCE_SETTINGS, resolveResilienceSettings } from "../stubs/lib/resilience/settings";
import { STANDARD_HEADERS, ANTHROPIC_HEADERS, parseResetTime, toPlainHeaders } from "./rateLimitManager/headers";
...
const connectionRateLimitOverrides = new Map();
...
function reconcileEnabledConnections(connectionsRaw, requestQueueSettings) {
  const nextEnabledConnections = new Set();
  let explicitCount = 0;
  let autoCount = 0;
...
function watchdogTick() {
  ...
  for (const [key, limiter] of Array.from(limiters)) {
    ...
    warnRateLimit(`🚨 [RATE-LIMIT] WEDGED: ${key} queued=${counts.QUEUED} running=0 executing=0 stalled=${stalledMs}ms — force-resetting`);
    limiters.delete(key);
    lastDispatchAt.delete(key);
    limiterLastUsed.delete(key);
...
export function startRateLimitWatchdog() {
...
export function stopRateLimitWatchdog() {
  if (!watchdogInterval) return;
...
export async function initializeRateLimits() {
  ...
  try {
    ...
    if (explicitCount > 0 || autoCount > 0) {
      logRateLimit(`🛡️ [RATE-LIMIT] Loaded ${explicitCount} explicit + ${autoCount} auto-enabled protection(s)`);
    }
...
export async function applyRequestQueueSettings(nextSettings) {
  currentRequestQueueSettings = {
    ...nextSettings
  };
  const {
...
export async function withRateLimit(provider, connectionId, model, fn, signal = null) {
  ...
  const scheduleOpts = maxWaitMs && maxWaitMs > 0 ? {
    expiration: maxWaitMs
  } : {};
  try {
    if (signal) {
      let abortListener;
    ...
    if (err?.message?.includes("This job timed out")) {
      const key = getLimiterKey(provider, connectionId, model);
      logRateLimit(`⏰ [RATE-LIMIT] ${key} — job expired after ${Math.ceil((maxWaitMs || 0) / 1000)}s in queue, dropping`);
      const queueErr = new Error(`Request dropped after exceeding the local rate-limit queue budget maxWaitMs (${maxWaitMs}ms) for ` + `${model ? `${provider}/${model}` : provider} — this is OmniRoute's request queue ` + `(resilienceSettings.requestQueue.maxWaitMs), not an upstream timeout. Raise it in ` + `Settings → Resilience if this is queue saturation rather than a slow provider.`, {
        cause: err
      });
...
export function updateFromHeaders(provider, connectionId, headers, status, model = null) {
  ...
  if (status === 429) {
    ...
    logRateLimit(`🚫 [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — 429 received, pausing for ${Math.ceil(retryAfterMs / 1000)}s, dropping ${counts.QUEUED} queued request(s)`);
  ...
  if (overLimit === "yes") {
    logRateLimit(`⚠️ [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — near capacity, slowing down`);
  ...
  if (!isNaN(limit) && limit > 0) {
    ...
    if (!isNaN(remaining)) {
      if (remaining < limit * 0.1) {
        ...
        logRateLimit(`⚠️ [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — ${remaining}/${limit} remaining, throttling`);
    ...
    recordLearnedLimit(provider, connectionId, {
      limit,
      remaining,
      minTime: updates.minTime
    }, model);
  }
}
...
async function persistLearnedLimitsNow() {
  try {
    const {
      updateSettings
    } = await import("../stubs/lib/db/settings");
    await updateSettings({
      learnedRateLimits: JSON.stringify(learnedLimits)
    });
    logRateLimit(`💾 [RATE-LIMIT] Persisted learned limits for ${Object.keys(learnedLimits).length} provider(s)`);
  } catch (err) {
...
export function updateFromResponseBody(provider, connectionId, responseBody, status, model = null) {
  ...
  const {
    retryAfterMs,
    reason
  } = parseRetryAfterFromBody(responseBody);
  if (retryAfterMs && retryAfterMs > 0) {
    const limiter = getLimiter(provider, connectionId, model);
    logRateLimit(`🚫 [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — body-parsed retry: ${Math.ceil(retryAfterMs / 1000)}s (${reason})`);
    limiter.updateSettings({
      reservoir: 0,
