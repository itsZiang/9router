 ...
 */
export function injectModelTag(messages, providerModel) {
  // Remove any existing tags first to avoid duplication on context compaction
  const cleaned = messages.map(msg => {
    if (msg.role === "assistant" && typeof msg.content === "string") {
      return {
        ...msg,
        content: msg.content.replace(CACHE_TAG_PATTERN_GLOBAL, "").trimEnd()
      };
    }
    return msg;
  });
  ...
  if (lastAssistantIdx === -1) {
    return [...cleaned, {
      role: "assistant",
      content: `<omniModel>${providerModel}</omniModel>`
    }];
  }
 ...
 */
export function extractPinnedModel(messages) {
 ...
 */
export function applySystemMessageOverride(messages, systemMessage) {
 ...
 */
export function applyToolFilter(tools, pattern) {
 ...
 */
export function stripModelTags(messages) {
 ...
 */
export function applyComboAgentMiddleware(body, comboConfig, providerModel) {
  ...
  // 3. Tool filter
  const filteredTools = applyToolFilter(body.tools, comboConfig.tool_filter_regex);
