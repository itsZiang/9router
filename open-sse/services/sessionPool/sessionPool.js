...
export class SessionPool {
  ...
  getSessionDetails() {
    return this.sessions.map(s => ({
      ...
      inflight: s.inflight,
      cooldownRemaining: s.cooldownRemaining > 0 ? `${(s.cooldownRemaining / 1000).toFixed(1)}s` : "0s",
      age: `${(s.age / 1000).toFixed(0)}s`
    }));
  }

  /** As acquire(), but blocks until a session is available */
  async acquireBlocking(timeoutMs = 10_000) {
    ...
    throw new Error(`[SessionPool:${this.provider}] No session available after ${timeoutMs}ms timeout`);
  }

  /** As acquireBlocking(), but accepts arbitrary function to wrap */
  async executeWithSession(fn, timeoutMs = 10_000) {
  ...
  pruneDeadSessions(maxIdleMs = 300_000) {
    const now = Date.now();
  ...
  startAutoPrune(intervalMs = 60_000) {
