...
import { getSettings } from "../../stubs/lib/db/settings";
import { getProviderRegistry } from "./providerRegistryAccessor";
import { NOAUTH_PROVIDERS } from "@/shared/constants/providers";
import { hasUsableWebSessionCredential } from "../../stubs/shared/providers/webSessionCredentials";
...
import { buildAutoCandidateFilter, tierToWeightVariant } from "./suffixComposition";
...
function isChatAutoComboNoAuthProvider(providerDef) {
  if (providerDef.noAuth !== true) return false;
  if (!Array.isArray(providerDef.serviceKinds) || providerDef.serviceKinds.length === 0) return true;
  return providerDef.serviceKinds.includes("llm");
}
function getNoAuthCandidates(excludedProviders, blockedProviders) {
  ...
  for (const providerDef of Object.values(NOAUTH_PROVIDERS)) {
    ...
    if (blockedProviders.has(providerId) || typeof providerDef.alias === "string" && blockedProviders.has(providerDef.alias)) continue;
 ...
 */
export function computeAdvertisedLimits(candidates) {
...
export async function createVirtualAutoCombo(variant, spec) {
  const [connections, settings] = await Promise.all([getProviderConnections({
    isActive: true
  }), getSettings().catch(() => ({}))]);
  const blockedProviders = new Set(Array.isArray(settings.blockedProviders) ? settings.blockedProviders : []);
  ...
  candidatePool.push(...getNoAuthCandidates(new Set(validConnections.map(conn => conn.provider)), blockedProviders));
  ...
  if (candidateFilter) {
    const narrowed = candidatePool.filter(c => candidateFilter({
      provider: c.provider,
      model: c.model
    }));
    if (narrowed.length > 0) {
      effectivePool = narrowed;
    } else if (process.env.OMNIROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL === "true" || process.env.OMNIROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL === "1") {
      // Opt-in legacy behavior: warn loudly, then keep the full pool.
      log.warn("AUTO", `auto/${spec?.category ?? ""}${spec?.tier ? `:${spec.tier}` : ""} matched no connected models; falling back to the full pool (OMNIROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL=true)`);
    } else {
      log.warn("AUTO", `auto/${spec?.category ?? ""}${spec?.tier ? `:${spec.tier}` : ""} matched no connected models; returning an empty pool. Set OMNIROUTE_AUTO_FREE_FALLBACK_TO_FULL_POOL=true to restore the legacy "use full pool" behavior.`);
      effectivePool = [];
    }
  }
  let weights = {
  ...
  switch (variant) {
    ...
    case "fast":
      weights = {
        ...MODE_PACKS["ship-fast"]
      };
      break;
    case "cheap":
      weights = {
        ...MODE_PACKS["cost-saver"]
      };
      break;
    case "offline":
      weights = {
        ...MODE_PACKS["offline-friendly"]
      };
      break;
    case "smart":
      weights = {
