...
import { getModelIntelligenceBySource, setUserFitnessOverrideEntry, deleteUserFitnessOverrideEntry } from "../../stubs/lib/db/modelIntelligence";
const FITNESS_TABLE = {
  coding: {
    "claude-sonnet": 0.95,
    "claude-opus": 0.92,
    ...
    "deepseek-r1": 0.88,
    "deepseek-chat": 0.84,
    // DeepSeek V3.2 Chat — strong code performance
    "deepseek-v3.2": 0.86,
    // Explicit V3.2 alias
    qwen: 0.78,
    llama: 0.72,
  ...
  analysis: {
    ...
    "gemini-pro": 0.88,
    "gemini-3.1-pro": 0.95,
    // Gemini 3.1 Pro — 1M context, ideal for long analysis
    "gpt-4o": 0.85,
    o1: 0.9,
    o3: 0.93,
    "deepseek-r1": 0.88,
    "deepseek-chat": 0.8,
    "kimi-k2": 0.82,
    // Kimi K2.5 agentic — good for analysis
    "glm-5.1": 0.82,
    // GLM-5.1 free reasoning, 200K context for long analysis
    "glm-5": 0.78,
    // GLM-5 with 128k output for long analysis
    "minimax-m2.5": 0.76
  },
  debugging: {
    "claude-sonnet": 0.93,
  ...
  default: {
    ...
    "grok-4-fast": 0.72,
    // ultra-fast, suitable for all tasks
    "grok-4": 0.74,
    "grok-3": 0.73,
    "kimi-k2": 0.76,
...
const _intelligenceCache = new Map();
function queryModelIntelligence(model, category, source) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      cache[modelId.toLowerCase()] = {
        tool_call: row.tool_call === true || row.tool_call === 1 ? true : row.tool_call === false || row.tool_call === 0 ? false : null,
        reasoning: row.reasoning === true || row.reasoning === 1 ? true : row.reasoning === false || row.reasoning === 0 ? false : null,
...
export function getModelsDevTierFitness(model, taskType) {
...
function lookupStaticFitnessTable(normalizedModel, normalizedTask) {
...
function lookupWildcardBoosts(normalizedModel, normalizedTask) {
  let baseScore = 0.5;
...
export function getTaskFitnessWithSource(model, taskType) {
  const normalizedModel = model.toLowerCase();
  const normalizedTask = taskType.toLowerCase();
 ...
 */
function lookupFreeAliasArenaElo(normalizedModel, normalizedTask) {
...
export function setUserFitnessOverride(model, category, score) {
  try {
    setUserFitnessOverrideEntry(model.toLowerCase(), category.toLowerCase(), score);
    invalidateFitnessCache();
  } catch (err) {
    throw new Error(`Failed to set user fitness override for ${model}/${category}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
export function clearUserFitnessOverride(model, category) {
  try {
    deleteUserFitnessOverrideEntry(model.toLowerCase(), category.toLowerCase());
    invalidateFitnessCache();
  } catch (err) {
    throw new Error(`Failed to clear user fitness override for ${model}/${category}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
