...
export const AUTO_CATEGORIES = ["coding", "reasoning", "vision", "chat", "multimodal"];
export const AUTO_TIERS = ["fast", "cheap", "floor", "free", "reliable", "pro"];
...
export function parseAutoSuffix(suffix) {
  ...
  if (tail !== undefined) {
    if (!CATEGORY_SET.has(head) || !TIER_SET.has(tail)) return {
      valid: false
    };
    return {
      valid: true,
  ...
  if (CATEGORY_SET.has(head)) return {
    valid: true,
    category: head
  };
  return {
 ...
 */
export function tierToWeightVariant(tier) {
  switch (tier) {
    case "fast":
      return "fast";
    case "cheap":
