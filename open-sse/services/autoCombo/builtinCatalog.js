...
export const VALID_AUTO_VARIANTS = new Set(VALID_VARIANTS);
 ...
 */
export const AUTO_SUFFIX_VARIANTS = ["auto/coding:fast", "auto/coding:cheap", "auto/coding:free", "auto/coding:pro", "auto/coding:reliable", "auto/reasoning", "auto/reasoning:pro", "auto/vision", "auto/multimodal"];
export function resolveAutoVariant(modelStr, suffix) {
  if (Object.prototype.hasOwnProperty.call(AUTO_TEMPLATE_VARIANTS, modelStr)) {
    return {
      recognized: true,
      variant: AUTO_TEMPLATE_VARIANTS[modelStr]
    };
  }
 ...
 */
export function isRecognizedBuiltinAuto(modelStr, suffix) {
  return resolveAutoVariant(modelStr, suffix).recognized || parseAutoSuffix(suffix).valid;
}
export async function createBuiltinAutoCombo(modelStr, suffix) {
  const {
    createVirtualAutoCombo
  } = await import("./virtualFactory");
  const resolved = resolveAutoVariant(modelStr, suffix);
  if (resolved.recognized) {
    const spec = modelStr === "auto/best-free" ? {
      tier: "free"
    } : undefined;
