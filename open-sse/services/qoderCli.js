...
export function getQoderCliCommand() {
  const explicit = String(process.env.CLI_QODER_BIN || "").trim();
  return explicit || "qodercli";
}
export function getQoderCliWorkspace() {
  const explicit = String(process.env.QODER_CLI_WORKSPACE || process.env.OMNIROUTE_QODER_WORKSPACE || "").trim();
  if (explicit) return explicit;
  const home = String(process.env.HOME || "").trim();
  return home || process.cwd();
}
export function normalizeQoderPatProviderData(providerSpecificData = {}) {
  return {
...
export function isQoderCliTransport(providerSpecificData = {}) {
...
export function getStaticQoderModels() {
  return QODER_STATIC_MODELS.map(model => ({
    ...model
  }));
}
export function mapQoderModelToLevel(model) {
...
export function buildQoderPrompt(body) {
  const requestBody = asRecord(body);
  const lines = ["You are answering an OmniRoute OpenAI-compatible request through the Qoder CLI transport.", "Respond as a plain language model only.", "Do not use your own tools, do not inspect files, and do not run commands.", "Do not mention the adapter unless the user explicitly asks."];
...
export function extractTextFromQoderEnvelope(parsed) {
...
export function parseQoderCliFailure(stderrText, stdoutText = "") {
  const stderr = String(stderrText || "").trim();
  const stdout = String(stdoutText || "").trim();
  const combined = `${stderr}\n${stdout}`.trim() || "Qoder API request failed";
  const normalized = combined.toLowerCase();
  if (normalized.includes("invalid api key") || normalized.includes("invalid token") || normalized.includes("personal access token") || normalized.includes("unauthorized") && normalized.includes("qoder")) {
    return {
      status: 401,
      message: combined,
...
export function createQoderErrorResponse(failure) {
...
-----END PUBLIC KEY-----`;
export function buildCosyHeadersForValidation(bodyStr, token) {
...
const qoderJobTokenCache = new Map();
const qoderJobTokenPending = new Map();
/** A Qoder Personal Access Token is the only credential that needs the exchange. */
export function isQoderPatToken(token) {
  return typeof token === "string" && token.trim().startsWith("pt-");
}

/** Pull a `jt-*` job token out of the (loosely-specified) exchange response. */
export function parseQoderJobTokenResponse(json) {
  const root = asRecord(json);
  const data = asRecord(root.data);
  const candidates = [root.job_token, root.jobToken, root.jt, root.token, data.job_token, data.jobToken, data.jt, data.token];
  const jobToken = candidates.map(getString).find(v => v.trim().startsWith("jt-")) || "";
  if (!jobToken) return null;
  const expiresRaw = [root.expires_in, root.expiresIn, data.expires_in, data.expiresIn].find(v => typeof v === "number" && Number.isFinite(v) && v > 0);
  ...
  return {
    jobToken,
    expiresInMs: Math.max(expiresInMs, QODER_JOB_TOKEN_MIN_TTL_MS)
  };
}

/** Exchange a `pt-*` PAT for a short-lived `jt-*` job token (no caching). */
export async function exchangeQoderJobToken(pat, options = {}) {
 ...
 */
export async function resolveQoderJobToken(token, options = {}) {
  ...
  if (!pending) {
    pending = exchangeQoderJobToken(trimmed, {
...
export function __clearQoderJobTokenCache() {
...
export async function validateQoderCliPat({
  ...
  const modelId = getString(providerSpecificData.validationModelId).trim() || getString(providerSpecificData.modelId).trim() || QODER_DEFAULT_MODEL;
  ...
  try {
    ...
    return {
      valid: false,
      error: `Cannot reach Qoder API (${pingErr.message}). ` + "If behind a proxy, configure HTTPS_PROXY. For Docker, ensure the container has internet access.",
      unsupported: false
    };
  }
  ...
  const endpoint = "https://api1.qoder.sh/algo/api/v2/service/pro/sse/agent_chat_generation?AgentId=agent_common";
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      return {
        valid: false,
        error: `Authentication failed (HTTP ${res.status}). ` + "Make sure you're using a valid Personal Access Token from https://qoder.com/account/integrations. " + "Note: tokens from ~/.qoder/.auth/user are encrypted and cannot be used directly." + (errorDetail ? ` Server: ${errorDetail}` : ""),
        unsupported: false
      };
    }
    ...
    if (res.status >= 500) {
      ...
      const hasAuthSignal = /(unauthorized|forbidden|expired|revoked|not\s*authorized|permission\s*denied|access\s*denied|invalid\s*(?:token|credential|api[\s_-]*key)|token\s*(?:invalid|expired|revoked))/i.test(errorDetail);
      if (isCosyResponse && hasAuthSignal) {
        return {
          valid: false,
          error: `Authentication failed (HTTP ${res.status}). The Qoder Cosy server rejected the token ` + "as invalid, expired, or not authorized. " + "Please check your token at https://qoder.com/account/integrations." + (errorDetail ? ` Server response: ${errorDetail}` : ""),
          unsupported: false
        };
      }
      return {
