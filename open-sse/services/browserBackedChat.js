...
import { acquireBrowserContext, openPage, readPageResponseBody, shutdownPool } from "./browserPool";
import tlsClient from "../utils/tlsClient";
...
export function __setBrowserBackedChatOverrideForTesting(fn) {
  testOverride = fn;
}
export function __resetBrowserBackedChatOverrideForTesting() {
  testOverride = null;
  cookieCache.clear();
}
export function __setHttpBackedChatOverrideForTesting(fn) {
  httpOverride = fn;
}
export function __resetHttpBackedChatOverrideForTesting() {
  httpOverride = null;
  cookieCache.clear();
}
...
//    ignored.
export function chatUrlMatcher(u, matchDomain, chatUrl) {
...
export async function browserBackedChat(req) {
  ...
  const {
    key,
    acquired: reuseAcquired
  } = await settlePoolKey(poolKey, reuseContext);
  ...
  try {
    ...
    const responsePromise = page.waitForResponse(r => r.request().method() === "POST" && chatUrlMatcher(r.url(), chatUrlMatchDomain, chatUrl), {
      timeout: 30000
    });
 ...
 */
export async function httpBackedChat(req) {
...
async function doRefresh(poolKey, chatPageUrl, cookieDomain, signal) {
  const {
    key
  } = await settlePoolKey(poolKey, true);
  let pooled;
  try {
...
async function startBrowserWarmup(req) {
  ...
  try {
    const {
      key
    } = await settlePoolKey(req.poolKey, true);
 ...
 */
export async function tryBackedChat(req) {
  ...
  try {
    const fast = await httpBackedChat({
      ...req,
    ...
    if (req.cookieDomain) {
      ...
      if (!freshCookie) {
        ...
        freshCookie = await getFreshCookiesWithWarmup(req.poolKey, req.chatPageUrl, req.cookieDomain, effectiveSignal, warmupPromise);
        if (freshCookie) {
          const retry = await httpBackedChat({
            ...req,
            cookieString: freshCookie,
