...
const TASK_PATTERNS = {
  coding: {
    patterns: ["write code", "write a function", "implement", "debug", "fix this", "fix the", "refactor", "unit test", "write test", "write a script", "code review", "complete this function", "add a feature", "javascript", "typescript", "python", "sql query", "api endpoint"],
    userPatterns: ["```", "def ", "function ", "class ", "import ", "const ", "let ", "var ", "SELECT ", "INSERT ", "<html", "<div"]
  },
  creative: {
    patterns: ["write a story", "write a poem", "write a song", "creative writing", "write a blog", "write an article", "write a script", "write an essay", "imagine", "roleplay", "brainstorm", "creative"]
  },
  analysis: {
    patterns: ["analyze", "analyse", "analysis", "compare", "evaluate", "assess", "explain", "reasoning", "pros and cons", "advantages and disadvantages", "what are the implications", "in-depth", "comprehensive"]
  },
  vision: {
    patterns: ["look at this image", "in this image", "what do you see", "describe this image", "analyze this image", "read this screenshot"],
    userPatterns: ["image_url", "data:image"]
  },
  summarization: {
    patterns: ["summarize", "summary", "tldr", "tl;dr", "brief overview", "key points", "main points", "what did", "highlights from"]
  },
  background: {
    patterns: ["generate a title", "generate title", "create a title", "name this", "short description", "brief description", "one-line summary", "conversation title"]
  },
  chat: {
    patterns: []
  }
...
export function setTaskRoutingConfig(config) {
  _config = {
    ..._config,
    ...config,
...
export function getTaskRoutingConfig() {
  return {
    ..._config,
    taskModelMap: {
      ..._config.taskModelMap
    },
    stats: {
      ..._config.stats
    }
...
export function resetTaskRoutingStats() {
  _config.stats = {
    detected: 0,
    routed: 0
  };
}
export function getDefaultTaskModelMap() {
  return {
    ...DEFAULT_TASK_MODEL_MAP
  };
}
...
function extractText(content) {
  ...
  if (Array.isArray(content)) {
    return content.map(part => typeof part === "string" ? part.toLowerCase() : part?.text?.toLowerCase() || "").join(" ");
  }
  return "";
}
...
export function detectTaskType(body) {
  ...
  const priorityOrder = ["background", "coding", "vision", "summarization", "analysis", "creative"];
  for (const taskType of priorityOrder) {
    const {
      patterns,
      userPatterns
    } = TASK_PATTERNS[taskType];
 ...
 */
export function applyTaskAwareRouting(originalModel, body) {
