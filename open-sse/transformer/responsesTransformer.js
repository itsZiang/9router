...
export function createResponsesApiTransformStream(logger = null, keepaliveIntervalMs = 3000) {
  ...
  const buildDenseOutput = () => state.completedOutputItems.slice().sort((left, right) => {
    ...
    transform(chunk, controller) {
      ...
      for (const msg of messages) {
        ...
        if (state.parseTextualReasoningTags !== true && typeof parsed.model === "string") {
          state.parseTextualReasoningTags = shouldParseTextualReasoningTags(undefined, parsed.model);
        }
          ...
          for (const tc of delta.tool_calls) {
            ...
            if (tc.function?.arguments) {
              ...
              if (deltaStr.includes('""') || deltaStr.includes("[]") || deltaStr.includes("[ ]")) {
                deltaStr = deltaStr.replace(/,"[a-zA-Z0-9_]+":""/g, "").replace(/"[a-zA-Z0-9_]+":"",/g, "").replace(/,"[a-zA-Z0-9_]+":\s*\[\s*\]/g, "").replace(/"[a-zA-Z0-9_]+":\s*\[\s*\],?/g, "");
              }
