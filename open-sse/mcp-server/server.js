...
import { getComboModelProvider, getComboModelString, getComboStepTarget } from "../stubs/lib/combos/steps";
import { registerToolSearchTool } from "./toolSearch/register";
import { MCP_TOOLS, getHealthInput, listCombosInput, getComboMetricsInput, switchComboInput, checkQuotaInput, routeRequestInput, costReportInput, listModelsCatalogInput, webSearchInput, webFetchInput, simulateRouteInput, setBudgetGuardInput, setRoutingStrategyInput, setResilienceProfileInput, testComboInput, getProviderMetricsInput, bestComboForTaskInput, explainRouteInput, getSessionSnapshotInput, dbHealthCheckInput, syncPricingInput, cacheStatsInput, cacheFlushInput, oneproxyFetchInput, oneproxyRotateInput, oneproxyStatsInput } from "./schemas/tools";
import { startMcpHeartbeat } from "./runtimeHeartbeat";
import { z } from "zod";
import { closeAuditDb, logToolCall } from "./audit";
import { evaluateToolScopes, resolveCallerScopeContext } from "./scopeEnforcement";
import { getMcpHttpAuthHeadersForInternalFetch } from "./httpAuthContext";
import { handleSimulateRoute, handleSetBudgetGuard, handleSetRoutingStrategy, handleSetResilienceProfile, handleTestCombo, handleGetProviderMetrics, handleBestComboForTask, handleExplainRoute, handleGetSessionSnapshot, handleDbHealthCheck, handleSyncPricing, handleCacheStats, handleCacheFlush, handleOneproxyFetch, handleOneproxyRotate, handleOneproxyStats } from "./tools/advancedTools";
...
import { skillRegistry } from "../stubs/lib/skills/registry";
import { skillExecutor } from "../stubs/lib/skills/executor";
...
import { compressMcpRegistryMetadata } from "./descriptionCompressor";
import { reduceToolManifest, readMcpToolProfileFromEnv } from "./toolCardinality";
import { smartFilterText } from "../services/compression/engines/mcpAccessibility/index";
import { DEFAULT_MCP_ACCESSIBILITY_CONFIG, clampMcpAccessibilityConfig } from "../services/compression/engines/mcpAccessibility/constants";
import { getDbInstance } from "../stubs/lib/db/core";
import { getProviderConnections } from "../stubs/lib/db/providers";
import { getCodexRequestDefaults } from "../stubs/lib/providers/requestDefaults";
import { normalizeQuotaResponse } from "../stubs/shared/contracts/quota";
import { AI_PROVIDERS, NOAUTH_PROVIDERS } from "../../src/shared/constants/providers";
import { resolveOmniRouteBaseUrl } from "../stubs/shared/utils/resolveOmniRouteBaseUrl";
const OMNIROUTE_BASE_URL = resolveOmniRouteBaseUrl();
const MCP_ENFORCE_SCOPES = process.env.OMNIROUTE_MCP_ENFORCE_SCOPES === "true";
const MCP_ALLOWED_SCOPES = new Set((process.env.OMNIROUTE_MCP_SCOPES || "").split(",").map(s => s.trim()).filter(Boolean));
const TOTAL_MCP_TOOL_COUNT = MCP_TOOLS.length + Object.keys(memoryTools).length + Object.keys(skillTools).length + Object.keys(agentSkillTools).length + Object.keys(poolTools).length + gamificationTools.length + pluginTools.length + notionTools.length + obsidianTools.length;
...
export async function getMcpModelsCatalog(args, deps = {}) {
  const fetchJson = deps.fetchJson ?? (path => omniRouteFetch(path));
  ...
  for (const spec of requestSpecs) {
    ...
    sources.add(source);
    const rawModels = Array.isArray(raw.models) ? raw.models : Array.isArray(raw.data) ? raw.data : [];
...
function withScopeEnforcement(toolName, handler, toolScopes) {
  return async (args, extra) => {
    ...
    if (!scopeCheck.allowed) {
      ...
      const safeArgs = args && typeof args === "object" ? toRecord(args) : {
        rawArgs: args
      };
      await logToolCall(toolName, {
        ...safeArgs,
        _scopeCheck: {
      ...
      }, null, 0, false, `scope_denied:${reason}`);
      return {
        content: [{
          type: "text",
          text: `Error: ${msg}`
...
async function handleGetHealth() {
  ...
  try {
    const [healthRaw, resilienceRaw, rateLimitsRaw] = await Promise.allSettled([omniRouteFetch("/api/monitoring/health"), omniRouteFetch("/api/resilience"), omniRouteFetch("/api/rate-limits")]);
...
async function handleGetComboMetrics(args) {
  const start = Date.now();
  try {
    const result = await omniRouteFetch(`/api/combos/metrics?comboId=${encodeURIComponent(args.comboId)}`);
...
async function handleCheckQuota(args) {
  ...
  try {
    ...
    if (args.connectionId) path += `?connectionId=${encodeURIComponent(args.connectionId)}`;else if (args.provider) path += `?provider=${encodeURIComponent(args.provider)}`;
...
async function handleRouteRequest(args) {
  ...
  try {
    ...
    await logToolCall("omniroute_route_request", {
      model: args.model,
      messageCount: args.messages.length
    }, result.routing, Date.now() - start, true);
    return {
      content: [{
        type: "text",
        text: JSON.stringify(result, null, 2)
...
async function handleCostReport(args) {
  ...
  try {
    ...
    const raw = toRecord(await omniRouteFetch(`/api/usage/analytics?range=${encodeURIComponent(range)}`));
...
async function handleListModelsCatalog(args) {
  ...
  try {
    ...
    await logToolCall("omniroute_list_models_catalog", args, {
...
export function createMcpServer() {
  ...
  server.registerTool = (name, config, handler) => {
    ...
    if (toolProfile && reduceToolManifest([{
      name,
      scopes: []
    }], toolProfile).length === 0) {
  ...
  const RESERVED_MCP_NAMES = new Set([...MCP_TOOLS.map(t => t.name), ...Object.keys(memoryTools), ...Object.keys(skillTools), ...Object.keys(compressionTools), ...Object.keys(poolTools), ...pluginTools.map(t => t.name), ...gamificationTools.map(t => t.name), ...obsidianTools.map(t => t.name), ...notionTools.map(t => t.name)]);
  ...
  server.registerTool("omniroute_web_fetch", {
  ...
  server.registerTool("omniroute_cache_stats", {
  ...
  server.registerTool("omniroute_cache_flush", {
  ...
  }, withScopeEnforcement("omniroute_cache_flush", args => handleCacheFlush(cacheFlushInput.parse(args))));
  server.registerTool("omniroute_oneproxy_fetch", {
  ...
  }, withScopeEnforcement("omniroute_oneproxy_fetch", args => handleOneproxyFetch(oneproxyFetchInput.parse(args))));
  server.registerTool("omniroute_oneproxy_rotate", {
  ...
  }, withScopeEnforcement("omniroute_oneproxy_rotate", args => handleOneproxyRotate(oneproxyRotateInput.parse(args))));
  server.registerTool("omniroute_oneproxy_stats", {
  ...
  }, withScopeEnforcement("omniroute_oneproxy_stats", args => handleOneproxyStats(oneproxyStatsInput.parse(args))));
  ...
  Object.values(poolTools).forEach(toolDef => {
    server.registerTool(toolDef.name, {
      description: toolDef.description,
      // @ts-ignore: dynamic zod access
      inputSchema: toolDef.inputSchema
  ...
  try {
    ...
    for (const skill of enabledSkills) {
      ...
      }, withScopeEnforcement(toolName, async (args, extra) => {
        const scopeContext = resolveCallerScopeContext(extra, Array.from(MCP_ALLOWED_SCOPES));
        const apiKeyId = scopeContext.callerId || "mcp";
        try {
          const execution = await skillExecutor.execute(skill.name, args ?? {}, {
            apiKeyId
          });
          return {
            content: [{
