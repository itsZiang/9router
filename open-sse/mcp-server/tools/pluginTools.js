...
import { listPlugins, getPluginByName, updatePluginConfig } from "../../stubs/lib/db/plugins";
import { pluginManager } from "../../stubs/lib/plugins/manager";
import { validatePluginConfig } from "../../stubs/lib/plugins/manifest";
...
export const pluginTools = [{
  ...
  handler: async args => {
    try {
      ...
      return {
        success: true,
        message: `Plugin '${args.name}' activated`
      };
    } catch (err) {
  ...
  handler: async args => {
    try {
      ...
      return {
        success: true,
        message: `Plugin '${args.name}' deactivated`
      };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      return {
  ...
  handler: async args => {
    try {
      ...
      return {
        success: true,
        message: `Plugin '${args.name}' uninstalled`
      };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      return {
  ...
  handler: async args => {
    ...
    } = await import("../../stubs/lib/db/plugins");
