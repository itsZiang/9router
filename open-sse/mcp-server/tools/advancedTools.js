...
const TASK_FITNESS = {
  coding: {
    preferred: ["claude", "deepseek", "codex"],
    traits: ["fast", "code-optimized"]
  },
  review: {
  ...
  planning: {
    preferred: ["gemini", "claude", "openai"],
    traits: ["reasoning", "structured"]
  },
  analysis: {
    preferred: ["gemini", "claude"],
    traits: ["deep-reasoning", "large-context"]
  },
  debugging: {
    preferred: ["claude", "deepseek", "codex"],
    traits: ["code-aware", "fast"]
  },
  documentation: {
    preferred: ["gemini", "claude", "openai"],
...
export async function handleSimulateRoute(args) {
  ...
  try {
    ...
    const [combosRaw, healthRaw, quotaRaw] = await Promise.allSettled([apiFetch("/api/combos"), apiFetch("/api/monitoring/health"), apiFetch("/api/usage/quota")]);
    ...
    // Find target combo
    const targetCombo = args.combo ? combos.find(combo => toString(combo.id) === args.combo || toString(combo.name) === args.combo) : combos.find(combo => combo.enabled !== false);
    if (!targetCombo) {
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
            error: "No matching combo found"
          })
        }],
        isError: true
      };
...
export async function handleSetBudgetGuard(args) {
  ...
  try {
    ...
    await logToolCall("omniroute_set_budget_guard", {
      maxCost: args.maxCost,
      action: args.action
    }, result, Date.now() - start, true);
    return {
      content: [{
        type: "text",
        text: JSON.stringify(result, null, 2)
...
export async function handleSetRoutingStrategy(args) {
  ...
  try {
    ...
    const combo = combos.find(comboEntry => toString(comboEntry.id) === args.comboId || toString(comboEntry.name) === args.comboId);
    if (!combo) {
    ...
    const currentConfig = toRecord(Object.keys(toRecord(combo.config)).length > 0 ? combo.config : comboData.config);
    ...
    const updatedCombo = toRecord(await apiFetch(`/api/combos/${encodeURIComponent(comboId)}`, {
      method: "PUT",
  ...
  try {
    ...
    if (!settings) {
      return {
        content: [{
          type: "text",
          text: `Error: Invalid profile "${args.profile}"`
        }],
        isError: true
      };
    }
...
export async function handleTestCombo(args) {
  const start = Date.now();
  try {
    // Get combo details
    const combos = normalizeCombosResponse(await apiFetch("/api/combos"));
    ...
    if (!combo) {
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
            error: `Combo "${args.comboId}" not found`
          })
        }],
        isError: true
      };
    ...
    await logToolCall("omniroute_test_combo", {
      comboId: args.comboId
    }, result.summary, Date.now() - start, true);
    return {
      content: [{
        type: "text",
        text: JSON.stringify(result, null, 2)
...
export async function handleGetProviderMetrics(args) {
  const start = Date.now();
  try {
    const [healthRaw, quotaRaw, analyticsRaw] = await Promise.allSettled([apiFetch("/api/monitoring/health"), apiFetch(`/api/usage/quota?provider=${encodeURIComponent(args.provider)}`), apiFetch(`/api/usage/analytics?period=session&provider=${encodeURIComponent(args.provider)}`)]);
    const health = healthRaw.status === "fulfilled" ? toRecord(healthRaw.value) : {};
    const quota = quotaRaw.status === "fulfilled" ? normalizeQuotaResponse(quotaRaw.value, {
      provider: args.provider
    }) : normalizeQuotaResponse({});
    const analytics = analyticsRaw.status === "fulfilled" ? toRecord(analyticsRaw.value) : {};
    const cb = toArrayOfRecords(health.circuitBreakers).find(breaker => toString(breaker.provider) === args.provider);
    ...
    if (enabledCombos.length === 0) {
      return {
        content: [{
          type: "text",
          text: JSON.stringify({
            error: "No enabled combos available"
          })
        }],
        isError: true
      };
...
export async function handleExplainRoute(args) {
  const start = Date.now();
  try {
    ...
    await logToolCall("omniroute_explain_route", args, {
      requestId: args.requestId
    }, Date.now() - start, true);
    return {
      content: [{
...
export async function handleSyncPricing(args) {
  const start = Date.now();
  try {
...
export async function handleGetSessionSnapshot() {
  ...
  try {
    ...
    await logToolCall("omniroute_get_session_snapshot", {}, {
...
export async function handleDbHealthCheck(args) {
  const start = Date.now();
  const autoRepair = args.autoRepair === true;
  try {
    const {
      runManagedDbHealthCheck
    } = await import("../../stubs/lib/db/core");
    const result = runManagedDbHealthCheck({
      autoRepair
    });
    await logToolCall("omniroute_db_health_check", args, {
...
export async function handleOneproxyFetch(args = {}) {
  const start = Date.now();
  try {
    const params = new URLSearchParams();
...
export async function handleOneproxyRotate(args = {}) {
  const start = Date.now();
  try {
    const body = {};
    if (args.strategy) body.strategy = args.strategy;
    const raw = toRecord(await apiFetch("/api/settings/oneproxy/rotate", {
      method: "POST",
...
export async function handleOneproxyStats(args = {}) {
  const start = Date.now();
  try {
