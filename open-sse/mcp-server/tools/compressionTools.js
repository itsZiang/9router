...
import { getCompressionSettings, updateCompressionSettings } from "../../stubs/lib/db/compression";
import { getCompressionAnalyticsSummary } from "../../stubs/lib/db/compressionAnalytics";
import { getCacheStatsSummary } from "../../stubs/lib/db/compressionCacheStats";
import { listCompressionCombos } from "../../stubs/lib/db/compressionCombos";
import { getMcpDescriptionCompressionStats, snapshotMcpDescriptionCompressionStats } from "../descriptionCompressor";

/**
 * Handle compression_status tool: return current compression config, analytics, and cache stats
 */
export async function handleCompressionStatus(args, extra) {
  ...
  try {
    ...
    const result = {
      ...
      analytics: {
        ...
        compressedRequests: Object.values(analyticsSummary.byMode ?? {}).reduce((sum, mode) => sum + mode.count, 0),
    ...
    await logToolCall("omniroute_compression_status", args, {
      error: errorMessage
 ...
 */
export async function handleCompressionConfigure(args, extra) {
  const start = Date.now();
  try {
    const updates = {};
    ...
    const duration = Date.now() - start;
    const errorMessage = error instanceof Error ? error.message : String(error);
    await logToolCall("omniroute_compression_configure", args, {
      error: errorMessage
    }, duration, false, "ERROR");
    throw error;
  }
...
import { z } from "zod";
import { compressionStatusInput, compressionConfigureInput, setCompressionEngineInput, listCompressionCombosInput, compressionComboStatsInput } from "../schemas/tools";
import { handleCcrRetrieve } from "../../services/compression/engines/ccr/index";
import { listRtkCommandSamples, discoverRepeatedNoise, suggestFilter, commandToId } from "../../services/compression/engines/rtk/index";
import { resolveCallerScopeContext } from "../scopeEnforcement";
import { resolveMcpCallerApiKeyId } from "../mcpCallerIdentity";
...
export async function handleSetCompressionEngine(args) {
  const updates = {
    enabled: true
  };
  if (args.engine) {
...
export async function handleListCompressionCombos() {
  return {
    combos: listCompressionCombos()
  };
}
export async function handleCompressionComboStats(args) {
  ...
  return {
    ...
    summary,
    combo: summary.byCompressionCombo[args.comboId] ?? {
      count: 0,
      tokensSaved: 0
    }
  };
...
export async function handleRtkDiscover(args) {
  const start = Date.now();
  const samples = listRtkCommandSamples({
    limit: resolveSampleLimit(args.limit)
  });
...
export async function handleRtkLearn(args) {
  const start = Date.now();
  const command = args.command.trim();
  const targetId = commandToId(command);
  const matching = listRtkCommandSamples({
    limit: resolveSampleLimit(args.limit)
  }).filter(sample => commandToId(sample.command) === targetId);
...
export const compressionTools = {
  ...
  omniroute_ccr_retrieve: {
    ...
    handler: async (args, extra) => {
      ...
      const {
        callerId
      } = resolveCallerScopeContext(extra, ["read:compression"]);
