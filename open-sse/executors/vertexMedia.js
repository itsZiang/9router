...
import { parseSAFromApiKey, getAccessToken, looksLikeServiceAccountJson, isExpressApiKey } from "./vertex";
...
function buildModelRequest(auth, model, action) {
  ...
  if (auth.expressKey) {
    return {
      url: `https://aiplatform.googleapis.com/v1/publishers/google/models/${model}:${action}?key=${encodeURIComponent(auth.expressKey)}`,
      headers
    };
  }
  throw new Error("Vertex AI requires a Service Account JSON (with project_id) or a Vertex AI Express API key");
}
...
export async function vertexGenerateSpeech(credentials, options) {
  const auth = await resolveVertexAuth(credentials);
  const {
    url,
    headers
  } = buildModelRequest(auth, options.model, "generateContent");
  const payload = {
    contents: [{
      role: "user",
      parts: [{
  ...
  return {
    audio: pcmToWav(pcm, parseSampleRate(inline.mimeType)),
    contentType: "audio/wav"
  };
}

/** Gemini transcription (audio → text). `audioBase64` is the raw file bytes, base64-encoded. */
export async function vertexTranscribe(credentials, options) {
  const auth = await resolveVertexAuth(credentials);
  const {
    url,
    headers
...
export async function vertexGenerateMusic(credentials, options) {
  const auth = await resolveVertexAuth(credentials);
  const model = options.model && options.model.trim() ? options.model.trim() : "lyria-002";
  const {
    url,
    headers
  } = buildModelRequest(auth, model, "predict");
  const instance = {
...
export async function vertexGenerateVideo(credentials, options) {
  const auth = await resolveVertexAuth(credentials);
