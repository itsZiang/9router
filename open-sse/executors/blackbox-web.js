 ...
 */
export function resolveBlackboxValidatedToken() {
...
export function normalizeBlackboxCookieHeader(apiKey) {
...
export class BlackboxWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    if (cached && Date.now() - cached.fetchedAt < SESSION_CACHE_TTL_MS) {
      ...
      const sideSignal = signal ? mergeAbortSignals(signal, AbortSignal.timeout(10_000)) : AbortSignal.timeout(10_000);
      try {
    ...
    if (hasTools) {
      const {
        ...
        toolCalls,
        finishReason
      } = buildToolAwareResult(responseText, requestedTools, "bbx");
      if (toolCalls) {
      ...
      const finalResponse = stream ? new Response(buildStreamingResponse(content, model, id, created), {
        status: 200,
        headers: {
