...
function buildMetadata(apiKey, sessionId) {
  return concatBytes([encodeString(1, apiKey), encodeString(2, WS_IDE_NAME), encodeString(3, WS_IDE_VERSION), encodeString(4, WS_EXT_VERSION), encodeString(5, sessionId), encodeString(6, WS_LOCALE)]);
}
...
function buildGetChatMessageRequest(apiKey, model, messages) {
  ...
  const parts = [encodeMessage(1, buildMetadata(apiKey, sessionId)),
  // metadata
  ...
  while (offset < buf.length) {
    ...
    if (wireType === 2) {
      ...
      if (fieldNum === 1) {
        ...
        return {
          kind: "error",
          message: msg ?? "unknown windsurf error"
        };
      }
...
function decodeDoneChunk(buf) {
  ...
  while (offset < usageBytes.length) {
    ...
    if (wireType === 0) {
      ...
      if (fieldNum === 1) promptTokens = v;else if (fieldNum === 2) completionTokens = v;
    } else if (wireType === 2) {
      let len;
    ...
    const sseStream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          emit(`data: ${JSON.stringify({
            error: {
              message: `Windsurf stream error: ${msg}`,
              type: "windsurf_error"
            }
          })}\n\n`);
          emit("data: [DONE]\n\n");
