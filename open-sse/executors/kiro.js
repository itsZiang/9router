...
import { splitInlineThinking, flushPendingThinking } from "./kiroThinking";
class ByteQueue {
  chunks = [];
  headOffset = 0;
  length = 0;
...
function ensureKiroUsage(state) {
  ...
  const estimatedOutputTokens = state.totalContentLength && state.totalContentLength > 0 ? Math.max(1, Math.floor(state.totalContentLength / 4)) : 0;
  const estimatedInputTokens = state.contextUsagePercentage && state.contextUsagePercentage > 0 ? Math.floor(state.contextUsagePercentage * 200000 / 100) : 0;
 ...
 */
export function kiroRuntimeHost(region) {
...
export class KiroExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    const userContent = tb?.conversationState?.currentMessage?.userInputMessage?.content || "";
  ...
  transformEventStreamToSSE(response, model, opts = {}) {
    ...
    const transformStream = new TransformStream({
        ...
        while (buffer.length >= 16 && iterations < maxIterations) {
          ...
          if (eventType === "assistantResponseEvent") {
            ...
            if (thinkingExpected && state.thinking) {
              ...
              splitInlineThinking(thinkingState, content, text => {
                if (!text) return;
                const chunk = {
                  id: responseId,
                  object: "chat.completion.chunk",
                ...
                const reasoningDelta = (state.reasoningChunkCount ?? 0) === 0 && chunkIndex === 0 ? {
                  role: "assistant",
          ...
          if (eventType === "contextUsageEvent") {
            const contextUsage = typeof event.payload?.contextUsagePercentage === "number" ? event.payload.contextUsagePercentage : 0;
            if (contextUsage <= 0) {
              continue;
            }
      ...
      flush(controller) {
        ...
        if (thinkingExpected && state.thinking) {
          const thinkingState = state.thinking;
          flushPendingThinking(thinkingState, text => {
            if (!text) return;
            const chunk = {
              id: responseId,
              object: "chat.completion.chunk",
  ...
  try {
    ...
    if (payloadEnd > payloadStart) {
      ...
      try {
        ...
        console.warn(`[Kiro] Failed to parse payload: ${err.message} | payload: ${payloadStr.substring(0, 100)}`);
        payload = {
          raw: payloadStr
        };
      }
