...
import { buildBedrockNativeConverseUrl, resolveBedrockRegion } from "../config/bedrock";
...
function toolResultContentFromMessage(message) {
  ...
  if (Array.isArray(content)) {
    ...
    for (const part of content) {
      ...
      const p = asRecord(part);
      if (typeof p.text === "string") result.push({
        text: p.text || " "
      });else if (p.type === "json" && p.json !== undefined) result.push({
        json: p.json
      });else if (p.content !== undefined) result.push({
        text: toText(p.content)
      });
    }
...
function sanitizeBedrockToolPairs(messages) {
  ...
  for (let i = 0; i < normalized.length; i++) {
    ...
    const nextResultIds = new Set(nextMessage?.role === "user" && Array.isArray(nextMessage.content) ? nextMessage.content.map(getToolResultIdFromBlock).filter(Boolean) : []);
...
function messagesFromOpenAI(messages) {
  ...
  for (const message of messages) {
    ...
    const toolCallIds = new Set(toolCalls.map(call => normalizeToolUseId(call?.id)).filter(Boolean));
...
function toolConfigFromOpenAI(tools, toolChoice) {
  ...
  if (toolChoice === "required") config.toolChoice = {
    any: {}
  };else if (toolChoice === "auto") config.toolChoice = {
    auto: {}
  };else if (toolChoice && typeof toolChoice === "object") {
    const fn = asRecord(toolChoice.function);
    const name = typeof fn.name === "string" ? fn.name : "";
    if (name) config.toolChoice = {
      tool: {
        name
      }
    };
...
export function openAIToBedrockConverse(model, body) {
  ...
  if (Array.isArray(request.stop)) inferenceConfig.stopSequences = request.stop.filter(Boolean);else if (typeof request.stop === "string" && request.stop) inferenceConfig.stopSequences = [request.stop];
  const payload = {
    modelId: model,
...
function streamExceptionPayload(event) {
  const candidates = [event?.throttlingException, event?.validationException, event?.modelStreamErrorException, event?.serviceUnavailableException, event?.internalServerException].filter(Boolean);
...
function createOpenAIStreamFromBedrock(stream, model) {
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        controller.enqueue(sse(openAIChunk(model, {
          role: "assistant"
        })));
...
export class BedrockExecutor extends BaseExecutor {
  ...
  buildUrl(model, stream, _urlIndex = 0, credentials = null) {
    return buildBedrockNativeConverseUrl(resolveBedrockRegion(credentials?.providerSpecificData), model, stream);
  }
  ...
  async execute({
    ...
    try {
      ...
      await prl.captureCurrentProviderRequest(url, headers, transformedBody, JSON.stringify(transformedBody), log);
      if (stream) {
