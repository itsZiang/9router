...
import { buildPrompt, buildWsUrl, redactWsUrl, resolveConnectionParams } from "./copilot-m365-connection";
import { buildChatInvocation, encodeFrame, extractBotText, handshakeError, handshakeFrame, incrementalDelta, isCompletionFrame, keepaliveFrame, parseFrame, splitFrames } from "./copilot-m365-frames";
...
export class CopilotM365WebExecutor extends BaseExecutor {
  constructor() {
    super("copilot-m365-web", {
      id: "copilot-m365-web",
      baseUrl: "wss://substrate.office.com"
    });
  }
  async wsChat(input) {
    return new ReadableStream({
      start: async controller => {
        ...
        input.signal?.addEventListener("abort", () => abort("Request aborted"), {
          once: true
        });
        const timeout = setTimeout(() => abort("Microsoft 365 Copilot WebSocket timeout"), FETCH_TIMEOUT_MS);
        try {
          ...
          ws.on("message", data => {
            ...
            for (const rawFrame of split.frames) {
              ...
              if (text) {
                ...
                previousText = text;
                if (delta) {
                  controller.enqueue(encoder.encode(sseChunk(input.model, {
                    content: delta
                  })));
                }
              }
          ...
          ws.on("error", err => {
            clearTimeout(timeout);
            abort(sanitizeErrorMessage(err instanceof Error ? err.message : "Microsoft 365 Copilot WebSocket error"));
          });
          ws.on("close", () => {
            clearTimeout(timeout);
            finish();
          ...
          clearTimeout(timeout);
          abort(sanitizeErrorMessage(err instanceof Error ? err.message : "Failed to connect to Microsoft 365 Copilot"));
        }
      }
    }, {
      highWaterMark: 16384
  ...
  async execute(input) {
    ...
    try {
      ...
      const message = sanitizeErrorMessage(err instanceof Error ? err.message : "Microsoft 365 Copilot executor error");
      return {
