...
export class CopilotWebExecutor extends BaseExecutor {
  ...
  async getSession(accessToken, signal) {
    ...
    if (existing && !existing.isBlocked && existing.remainingTurns > MIN_REMAINING_TURNS && Date.now() - existing.createdAt < 3_600_000 // 1 hour max session age
    ) {
      return existing;
    }
  ...
  async wsChat(conversationId, prompt, mode, accessToken, signal) {
    ...
    return new ReadableStream({
      start: async controller => {
        ...
        signal?.addEventListener("abort", () => abort("Request aborted"), {
          once: true
        });
        try {
  ...
  async execute(input) {
    ...
    try {
      ...
      const msg = sanitizeErrorMessage(err instanceof Error ? err.message : "Failed to start Copilot conversation");
      return {
        response: new Response(JSON.stringify({
          error: {
            message: msg
    ...
    if (!stream) {
      try {
        const wsStream = await this.wsChat(conversationId, fullPrompt, mode, accessToken || undefined, signal);
