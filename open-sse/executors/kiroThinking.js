...
export function splitInlineThinking(state, raw, onContent, onReasoning) {
  ...
  while (text.length > 0) {
    ...
    if (idx === -1) {
      ...
      if (flushable) {
        if (state.thinkingMode) onReasoning(flushable);else onContent(flushable);
      }
    ...
    if (before) {
      if (state.thinkingMode) onReasoning(before);else onContent(before);
    }
...
export function flushPendingThinking(state, onContent, onReasoning) {
  ...
  state.pendingTag = "";
  if (state.thinkingMode) onReasoning(leftover);else onContent(leftover);
}