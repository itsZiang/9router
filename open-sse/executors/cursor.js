...
import { PROVIDERS, HTTP_STATUS } from "../config/constants";
import { buildAgentRequestBody, decodeAgentServerMessage, decodeExecServerEvent, decodeKvServerEvent, encodeRequestContextResponse, encodeKvGetBlobResult, encodeKvSetBlobResult, encodeExecReadRejected, encodeExecWriteRejected, encodeExecDeleteRejected, encodeExecLsRejected, encodeExecShellRejected, encodeExecBackgroundShellSpawnRejected, encodeExecGrepError, encodeExecFetchError, encodeExecWriteShellStdinError, encodeExecDiagnosticsResult, flattenMessages, openAIToolsToMcpDefs } from "../utils/cursorAgentProtobuf";
import { resolveCursorImages, extractImageUrls, CursorImageError } from "../utils/cursorImages";
import { estimateInputTokens, estimateOutputTokens, addBufferToUsage } from "../utils/usageTracking";
import { getCursorVersion } from "../utils/cursorVersionDetector";
import { sanitizeErrorMessage } from "../utils/error";
import { generateToolCallId } from "../translator/helpers/toolCallHelper";
import { parseComposerToolCalls, createStreamingState, feedStreamingChunk } from "../utils/composerToolCalls";
import { cursorSessionManager } from "../services/cursorSessionManager";
...
const BUILTIN_TOOL_REJECT_REASON = "Tool not available in this environment. Use the MCP tools provided instead.";
 ...
 */
function toolChoiceDirectiveLine(toolChoice) {
  if (toolChoice === "required") {
    return "\nYou MUST call at least one of the available tools now; do not answer without calling a tool.";
  }
  if (isRecordLike(toolChoice) && toolChoice.type === "function" && isRecordLike(toolChoice.function) && typeof toolChoice.function.name === "string" && toolChoice.function.name) {
...
function buildCursorOutputConstraints(body) {
  ...
  if (isRecordLike(fmt)) {
    if (fmt.type === "json_object") {
      ...
      constraints.push(`Return only valid JSON (no prose or code fences) matching this schema: ${JSON.stringify(js ?? fmt)}`);
    }
  }
 ...
 */
function buildExecRejection(event) {
  switch (event.kind) {
...
function tryParseJsonError(payload) {
  ...
  try {
    ...
    const message = err?.details?.[0]?.debug?.details?.title || err?.details?.[0]?.debug?.details?.detail || err?.message || text;
    const status = err?.code === "resource_exhausted" ? HTTP_STATUS.RATE_LIMITED : HTTP_STATUS.BAD_REQUEST;
    return {
      message,
      status
    };
...
export function isComposerModel(model) {
...
export function visibleComposerContentFromThinking(thinking) {
  if (!thinking) return "";
...
export function composerReasoningRemainder(thinking) {
  if (!thinking) return "";
  const endIdx = thinking.lastIndexOf(COMPOSER_THINK_END);
...
export function newStreamCtx(model, emit) {
  return {
...
function emitChunk(ctx, delta, finishReason = null) {
  const payload = {
    id: ctx.responseId,
...
export function buildCursorUsage(ctx, body) {
  const promptTokens = estimateInputTokens(body);
  const completionTokens = ctx.tokenDelta > 0 ? ctx.tokenDelta : estimateOutputTokens(ctx.totalText.length + ctx.thinkingText.length);
  const usage = {
 ...
 */
export function processFrame(payload, ctx, ackedExecIds, opts = {}) {
  ...
  try {
    ...
    debugLog("[cursor-agent] decode failed:", err.message);
    return;
  }
...
export class CursorExecutor extends BaseExecutor {
   ...
   */
  assembleTextAndTools(body) {
    const messages = body.messages || [];
    const declaredTools = Array.isArray(body.tools) ? body.tools : undefined;
   ...
   */
  async resolveRequestImages(body) {
    const messages = body.messages || [];
    const imageUrls = [];
  ...
  async buildRequest(model, body) {
    const {
      userText,
      tools
    } = this.assembleTextAndTools(body);
  ...
  async openH2(url, headers, body, signal) {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.on("response", h => {
        ...
        const consumeError = () => new Promise(res => {
   ...
   */
  driveH2(h2, ctx, mcpTools, blobStore, signal) {
    const ackedExecIds = new Set();
    ...
    let buf = h2.initialBytes.length > 0 ? h2.initialBytes : Buffer.alloc(0);
    return new Promise((resolve, reject) => {
      let scanning = false;
      let settled = false;
        ...
        try {
          ...
          while (!settled && pos + 5 <= buf.length) {
            ...
            try {
              ...
              debugLog("[cursor-agent] frame decode failed at pos", pos, ":", err.message);
            }
            pos += 5 + length;
            if (ctx.endReason) {
  ...
  async execute({
    ...
    const mcpTools = Array.isArray(body.tools) ? openAIToolsToMcpDefs(body.tools) : undefined;
    ...
    const buildErrorResponse = (status, message, type = "invalid_request_error") => new Response(JSON.stringify({
      error: {
    ...
    if (!http2) {
      return {
        response: buildErrorResponse(501, "Cursor provider requires Node.js http2, which is unavailable in this runtime (Edge / Cloudflare Workers / similar). Run OmniRoute on a Node.js runtime to use cursor.", "unsupported_runtime"),
        url,
        headers,
    ...
    const finishLifecycle = (ctx, errored) => {
    ...
    if (ctx.midStreamError && ctx.totalText.length === 0) {
      const payload = {
        ...
        error: {
          ...
          type: ctx.midStreamError.status === HTTP_STATUS.RATE_LIMITED ? "rate_limit_error" : "api_error"
        }
      };
      ctx.emit(`data: ${JSON.stringify(payload)}\n\n`);
      ctx.emit("data: [DONE]\n\n");
  ...
  buildResponseFromCtx(ctx, body) {
    ...
    if (ctx.thinkingText.length > 0) {
      ...
      const reasoningPayload = isComposerModel(ctx.model) ? composerReasoningRemainder(ctx.thinkingText) : ctx.thinkingText;
