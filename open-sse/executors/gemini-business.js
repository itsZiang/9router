...
export class GeminiBusinessExecutor extends BaseExecutor {
  ...
  async execute(input) {
    ...
    const psid = readProviderSpecificString(credentials?.providerSpecificData, ["__Secure-1PSID", "cookie"]);
    const psidts = readProviderSpecificString(credentials?.providerSpecificData, ["__Secure-1PSIDTS"]);
    const cookie = directCookie || [psid, psidts].filter(Boolean).join("; ");
    if (!cookie) {
      return makeErrorResult(401, "Missing Gemini Business cookies. Set __Secure-1PSID and __Secure-1PSIDTS from your enterprise account (business.gemini.google).", body, DEFAULT_ENTRY_URL);
    }
    ...
    try {
      ...
      return makeErrorResult(isTimeout ? 504 : 502, `Gemini Business ${isTimeout ? "request timed out" : "network error"}: ${message}`, body, streamUrl);
    }
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      return makeErrorResult(response.status, `Gemini Business returned HTTP ${response.status}: ${text.slice(0, 200)}`, body, streamUrl);
    }
    const rawText = await response.text();
    if (rawText.includes("auth.business.gemini.google/account-chooser")) {
      return makeErrorResult(403, "Gemini Business account-chooser detected. Your enterprise cookies may be stale or the entry URL is wrong. Re-extract __Secure-1PSID/PSIDTS from business.gemini.google/home/cid/{YOUR-CID} after signing in.", body, streamUrl);
    }
    const text = parseStreamResponse(rawText);
    if (!text) {
      return makeErrorResult(502, "Gemini Business returned no text. The cookie may be expired or the entry URL is wrong.", body, streamUrl);
    }
    if (wantStream) {
      return {
