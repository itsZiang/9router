...
function flattenQuery(messages) {
  ...
  for (const m of messages) {
    let content = "";
    if (typeof m.content === "string") content = m.content;else if (Array.isArray(m.content)) {
...
export class TraeExecutor extends BaseExecutor {
  ...
  async createSession(headers, query, model, psd, signal) {
    ...
    return {
      sessionId: json.data.chat_session_id,
    ...
    try {
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          buf = buf.slice(nl + 1);
          if (line.startsWith("event:")) ev = line.slice(6).trim();else if (line.startsWith("data:")) {
  ...
  async execute({
    ...
    const errResponse = (status, message) => new Response(JSON.stringify({
      error: {
