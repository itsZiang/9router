...
import { getGitHubCopilotChatHeaders, getGitHubCopilotRefreshHeaders } from "../config/providerHeaderProfiles";
...
export class GithubExecutor extends BaseExecutor {
  ...
  getCopilotTokenExpiresAt(credentials) {
    return credentials?.copilotTokenExpiresAt || credentials?.providerSpecificData?.copilotTokenExpiresAt || null;
  }
  ...
  buildUrl(model, _stream, _urlIndex = 0) {
    ...
    if ((targetFormat === "openai-responses" || /codex/i.test(model)) && this.supportsResponsesEndpoint(model)) {
      return this.config.responsesBaseUrl || this.config.baseUrl?.replace(/\/chat\/completions\/?$/, "/responses") || "https://api.githubcopilot.com/responses";
    }
    return this.config.baseUrl;
  }
  injectResponseFormat(messages, responseFormat) {
    if (!responseFormat) return messages;
    let formatInstruction = "";
    if (responseFormat.type === "json_object") {
      formatInstruction = "Respond only with valid JSON. Do not include any text before or after the JSON object.";
    ...
    if (systemIdx >= 0) {
      return messages.map((m, i) => i === systemIdx ? {
        ...m,
        content: `${m.content}\n\n${formatInstruction}`
      } : m);
    }
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    if (modifiedBody.response_format && model.toLowerCase().includes("claude")) {
      modifiedBody.messages = this.injectResponseFormat(Array.isArray(modifiedBody.messages) ? modifiedBody.messages : [], modifiedBody.response_format);
    ...
    if (typeof model === "string" && /gpt-5\.4/i.test(model) && modifiedBody.temperature !== undefined) {
    ...
    if (Array.isArray(modifiedBody.messages)) {
      modifiedBody.messages = modifiedBody.messages.map(msg => this.sanitizeChatCompletionsMessage(msg));
    }
  ...
  sanitizeChatCompletionsMessage(msg) {
    if (!msg || typeof msg !== "object") return msg;
    ...
    const cleanContent = msg.content.map(part => {
      ...
      const raw = typeof part.text === "string" && part.text || typeof part.thinking === "string" && part.thinking || typeof part.content === "string" && part.content || part.content != null && JSON.stringify(part.content) || JSON.stringify(part);
      return {
        type: "text",
  ...
  async execute(input) {
    ...
    if (!input.stream) {
      ...
      result.response = new Response(payload, {
        status,
        statusText,
        headers
      });
  ...
  buildHeaders(credentials, stream = true, clientHeaders) {
    ...
    const initiator = clientInitiator === "agent" || clientInitiator === "user" ? clientInitiator : "user";
    return {
