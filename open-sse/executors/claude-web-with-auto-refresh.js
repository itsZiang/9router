...
class ClaudeWebWithAutoRefresh extends ClaudeWebExecutor {
  ...
  async executeWithRetry(input) {
    ...
    if ((result.response.status === 403 || result.response.status === 401) && this.retryCount < this.maxRetries) {
      this.retryCount++;
      log?.warn?.("CLAUDE-WEB", `HTTP ${result.response.status} detected - attempt ${this.retryCount}/${this.maxRetries}`);
      try {
        // Get fresh cf_clearance
        const cacheStatus = getCacheStatus();
        const shouldForce = this.retryCount > 1;
        log?.info?.("CLAUDE-WEB", `Solving Turnstile (cache: ${cacheStatus.hasCached ? `${Math.round((cacheStatus.expiresIn || 0) / 1000)}s left` : "empty"})...`);
