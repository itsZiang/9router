...
import { buildGrokCookieHeader } from "../stubs/lib/providers/webCookieAuth";
import { tlsFetchGrok, TlsClientUnavailableError } from "../services/grokTlsClient";
import { sanitizeErrorMessage } from "../utils/error";

// ─── Constants ──────────────────────────────────────────────────────────────

const GROK_CHAT_API = "https://grok.com/rest/app-chat/conversations/new";
const GROK_USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36";
...
function stripInjectedRuntimeReminders(text) {
...
function extractTextContent(msg) {
  ...
  if (Array.isArray(msg.content)) {
    return stripInjectedRuntimeReminders(msg.content.filter(c => c.type === "text").map(c => String(c.text || "")).join(" "));
  }
  return "";
}
...
function semanticToolKey(name, args) {
...
function summarizeCompletedToolCall(name, args) {
  const record = normalizeToolArgumentObject(args);
  const command = firstString(record.command, record.cmd, record.shell);
...
function getExecutedToolState(messages) {
  let lastUserIdx = -1;
  for (let i = messages.length - 1; i >= 0; i--) {
  ...
  for (let i = Math.max(0, lastUserIdx + 1); i < messages.length; i++) {
    ...
    if (String(msg.role || "") === "assistant" && Array.isArray(msg.tool_calls)) {
      for (const call of msg.tool_calls) {
        ...
        callsById.set(id, {
          name,
          args: normalizeToolArgumentObject(fnRecord.arguments)
        });
      }
    }
...
function parseOpenAIMessages(messages, beforeLatestUser = "") {
  const parts = [];
  let lastUserIdx = -1;
  ...
  for (let msgIdx = 0; msgIdx < messages.length; msgIdx++) {
    ...
    if (role === "tool") {
      ...
      content = [content, `Previous assistant tool calls: ${JSON.stringify(calls)}`].filter(Boolean).join("\n");
    }
...
function buildGrokToolRegistry(body) {
...
function getSchemaProperties(parameters) {
  if (!parameters || typeof parameters !== "object") return {};
  const properties = parameters.properties;
  return properties && typeof properties === "object" ? properties : {};
}
function getSchemaRequired(parameters) {
  if (!parameters || typeof parameters !== "object") return [];
  const required = parameters.required;
  return Array.isArray(required) ? required.filter(key => typeof key === "string") : [];
}
function formatToolArgsSummary(parameters) {
...
function hasAnyProperty(tool, names) {
...
function isFileReadTool(tool) {
  const text = toolText(tool);
  return hasAnyProperty(tool, ["filePath", "file_path", "path"]) && /\b(read|file|filesystem|open)\b/.test(text) && !/\b(write|edit|patch|delete|remove|grep|search|bash|shell|command)\b/.test(text);
}
function isUrlFetchTool(tool) {
  const text = toolText(tool);
  const name = tool.name.toLowerCase();
...
function isWebSearchTool(tool) {
  const text = toolText(tool);
  return hasAnyProperty(tool, ["query", "search"]) && /\b(web|internet|exa|browser|browse|serp)\b/.test(text) && !isMetaOrInfrastructureTool(tool) && !isContextMemoryTool(tool);
}
function isContextMemoryTool(tool) {
  const text = toolText(tool);
...
function isMetaOrInfrastructureTool(tool) {
  const text = toolText(tool);
...
function baseToolOrderScore(tool) {
...
function latestUserIntentScore(tool, lastUserText) {
...
function orderedToolsForManifest(toolRegistry) {
  return [...toolRegistry.toolsByName.values()].map((tool, index) => ({
    tool,
    score: latestUserIntentScore(tool, toolRegistry.lastUserText) + baseToolOrderScore(tool),
    meta: isMetaOrInfrastructureTool(tool),
    index
  })).sort((a, b) => b.score - a.score || Number(a.meta) - Number(b.meta) || a.index - b.index).map(({
    tool,
    score
  }) => ({
    tool,
...
function formatToolManifestEntry(tool, rank) {
...
function buildClientToolManifest(toolRegistry, toolChoice) {
  if (!toolRegistry.enabled) return "";
  const orderedTools = orderedToolsForManifest(toolRegistry);
  const lines = ['CLIENT_TOOLS: use this caller-runtime tool list as the tool interface for this request. To call one, respond only with <tool_call>{"name":"exact_tool_name","arguments":{...}}</tool_call>. After tool results, answer normally.', `tool_choice=${JSON.stringify(toolChoice ?? "auto")}`, ...(toolRegistry.completedToolCalls.length > 0 ? ["completed_tool_calls:", ...toolRegistry.completedToolCalls.map(summary => `- ${summary}`), "Do not repeat completed tool calls unless a different result is required; use their tool results to answer."] : []), "tools (priority order for this request):", ...orderedTools.map(({
    tool
  }, index) => formatToolManifestEntry(tool, index + 1))];
  return lines.join("\n");
}
function buildGrokMessage(messages, toolRegistry, toolChoice) {
  const manifest = buildClientToolManifest(toolRegistry, toolChoice);
  return parseOpenAIMessages(messages, manifest);
}
function propertyType(properties, key) {
...
function defaultRequiredValue(key, type, args, intent) {
...
function extractNumericUserParam(text, names) {
...
function adaptArgumentsToDeclaredTool(toolName, args, toolRegistry, intent, options = {
...
function normalizeArbitraryToolArguments(value) {
  if (!value) return {};
  if (typeof value === "object") return value;
  if (typeof value === "string") {
    try {
...
function parseClientToolCallMarkup(text, toolRegistry) {
...
function pickDeclaredToolForIntent(intent, toolRegistry) {
  let best = null;
  for (const tool of toolRegistry.toolsByName.values()) {
    const score = toolScore(tool, intent, {
      lastUserText: toolRegistry.lastUserText
    });
    if (score <= 0) continue;
...
function mapGrokNativeToolToOpenAI(resp, toolRegistry) {
  if (!toolRegistry.enabled || !resp.toolUsageCard) return null;
  const card = resp.toolUsageCard;
  ...
  const readFile = card.readFile || card.read_file;
  if (readFile?.args) {
  ...
  const webSearch = card.webSearch;
  if (webSearch?.args) {
    ...
    if (name) {
      ...
      const args = adaptArgumentsToDeclaredTool(name, requestedUrl ? {
        ...webSearch.args,
        url: requestedUrl,
        uri: requestedUrl
      } : webSearch.args, toolRegistry, requestedUrl ? "webFetch" : "webSearch", {
        preserveUnknownArgs: false
      });
      if (toolRegistry.executedToolKeys.has(semanticToolKey(name, args))) return null;
      return {
  ...
  const browsePage = card.browsePage || card.browse_page;
  if (browsePage?.args) {
    ...
    if (name && url) {
      const args = adaptArgumentsToDeclaredTool(name, {
        ...browsePage.args,
        url,
        uri: url,
        input: url
      }, toolRegistry, "browsePage", {
        preserveUnknownArgs: false
      });
      if (toolRegistry.executedToolKeys.has(semanticToolKey(name, args))) return null;
      return {
...
function stripLooseGrokMarkup(text) {
  return text.replace(/<\/?xai:[^>]*>/g, "").replace(/<\/?grok:[^>]*>/g, "").replace(/<\/?argument\b[^>]*>/g, "").replace(/<!\[CDATA\[/g, "").replace(/\]\]>/g, "");
}
...
function cleanGrokContentText(text) {
  return cleanGrokText(text);
}
function cleanGrokThinkingText(resp) {
  const text = resp.token || "";
  const cleaned = cleanGrokText(text);
  const trimmed = cleaned.trim();
  if (!trimmed) return "";
  const isGenericOpeningHeader = resp.messageTag === "header" && resp.messageStepId === 0 && /^(?:\.{3}|thinking(?: about your request)?)$/i.test(trimmed);
  if (isGenericOpeningHeader) return "";
  if (resp.messageTag === "header") return `${trimmed}\n`;
  if (resp.messageTag === "summary") return `${trimmed}\n`;
  return cleaned;
}
function extractStructuredReasoning(value) {
  if (!value) return "";
  const record = value;
...
async function* extractContent(eventStream, isThinkingModel, toolRegistry, signal, suppressThinkingAfterVisibleContent = false) {
  let fingerprint = "";
  let responseId = "";
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    ...
    if (resp.modelResponse) {
      ...
      if ((!suppressThinkingAfterVisibleContent || !emittedVisibleContent) && finalThinking) {
        ...
        const thinkingDelta = cleanedThinking.startsWith(emittedThinking) ? cleanedThinking.slice(emittedThinking.length) : cleanedThinking;
        if (thinkingDelta) {
    ...
    if (resp.token != null) {
      if (resp.isThinking) {
        const thinkingDelta = suppressThinkingAfterVisibleContent && emittedVisibleContent ? "" : cleanGrokThinkingText(resp);
        if (thinkingDelta) yield {
          thinking: thinkingDelta,
          fingerprint,
          responseId
        };
  ...
  const trailingThinking = suppressThinkingAfterVisibleContent && emittedVisibleContent ? "" : thinkingFilter.flush();
  if (trailingThinking) {
    const thinkingDelta = trailingThinking.startsWith(emittedThinking) ? trailingThinking.slice(emittedThinking.length) : trailingThinking;
    if (thinkingDelta) yield {
      thinking: thinkingDelta,
      fingerprint,
      responseId
  ...
  if (trailingContentWithTrace) yield {
    delta: trailingContentWithTrace,
    fingerprint,
    responseId
  };
  yield {
...
function enqueueStreamingToolCalls(controller, encoder, params) {
...
function buildStreamingResponse(eventStream, model, cid, created, isThinkingModel, toolRegistry, signal) {
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        ...
        let fp = "";
        let buffered = "";
        for await (const chunk of extractContent(eventStream, isThinkingModel, toolRegistry, signal, true)) {
        ...
        controller.enqueue(encoder.encode(sseChunk({
          ...
          choices: [{
            index: 0,
            delta: {
              content: sanitizeErrorMessage(`[Stream error: ${err instanceof Error ? err.message : String(err)}]`)
            },
            finish_reason: "stop",
            logprobs: null
          }]
...
async function buildNonStreamingResponse(eventStream, model, cid, created, isThinkingModel, toolRegistry, signal) {
  let fullContent = "";
  let fingerprint = "";
  const thinkingParts = [];
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    const toolRegistry = buildGrokToolRegistry(body);
    const {
      modeId,
      isThinking
    } = modelInfo || MODEL_MAP.fast;

    // Parse OpenAI messages → single Grok message string
    const message = buildGrokMessage(messages, toolRegistry, body.tool_choice);
    if (!message.trim()) {
      const errResp = new Response(JSON.stringify({
        error: {
    ...
    try {
      ...
      const errResp = new Response(JSON.stringify({
        error: {
          message: sanitizeErrorMessage(`Grok connection failed: ${err instanceof Error ? err.message : String(err)}`),
          type: "upstream_error"
        }
      }), {
        status: 502,
    ...
    let finalResponse;
    if (stream) {
      const sseStream = buildStreamingResponse(tlsResult.body, model, cid, created, isThinking, toolRegistry, signal);
      ...
      finalResponse = await buildNonStreamingResponse(tlsResult.body, model, cid, created, isThinking, toolRegistry, signal);
    }
    return {
