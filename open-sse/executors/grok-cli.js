...
export class GrokCliExecutor extends BaseExecutor {
  ...
  async refreshCredentials(credentials, log) {
    ...
    try {
      ...
      const result = await this.nativeHttpsPost(GROK_TOKEN_URL, {
        "Content-Type": "application/x-www-form-urlencoded"
      }, body.toString(), 10_000);
      ...
      log?.warn?.("TOKEN_REFRESH", `Grok Build: refresh error: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  nativeHttpsPost(url, headers, bodyStr, timeoutMs = 10_000) {
  ...
  nativePost(url, headers, bodyStr, signal) {
    const urlObj = new URL(url);
    if (signal?.aborted) {
        ...
        res.on("end", () => {
          settle(() => {
            ...
            for (const [key, value] of Object.entries(res.headers)) {
              if (typeof value === "string") responseHeaders[key] = value;else if (Array.isArray(value)) responseHeaders[key] = value.join(", ");
            }
  ...
  transformRequest(model, body, stream, _credentials) {
    const transformed = body && typeof body === "object" ? {
      ...body
    } : {};
