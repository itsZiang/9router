...
import { makeExecutorErrorResult as makeErrorResult, sanitizeErrorMessage } from "../utils/error";
import { extractKimiJwt } from "../stubs/lib/providers/webCookieAuth";
...
export function decodeConnectFrame(buf, byteOffset) {
  if (byteOffset + 5 > buf.length) return {
    consumed: 0,
    frame: null
  };
  ...
  if (msgLen > MAX_FRAME_LEN) return {
    consumed: -1,
    frame: null
  };
  if (byteOffset + 5 + msgLen > buf.length) return {
    consumed: 0,
    frame: null
  };
...
export class KimiWebExecutor extends BaseExecutor {
  ...
  async execute(input) {
    ...
    if (!jwt) {
      return makeErrorResult(400, "Missing Kimi session — paste the full Cookie header from www.kimi.com (must contain kimi-auth=<JWT>) or just the JWT itself.", body, CHAT_URL);
    }
    ...
    try {
      ...
      return makeErrorResult(502, `Kimi fetch failed: ${err instanceof Error ? err.message : "unknown"}`, body, CHAT_URL);
    }
    if (!upstream.ok) {
    ...
    const sourceStream = upstream.body ?? new ReadableStream({
      start: c => c.close()
    });
    if (wantStream) {
              ...
              if (value) {
                ...
                while (offset < buffer.length) {
                  const {
                    consumed,
                    frame
                  } = decodeConnectFrame(buffer, offset);
                  if (consumed === -1) {
    ...
    try {
      while (true) {
        ...
        while (offset < buffer.length) {
          ...
          const delta = extractDelta(frame.message);
          if (delta) {
            if (delta.kind === "think") reasoning += delta.text;else answer += delta.text;
          }
          if (isEndOfStream(frame.message)) {
