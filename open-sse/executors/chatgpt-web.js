...
import { describeChatGptWebHttpError } from "./chatgptWebErrors";
import { prepareToolMessages } from "../translator/webTools";
import { buildToolModeResponse } from "./chatgptWebTools";
import { createHash, randomUUID, randomBytes } from "node:crypto";
import { sha3_512Hex } from "../utils/sha3-512";
import { tlsFetchChatGpt, TlsClientUnavailableError } from "../services/chatgptTlsClient";
import { storeChatGptImage, getChatGptImageConversationContext, __resetChatGptImageCacheForTesting } from "../services/chatgptImageCache";
...
const THINKING_CAPABLE_SLUGS = new Set(Object.entries(MODEL_MAP).filter(([k]) => k.includes("thinking") || k === "o3").map(([, v]) => v));
...
async function runSessionWarmup(accessToken, accountId, sessionId, deviceId, cookie, signal, log) {
  ...
  const urls = [`${CHATGPT_BASE}/backend-api/me`, `${CHATGPT_BASE}/backend-api/conversations?offset=0&limit=28&order=updated`, `${CHATGPT_BASE}/backend-api/models?history_and_training_disabled=false`];
  for (const url of urls) {
    try {
      ...
      log?.debug?.("CGPT-WEB", `warmup ${url} failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
...
const thinkingEffortCache = new Map();
...
async function setUserThinkingEffort(modelSlug, effort, accessToken, accountId, sessionId, deviceId, cookie, signal, log) {
  ...
  try {
    ...
    if (r.status >= 400) {
      log?.warn?.("CGPT-WEB", `thinking_effort PATCH ${r.status} for ${modelSlug}=${effort} (continuing)`);
      return;
    }
    thinkingEffortCache.set(cacheKey, now);
    log?.debug?.("CGPT-WEB", `thinking_effort PATCH OK (${modelSlug}=${effort})`);
  } catch (err) {
    log?.warn?.("CGPT-WEB", `thinking_effort PATCH failed: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
function buildPrekeyConfig(userAgent, dpl, scriptSrc) {
  ...
  return [pick(screenSizes), dateStr, 4294705152, 0,
  // mutated by solver
  userAgent, scriptSrc, dpl, "en-US", "en-US,en", 0,
  // mutated by solver
  pick(NAVIGATOR_KEYS), pick(DOCUMENT_KEYS), pick(WINDOW_KEYS), perfNow, randomUUID(), "", pick(cores), epochOffset];
}
...
async function solvePow(opts) {
  ...
  opts.log?.warn?.("CGPT-WEB", `PoW (${opts.label}) exhausted ${opts.maxIter} iterations against target=${opts.target || "<empty>"}; submitting unsolved token (Sentinel may reject)`);
...
function stripInlinedImages(content) {
  return content.replace(DATA_URI_IMAGE_RE, (_, alt) => alt ? `[${alt}: generated image]` : "[generated image]");
}
 ...
 */
const OPENWEBUI_TOOL_PROMPT_MARKERS = [/<chat_history>/i, /^### Task:/im, /\bJSON format:\s*\{/i, /\bfollow_?ups\b.*\barray of strings\b/i];
const OPENWEBUI_IMAGE_CONTEXT_MARKERS = [/<context>\s*The requested image has been (?:created|edited and created) by the system successfully/i, /<context>\s*The requested image has been edited and created and is now being shown to the user/i, /<context>\s*Image generation was attempted but failed/i];
  ...
  for await (const event of readChatGptSseEvents(eventStream, signal)) {
    ...
    if (status === "finished_successfully" || status === "" || isLive) {
      for (const ptr of extractImagePointers(parts)) {
        ...
        imagePointers.set(ptr, existing?.messageId ? existing : {
          pointer: ptr,
          ...(id ? {
            messageId: id
          } : {})
...
async function fetchConversationDetail(conversationId, ctx) {
  ...
  try {
    ...
    if (response.status >= 400) {
      ctx.log?.warn?.("CGPT-WEB", `conversation poll ${response.status}: ${(response.text || "").slice(0, 300)}`);
      return {
        detail: null,
    ...
    ctx.log?.warn?.("CGPT-WEB", `conversation poll failed: ${err instanceof Error ? err.message : String(err)}`);
    return {
      detail: null,
      terminal: false
    };
...
async function pollForFinalAssistantAnswer(conversationId, ctx) {
  ...
  while (!ctx.signal?.aborted && Date.now() - started < timeoutMs) {
    const {
      detail,
      terminal
    } = await fetchConversationDetail(conversationId, ctx);
    if (detail) {
  ...
  if (last) {
    ctx.log?.warn?.("CGPT-WEB", terminalPollFailure ? `conversation poll stopped before finished_successfully; returning latest assistant text for ${conversationId}` : `conversation poll timed out before finished_successfully; returning latest assistant text for ${conversationId}`);
  } else {
    ctx.log?.warn?.("CGPT-WEB", terminalPollFailure ? `conversation poll stopped without assistant text for ${conversationId}` : `conversation poll timed out without assistant text for ${conversationId}`);
  }
  return last;
}
...
async function resolveImagePointers(pointers, conversationId, resolver, log, fallbackParentMessageId) {
  ...
  for (const ref of pointers) {
    try {
      const url = await resolver(ref.pointer, conversationId, ref.messageId ?? fallbackParentMessageId);
      if (url) urls.push(url);
    } catch (err) {
      log?.warn?.("CGPT-WEB", `Image resolve failed (${ref.pointer}): ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  return urls;
}
function buildStreamingResponse(eventStream, model, cid, created, resolver,
      ...
      try {
        ...
        if (imageGenAsync && conversationId && (!imagePointers || imagePointers.length === 0) && pollAsyncImage) {
          ...
          try {
            ...
            log?.warn?.("CGPT-WEB", `Async image poll failed: ${err instanceof Error ? err.message : String(err)}`);
          } finally {
            stopHb();
          }
        }
        ...
        let urls = [];
        try {
          urls = await resolveImagePointers(imagePointers, conversationId, resolver, log, parentCandidateMessageId);
        } finally {
          stopHb2();
        }
        ...
        if (mdBlock) {
          if (!safeEnqueue(encoder.encode(sseChunk({
            id: cid,
            object: "chat.completion.chunk",
            created,
            model,
...
async function buildNonStreamingResponse(eventStream, model, cid, created, currentMsg, resolver, pollAsyncImage, pollFinalAnswer, log, signal) {
  ...
  const urls = await resolveImagePointers(imagePointers, conversationId, resolver, log, parentCandidateMessageId);
...
function derivePublicBaseUrl(clientHeaders, log) {
  ...
  log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: configured=${configuredBase ?? "-"} header=${headerBase ?? "-"}`);
...
async function fetchDownloadUrl(endpoint, ctx) {
  ...
  if (response.status !== 200) {
    ctx.log?.warn?.("CGPT-WEB", `Image download URL fetch failed (${response.status}) for ${endpoint}`);
    return null;
  }
  let parsed = {};
  try {
...
async function imageUrlToCachedImageUrl(signedUrl, ctx, imageContext) {
  ...
  try {
    ...
    ctx.log?.warn?.("CGPT-WEB", `Image fetch failed: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
  if (response.status !== 200) {
    ctx.log?.warn?.("CGPT-WEB", `Image fetch returned HTTP ${response.status} (${(response.text || "").slice(0, 120)})`);
    return null;
  }
  ...
  if (bytes.length === 0 || bytes.length > IMAGE_DOWNLOAD_MAX_BYTES) {
    if (bytes.length > IMAGE_DOWNLOAD_MAX_BYTES) {
      ctx.log?.warn?.("CGPT-WEB", `Image too large to cache (${bytes.length} bytes > ${IMAGE_DOWNLOAD_MAX_BYTES}); skipping`);
    }
    return null;
  }
...
async function registerWebSocket(ctx) {
  ...
  const candidates = [{
    url: `${CHATGPT_BASE}/backend-api/celsius/ws/user`,
    method: "GET"
  }, {
    url: `${CHATGPT_BASE}/backend-api/register-websocket`,
    method: "POST"
  }];
  const headers = {
    ...browserHeaders(),
  ...
  for (const {
    ...
    try {
      ...
      ctx.log?.warn?.("CGPT-WEB", `register-websocket fetch failed for ${url}: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }
    if (r.status === 200) {
      try {
    ...
    ctx.log?.warn?.("CGPT-WEB", `register-websocket via ${url} → ${r.status}: ${(r.text || "").slice(0, 200)}`);
  }
  return null;
}
      ...
      for (const data of candidates) {
        ...
        if (Array.isArray(m?.content?.parts)) {
          for (const ptr of extractImagePointers(m.content?.parts ?? [])) {
            ...
            found.set(ptr, existing?.messageId ? existing : {
              pointer: ptr,
              ...(m?.id ? {
                messageId: m.id
              } : {})
...
async function pollForAsyncImage(conversationId, ctx, opts = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!wssUrl) {
      ctx.log?.warn?.("CGPT-WEB", attempt === 0 ? "Could not register WebSocket — async image gen not retrievable" : `WebSocket re-registration failed on retry attempt ${attempt + 1}`);
      if (attempt === 0) continue; // try again — registration can be flaky
      return [];
    }
    ctx.log?.debug?.("CGPT-WEB", `Registered WebSocket for async image (attempt ${attempt + 1}, ${remaining}ms remaining)`);
    ...
    ctx.log?.warn?.("CGPT-WEB", `WebSocket attempt ${attempt + 1} ended in transport error before any frame; retrying`);
  }
  return [];
}
function makeImageResolver(ctx) {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      signedUrl = await fetchDownloadUrl(`${CHATGPT_BASE}/backend-api/files/${encodeURIComponent(fileId)}/download`, ctx);
      if (!signedUrl && conversationId) {
        signedUrl = await fetchDownloadUrl(`${CHATGPT_BASE}/backend-api/conversation/${encodeURIComponent(conversationId)}/attachment/${encodeURIComponent(fileId)}/download`, ctx);
      }
    }
    let finalUrl = null;
    if (signedUrl) {
      ...
      finalUrl = await imageUrlToCachedImageUrl(signedUrl, ctx, conversationId && parentMessageId ? {
        conversationId,
        parentMessageId
      } : undefined);
    }
...
export class ChatGptWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    const {
      ...
      requestedTools,
      effectiveMessages
    } = prepareToolMessages(body || {}, messages);
    if (!credentials.apiKey) {
      return {
        response: errorResponse(401, "ChatGPT auth failed — paste your __Secure-next-auth.session-token cookie value."),
        url: CONV_URL,
        headers: {},
        transformedBody: body
      };
    ...
    try {
      ...
      log?.error?.("CGPT-WEB", `Session exchange failed: ${err instanceof Error ? err.message : String(err)}`);
      return {
        response: errorResponse(502, `ChatGPT session exchange failed: ${err instanceof Error ? err.message : String(err)}`),
        url: SESSION_URL,
        headers: {},
        transformedBody: body
      };
    ...
    if (tokenEntry.refreshedCookie && tokenEntry.refreshedCookie !== cookie) {
      const updated = {
        ...credentials,
        apiKey: tokenEntry.refreshedCookie
      };
      try {
        await onCredentialsRefreshed?.(updated);
      } catch (err) {
        log?.warn?.("CGPT-WEB", `Failed to persist refreshed cookie: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    ...
    try {
      ...
      log?.warn?.("CGPT-WEB", `DPL warmup failed (continuing with fallback): ${err instanceof Error ? err.message : String(err)}`);
      dplInfo = {
    ...
    await runSessionWarmup(tokenEntry.accessToken, tokenEntry.accountId, sessionId, deviceId, cookie, signal, log);
    ...
    if (requestedEffort && isThinkingCapableModel(model, modelSlug)) {
      await setUserThinkingEffort(modelSlug, requestedEffort, tokenEntry.accessToken, tokenEntry.accountId, sessionId, deviceId, cookie, signal, log);
    }

    // 2b. Sentinel chat-requirements
    let reqs;
    try {
      reqs = await prepareChatRequirements(tokenEntry.accessToken, tokenEntry.accountId, sessionId, deviceId, cookie, dplInfo, signal, log);
    } catch (err) {
      if (err instanceof SentinelBlockedError) {
        log?.warn?.("CGPT-WEB", err.message);
        return {
          response: errorResponse(403, "ChatGPT blocked the request (Sentinel/Turnstile required). Try again later or open chatgpt.com in a browser to refresh state.", "SENTINEL_BLOCKED"),
          url: SENTINEL_PREPARE_URL,
          headers: {},
          transformedBody: body
        };
      }
      log?.error?.("CGPT-WEB", `Sentinel failed: ${err instanceof Error ? err.message : String(err)}`);
      return {
        response: errorResponse(502, `ChatGPT sentinel failed: ${err instanceof Error ? err.message : String(err)}`),
        url: SENTINEL_PREPARE_URL,
        headers: {},
        transformedBody: body
      };
    ...
    if (forImageGen) {
      log?.debug?.("CGPT-WEB", continuation ? "Image edit intent detected — continuing saved image conversation" : "Image-gen intent detected — disabling Temporary Chat for this turn");
    ...
    try {
      ...
      return {
        response: errorResponse(502, `ChatGPT connection failed: ${err instanceof Error ? err.message : String(err)}`, code),
        url: CONV_URL,
        headers,
    ...
    let finalResponse;
    if (stream && !toolMode) {
      const sseStream = buildStreamingResponse(bodyStream, model, cid, created, imageResolver, pollAsyncImage, pollFinalAnswer, log, signal);
      ...
      finalResponse = await buildNonStreamingResponse(bodyStream, model, cid, created, parsed.currentMsg, imageResolver, pollAsyncImage, pollFinalAnswer, log, signal);
      if (toolMode) {
