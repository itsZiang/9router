...
export function stainlessRuntimeVersion() {
...
export function passthroughUpstreamSessionId(clientHeaders) {
...
export function getSessionId(seed) {
...
export function generateCliUserID() {
 ...
 */
export function resolveCliUserID(providerSpecificData, seed) {
...
export async function fetchClaudeBootstrap(accessToken) {
...
async function backgroundFetchAccountUUID(accessToken, seed) {
  ...
  try {
    ...
    setBounded(accountUuidCache, seed, {
      uuid: bootstrap?.account_uuid ?? null,
      fetchedAt: Date.now()
...
export function uuidV4FromHash(hex64) {
  return [hex64.slice(0, 8), hex64.slice(8, 12), "4" + hex64.slice(13, 16), (parseInt(hex64.charAt(16), 16) & 0x3 | 0x8).toString(16) + hex64.slice(17, 20), hex64.slice(20, 32)].join("-");
}
 ...
 */
export function resolveAccountUUID(providerSpecificData, seed, accessToken) {
  ...
  return uuidV4FromHash(createHash("sha256").update("account:" + seed).digest("hex") // nosemgrep: insufficient-password-hash
  );
}
...
export function parseUpstreamMetadataUserId(body) {
  if (!body) return null;
 ...
 */
export function selectBetaFlags(body, model,
...
clientBeta) {
  const b = body || {};
  const clientBetaSet = typeof clientBeta === "string" && clientBeta.trim().length > 0 ? new Set(clientBeta.split(",").map(f => f.trim()).filter(Boolean)) : null;
  ...
  const allowHeavy = clientBetaSet === null || clientBetaSet.has("advanced-tool-use-2025-11-20") || clientBetaSet.has("effort-2025-11-24");
  ...
  if (allowThinking) {
    flags.push("interleaved-thinking-2025-05-14", "redact-thinking-2026-02-12", "thinking-token-count-2026-05-13");
  }
 ...
 */
export function buildHashFor(version, dayStamp) {
...
export function stripProxyToolPrefix(body) {
  const stripName = n => {
