...
export function isMissingBrowserExecutable(message) {
  ...
  return /executable doesn't exist|executablenotfound|playwright install|chromium.*download/i.test(message);
}
...
export class GeminiWebExecutor extends BaseExecutor {
  ...
  async execute(input) {
    ...
    try {
      ...
      await context.addCookies(cookiePairs.map(({
        name,
        value
      }) => ({
        name,
