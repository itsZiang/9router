...
const SEEDED_COOKIES = [["5", "1"], ["ah", "wt-wt"], ["dcs", "1"], ["dcm", "3"], ["isRecentChatOn", "1"]];
...
export class DuckDuckGoWebExecutor extends BaseExecutor {
  ...
  constructor() {
    super("duckduckgo-web", {
      baseUrl: DUCKDUCKGO_BASE
    });
  }
  warmed = false;
  seeded = false;
  ...
  async execute(input) {
    ...
    const errorResponse = (status, message) => new Response(JSON.stringify({
      error: {
        message
      }
    }), {
  ...
  async seedChallengeChain(model, signal) {
    ...
    try {
      ...
      this.rememberResponseCookies(response);
      if (response.ok) this.rememberChallengeHeader(response);else this.pendingVqdHash1 = previousPending;
    ...
    if (streaming) {
      ...
      const openaiResponse = hasTools ? (() => {
        const {
          ...
          toolCalls,
          finishReason
        } = buildToolAwareResult(fullContent, requestedTools, "ddg");
        const message = {
          role: "assistant",
          content
        };
