...
import { buildClaudeCodeCompatibleHeaders, CLAUDE_CODE_COMPATIBLE_DEFAULT_CHAT_PATH, joinClaudeCodeCompatibleUrl } from "../services/claudeCodeCompatible";
import { getGigachatAccessToken } from "../services/gigachatAuth";
...
import { isOfficialAnthropicBaseUrl } from "../utils/anthropicHost";
import { applyProviderRequestDefaults } from "../services/providerRequestDefaults";
import { stripUnsupportedParams } from "../translator/paramSupport";
import { detectFormat, getOpenAICompatibleType, getTargetFormat, isClaudeCodeCompatible } from "../services/provider";
import { sanitizeQwenThinkingToolChoice } from "../services/qwenThinking";
import { buildDataRobotChatUrl } from "../config/datarobot";
import { buildAzureAiChatUrl } from "../config/azureAi";
import { buildWatsonxChatUrl } from "../config/watsonx";
import { buildOciChatUrl } from "../config/oci";
import { buildSapChatUrl, getSapResourceGroup } from "../config/sap";
import { buildMaritalkChatUrl } from "../config/maritalk";
import { LOCAL_PROVIDERS } from "@/shared/constants/providers";
import { isForbiddenCustomHeaderName } from "../stubs/shared/constants/upstreamHeaders";
import { getClaudeCodeCompatibleRequestDefaults } from "../stubs/lib/providers/requestDefaults";
import { buildClineHeaders } from "../stubs/shared/utils/clineAuth";
import { normalizeBaseUrl } from "../utils/urlSanitize";
import { normalizeHerokuChatUrl, normalizeDatabricksChatUrl, normalizeSnowflakeChatUrl, normalizeGigachatChatUrl } from "../stubs/lib/providers/validation/urlHelpers";
...
function getOpenRouterConnectionPreset(providerSpecificData) {
  const preset = typeof providerSpecificData?.preset === "string" ? providerSpecificData.preset.trim() : "";
  return preset || null;
}
export class DefaultExecutor extends BaseExecutor {
  ...
  buildUrl(model, stream, urlIndex = 0, credentials = null) {
    ...
    if (this.provider?.startsWith?.("openai-compatible-")) {
      ...
      const path = forceResponses || getOpenAICompatibleType(this.provider, psd) === "responses" ? "/responses" : "/chat/completions";
    ...
    if (this.provider?.startsWith?.("anthropic-compatible-")) {
      ...
      if (isClaudeCodeCompatible(this.provider)) {
        return joinClaudeCodeCompatibleUrl(baseUrl, customPath || CLAUDE_CODE_COMPATIBLE_DEFAULT_CHAT_PATH);
      }
      const normalized = baseUrl.replace(/\/$/, "");
    ...
    switch (this.provider) {
      case "openai":
        {
          ...
          const customBaseUrl = typeof credentials?.providerSpecificData?.baseUrl === "string" && credentials.providerSpecificData.baseUrl.trim() ? credentials.providerSpecificData.baseUrl : null;
      ...
      case "azure-ai":
        {
          const forceResponses = credentials?.providerSpecificData?._omnirouteForceResponsesUpstream === true;
          const apiType = forceResponses || credentials?.providerSpecificData?.apiType === "responses" ? "responses" : "chat";
          const baseUrl = this.resolveBaseUrl(credentials);
      ...
      case "oobabooga":
        {
          ...
          const baseUrl = credentials?.providerSpecificData?.baseUrl || localDefault || this.config.baseUrl;
          return normalizeOpenAIChatUrl(baseUrl);
        }
      case "zai":
  ...
  buildHeaders(credentials, stream = true, clientHeaders) {
    const {
      headers,
      effectiveKey
    ...
    switch (this.provider) {
      case "gemini":
        effectiveKey ? headers["x-goog-api-key"] = effectiveKey : headers["Authorization"] = `Bearer ${credentials.accessToken}`;
        break;
      case "snowflake":
        {
          ...
          headers["X-Snowflake-Authorization-Token-Type"] = usesProgrammaticAccessToken ? "PROGRAMMATIC_ACCESS_TOKEN" : "KEYPAIR_JWT";
          break;
        }
      case "gigachat":
      ...
      case "oci":
        {
          ...
          const projectId = credentials.projectId || credentials?.providerSpecificData?.projectId || credentials?.providerSpecificData?.project;
          if (projectId) {
      ...
      case "claude":
      case "anthropic":
        effectiveKey ? headers["x-api-key"] = effectiveKey : headers["Authorization"] = `Bearer ${credentials.accessToken}`;
        break;
      case "glm":
      case "glmt":
      case "kimi-coding":
      ...
      default:
        if (isClaudeCodeCompatible(this.provider)) {
          ...
          const ccHeaders = buildClaudeCodeCompatibleHeaders(effectiveKey || credentials.accessToken || "", stream, credentials?.providerSpecificData?.ccSessionId, {
        ...
        if (this.provider?.startsWith?.("anthropic-compatible-")) {
          ...
          const hasAnthropicVersion = Object.keys(headers).some(key => key.toLowerCase() === "anthropic-version");
    ...
    const isCompatibleProvider = this.provider?.startsWith?.("openai-compatible-") || this.provider?.startsWith?.("anthropic-compatible-");
  ...
  applyJsonSchemaFallback(body) {
    ...
    const record = body;
    const rf = record.response_format;
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    if (withDefaults && typeof withDefaults === "object" && !Array.isArray(withDefaults) && (this.provider === "cerebras" || this.provider === "mistral") && Object.prototype.hasOwnProperty.call(withDefaults, "client_metadata")) {
      const withoutClientMetadata = {
        ...withDefaults
      };
    ...
    const requestFormat = withDefaults && typeof withDefaults === "object" && !Array.isArray(withDefaults) ? detectFormat(withDefaults) : "openai";
    if (typeof withDefaults === "object" && withDefaults !== null && !Array.isArray(withDefaults)) {
      if (this.provider?.startsWith?.("anthropic-compatible-")) {
        ...
        const defaultsRecord = withDefaults;
        const bodyDisablesStreamOptions = defaultsRecord.stream !== undefined && defaultsRecord.stream !== true;
        const qwenBlocksStreamOptions = this.provider === "qwen" && (Boolean(defaultsRecord.thinking) || Boolean(defaultsRecord.enable_thinking));
        ...
        delete withoutStreamOptions.stream_options;
        withDefaults = withoutStreamOptions;
      } else if ((targetFormat === "openai-responses" || requestFormat === "openai-responses") && Object.prototype.hasOwnProperty.call(withDefaults, "stream_options")) {
        const withoutStreamOptions = {
          ...withDefaults
        };
        delete withoutStreamOptions.stream_options;
      ...
      if (this.provider === "openrouter") {
        ...
        if (connectionPreset && withDefaults.preset === undefined) {
          withDefaults = {
            ...withDefaults,
    ...
    if (typeof withDefaults === "object" && withDefaults !== null) {
      ...
      if (entry?.modelIdPrefix) {
        ...
        if (typeof body.model === "string") {
          ...
          const alreadyQualified = acceptedPrefixes.some(prefix => body.model.startsWith(prefix));
          if (!alreadyQualified) {
