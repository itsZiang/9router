import { copyOpenAICompatibleReasoningFields, getReadableReasoningValue } from "../utils/reasoningFields";
import { normalizeOpenAICompatibleFinishReason } from "../utils/finishReason";
import { collapseExcessiveNewlines, extractThinkingFromContent } from "./responseSanitizer/reasoning";
export { extractThinkingFromContent, shouldParseTextualReasoningTags } from "./responseSanitizer/reasoning";
...
function stripZeroWidthValue(value) {
  ...
  if (record) {
    return Object.fromEntries(Object.entries(record).map(([key, item]) => [key, stripZeroWidthValue(item)]));
  }
  return value;
}
...
function parseTextualToolCallContent(content) {
  ...
  const decoders = [value => value, value => {
  ...
  for (const decode of decoders) {
    try {
      ...
      return {
        name,
        args: stripZeroWidthValue(JSON.parse(decoded))
      };
    } catch {}
  }
  return null;
...
export function sanitizeOpenAIResponse(body, options = {}) {
...
export function sanitizeResponsesApiResponse(body) {
  const bodyRecord = toRecord(body);
  if (!bodyRecord) return body;
  if (Array.isArray(bodyRecord.choices)) {
  ...
  const sanitized = {
    ...
    object: "response",
    created_at: toNumber(responseRoot.created_at) ?? toNumber(responseRoot.created) ?? Math.floor(Date.now() / 1000),
  ...
  if (output.length === 0 && typeof responseRoot.output_text === "string" && responseRoot.output_text.trim().length > 0) {
    output.push({
      ...
      content: [{
        type: "output_text",
        text: responseRoot.output_text.trim()
      }]
    });
  }
...
function sanitizeMessageContent(msgRecord, options = {}) {
  if (typeof msgRecord.content === "string") {
    ...
    const {
      content,
      thinking
    } = options.parseTextualReasoningTags === true && !nativeReasoning ? extractThinkingFromContent(strippedContent) : {
      content: strippedContent,
      thinking: null
    };
    const sanitized = {
      content: collapseExcessiveNewlines(content)
    };
    if (thinking) sanitized.reasoning_content = thinking;
    return sanitized;
  }
  return msgRecord.content !== undefined ? {
...
function sanitizeResponsesUsage(usage) {
  ...
  if (normalized.prompt_tokens_details !== undefined && normalized.input_tokens_details === undefined) {
    normalized.input_tokens_details = normalized.prompt_tokens_details;
  }
  if (normalized.completion_tokens_details !== undefined && normalized.output_tokens_details === undefined) {
  ...
  if (normalized.cache_read_input_tokens !== undefined && inputDetails.cached_tokens === undefined) {
    inputDetails.cached_tokens = normalized.cache_read_input_tokens;
  }
  if (normalized.cache_creation_input_tokens !== undefined && inputDetails.cache_creation_tokens === undefined) {
...
function sanitizeResponsesStreamingOutputItem(item) {
  ...
  if (type === "function_call") {
    return {
      ...itemRecord,
      type: "function_call",
      arguments: typeof itemRecord.arguments === "string" ? itemRecord.arguments : JSON.stringify(itemRecord.arguments || {})
    };
  }
...
const RESPONSES_STREAMING_TEXT_DELTA_EVENTS = new Set(["response.output_text.delta", "response.reasoning_summary_text.delta", "response.reasoning_text.delta"]);

// Matching `*.done` events that carry the finalized text on the root `text` field.
const RESPONSES_STREAMING_TEXT_DONE_EVENTS = new Set(["response.output_text.done", "response.reasoning_summary_text.done", "response.reasoning_text.done"]);
function sanitizeResponsesStreamingEvent(parsedRecord) {
  ...
  if (responseRecord) {
    const responseOutput = Array.isArray(responseRecord.output) ? sanitizeResponsesStreamingOutput(responseRecord.output) : undefined;
...
function sanitizeResponsesMessageContent(content) {
  ...
  return content.map(part => {
    ...
    if (partType === "output_text" || partType === "text" || (partType === undefined || partType === "") && typeof partRecord.text === "string") {
      return {
        ...partRecord,
        type: "output_text",
        text: collapseExcessiveNewlines(stripInternalToolEnvelopeText(toString(partRecord.text) || "")),
...
function convertOpenAIResponseToResponses(openaiResponse) {
  ...
  if (messageContent.length > 0 || !hasToolCalls && !reasoningContent) {
    output.push({
      ...
      content: messageContent.length > 0 ? messageContent : [{
        type: "output_text",
        text: "",
        annotations: []
      }]
 ...
 */
export function sanitizeStreamingChunk(parsed) {
    ...
    sanitized.choices = parsedRecord.choices.map(choice => {
      ...
      if (choiceRecord.delta !== undefined) {
        ...
        if (deltaRecord) {
          ...
          if (deltaRecord.function_call !== undefined) delta.function_call = deltaRecord.function_call;
          c.delta = delta;
        } else {
