...
import { getChatGptImage } from "../../../services/chatgptImageCache";
import { saveImageErrorResult, saveImageSuccessResult } from "../../imageGeneration";
...
export async function handleChatGptWebImageGeneration({
  ...
  if (log && requestedCount > 1) {
    log.warn("IMAGE", `ChatGPT Web returns one image per chat turn; requested n=${requestedCount} will run sequentially`);
  }
  ...
  for (let i = 0; i < requestedCount; i++) {
    ...
    const result = await executor.execute({
      ...
      body: {
        messages: [{
          role: "user",
          content: buildChatGptWebImagePrompt(body)
        }]
      },
      stream: false,
      credentials,
    ...
    for (const url of urls) {
      ...
      images.push({
        b64_json: cached.bytes.toString("base64")
      });
    }
  }
