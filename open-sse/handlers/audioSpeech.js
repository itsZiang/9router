...
import { getSpeechProvider, parseSpeechModel } from "../config/audioRegistry";
import { buildAuthHeaders } from "../config/registryUtils";
import { kieExecutor } from "../executors/kie";
import { vertexGenerateSpeech } from "../executors/vertexMedia";
import { errorResponse } from "../utils/error";
import { getKieCallbackUrl, getKieErrorMessage, getKieErrorStatus, isJsonObject, parseKieResultJson } from "../utils/kieTask";
...
function extractUpstreamErrorMessage(parsed) {
  ...
  const candidates = [parsed?.err_msg, parsed?.error?.message, typeof parsed?.error === "string" ? parsed.error : null, parsed?.message, typeof detail === "string" ? detail : detail?.message];
...
function findAudioUrlDeep(value) {
  ...
  if (isJsonObject(value)) {
    const preferredKeys = ["audio_url", "audioUrl", "stream_audio_url", "streamAudioUrl", "resultUrl", "url", "downloadUrl", "resultUrls"];
...
function findKieAudioUrl(recordData) {
  ...
  const candidates = [response, data, resultJson, ...(Array.isArray(response) ? response : []), ...(Array.isArray(nestedData) ? nestedData : []), ...(Array.isArray(resultJson.data) ? resultJson.data : []), ...(Array.isArray(resultJson.result) ? resultJson.result : [])];
...
function resolveAwsPollyRegion(providerSpecificData) {
  return getStringValue(providerSpecificData.region) || getStringValue(providerSpecificData.awsRegion) || process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION || "us-east-1";
}
...
async function handleKieAudioSpeech(providerConfig, body, modelId, token) {
  ...
  try {
    ...
    return Response.json({
      error: {
        message: getKieErrorMessage(err, "Kie audio createTask failed"),
        code: status
      }
    }, {
      status,
...
export async function handleAudioSpeech({
  ...
  if (!providerConfig) {
    return errorResponse(400, `No speech provider found for model "${body.model}". Use format provider/model. Available: openai, hyperbolic, deepgram, nvidia, elevenlabs, huggingface, inworld, cartesia, playht, kie, aws-polly, xiaomi-mimo, coqui, tortoise, qwen`);
  }
  ...
  try {
    ...
    if (providerConfig.format === "vertex-gemini-tts") {
      const {
        audio,
        contentType
      } = await vertexGenerateSpeech(credentials, {
        model: modelId,
        input: body.input,
        voice: body.voice
      });
