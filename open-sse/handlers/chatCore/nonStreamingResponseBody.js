...
import { withBodyTimeout } from "../../utils/stream";
import { FETCH_BODY_TIMEOUT_MS } from "../../config/constants";
import { createBodyTimeoutError, readStreamChunkWithTimeout } from "./upstreamTimeouts";
import { appendNonStreamingSseTerminalSignal } from "./nonStreamingSse";
...
export class NonStreamingResponseTooLargeError extends Error {
  ...
  constructor(bytesSeen, maxBytes) {
    super(`Upstream non-streaming response exceeded the ${maxBytes}-byte cap (saw at least ${bytesSeen} bytes)`);
...
export async function readNonStreamingResponseBody(response, contentType, upstreamStream, maxBytes = MAX_NONSTREAMING_RESPONSE_BYTES) {
...
async function readNextNonStreamingChunk(reader, deadline) {
  ...
  const {
    ...
    value
  } = await readStreamChunkWithTimeout(reader, timeoutMs);
  if (done) return {
    kind: "done"
  };
  if (!value) return {
...
async function drainNonStreamingSseBody(body, maxBytes) {
  ...
  try {
    while (true) {
      ...
      const decodedChunk = decoder.decode(next.value, {
        stream: true
      });
