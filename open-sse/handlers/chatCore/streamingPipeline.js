 ...
 */
import { pipeWithDisconnect as defaultPipeWithDisconnect } from "../../utils/streamHandler";
import { createSseHeartbeatTransform as defaultHeartbeat, shapeForClientFormat as defaultShape } from "../../utils/sseHeartbeat";
import { createModelEchoTransform as defaultModelEcho } from "../../services/responseModelEcho";
import { createProgressTransform as defaultProgress, wantsProgress as defaultWantsProgress } from "../../utils/progressTracker";
import { createPiiSseTransform as defaultPiiSse } from "../../stubs/lib/streamingPiiTransform";
import { isFeatureFlagEnabled as defaultFeatureFlag } from "../../stubs/shared/utils/featureFlags";
...
export function assembleStreamingPipeline(args, deps = DEFAULT_DEPS) {
  // ── Phase 9.3: Progress tracking (opt-in) ──
  const progressEnabled = deps.wantsProgress(args.clientRawRequestHeaders);
  let finalStream;
  let piiStream = deps.pipeWithDisconnect(args.providerResponse, args.transformStream, args.streamController);
  ...
  finalStream = finalStream.pipeThrough(deps.createSseHeartbeatTransform({
    signal: args.streamController.signal,
