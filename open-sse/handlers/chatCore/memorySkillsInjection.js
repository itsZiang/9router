import { retrieveMemories } from "../../stubs/lib/memory/retrieval";
import { getMemorySettings, DEFAULT_MEMORY_SETTINGS, toMemoryRetrievalConfig } from "../../stubs/lib/memory/settings";
import { injectMemory, shouldInjectMemory } from "../../stubs/lib/memory/injection";
import { injectSkills } from "../../stubs/lib/skills/injection";
...
export async function injectMemoryAndSkills({
  ...
  if (memoryOwnerId && memorySettings && shouldInjectMemory(body, {
    ...
    try {
      const lastUserQuery = (() => {
        const NON_USER_TYPES = new Set(["function_call", "function_call_output", "tool_call", "tool_call_output", "reasoning", "computer_call", "computer_call_output", "web_search_call", "file_search_call"]);
      ...
      const memories = await retrieveMemories(memoryOwnerId, toMemoryRetrievalConfig(memorySettings, {
        query: lastUserQuery
      }));
      if (memories.length > 0) {
        ...
        const cacheSafe = detectCachingContext(body, {
          provider,
          targetFormat
        }).hasCacheControl;
        const injected = injectMemory(body, memories, provider, {
          cacheSafe
        });
        body = injected;
      ...
      log?.debug?.("MEMORY", `Memory injection skipped: ${memErr instanceof Error ? memErr.message : String(memErr)}`);
    }
  }
  if (memoryOwnerId && memorySettings?.skillsEnabled) {
    ...
    const mergedTools = injectSkills({
      ...
      targetFormat,
      backgroundReason,
      messages: Array.isArray(body.messages) ? body.messages : Array.isArray(body.input) ? body.input : undefined
    });
