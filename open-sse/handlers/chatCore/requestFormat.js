...
import { detectFormatFromEndpoint } from "../../services/provider";
import { shouldUseNativeCodexPassthrough } from "./passthroughHelpers";
...
function isCopilotClient(headers, userAgent) {
  const isMatch = value => typeof value === "string" && value.toLowerCase().includes("copilot");
...
export function resolveChatCoreRequestFormat(opts) {
  ...
  const clientResponseFormat = sourceFormat === FORMATS.OPENAI_RESPONSES && !isResponsesEndpoint && !isDroidCLI ? FORMATS.OPENAI : sourceFormat;
  return {
    endpointPath,
    sourceFormat,
    isResponsesEndpoint,
