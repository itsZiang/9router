...
const STREAMING_RESPONSE_HEADER_DENYLIST = new Set(["content-type", "content-encoding", "content-length", "transfer-encoding"]);
 ...
 */
export function isNextMiddlewareControlHeader(headerName) {
 ...
 */
export function stripNextMiddlewareControlHeaders(headers) {
  const toDelete = [];
...
export function buildStreamingResponseHeaders(providerHeaders, meta) {
  const forwardedHeaders = [];
  providerHeaders.forEach((value, key) => {
    if (!STREAMING_RESPONSE_HEADER_DENYLIST.has(key.toLowerCase()) && !isNextMiddlewareControlHeader(key)) {
...
export function materializeDeduplicatedExecutionResult(result) {
  const snapshot = result && typeof result === "object" ? result._dedupSnapshot : undefined;
 ...
 */
export function stripStaleForwardingHeaders(headers) {
