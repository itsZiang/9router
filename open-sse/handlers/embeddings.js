...
import { getEmbeddingProvider, getEmbeddingModelDefaultParams, parseEmbeddingModel } from "../config/embeddingRegistry";
import { saveCallLog } from "@/lib/usageDb";
import { createRequestLogger } from "../utils/requestLogger";
import { isDetailedLoggingEnabled } from "../stubs/lib/db/detailedLogs";
import { getCallLogPipelineCaptureStreamChunks } from "../stubs/lib/logEnv";
import { toJsonErrorPayload } from "../stubs/shared/utils/upstreamError";
import { stripStaleEncodingHeaders } from "../utils/upstreamResponseHeaders";
...
export async function handleEmbedding({
  ...
  if (resolvedProvider) {
    ...
    const parsed = parseEmbeddingModel(body.model);
  ...
  const reqLogger = await createRequestLogger(provider || "openai", "openai", body.model, {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const clientErrorBody = toJsonErrorPayload(errorText.slice(0, 500), "Embedding provider error");
