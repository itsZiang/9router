...
import { GOOGLE_FLOW_POLL_PATH, GOOGLE_FLOW_SUBMIT_PATH, buildGoogleFlowSubmitBody, normalizeFlowVideoParams, parseFlowOperationName, parseFlowOperationResult, resolveFlowAccessToken, resolveFlowProjectId } from "./googleFlow";
...
export async function handleGoogleFlowVideoGeneration({
  ...
  if (!token) {
    return {
      success: false,
      status: 401,
      error: "Missing Google OAuth token for Google Flow. Connect a Google account in Providers (the Antigravity/Cloud Code connection) first."
    };
  }
  ...
  if (!projectId) {
    return {
      success: false,
      status: 400,
      error: "Missing Google projectId for Google Flow. Please reconnect OAuth in Providers so OmniRoute can fetch your Cloud Code project."
    };
  }
  ...
  try {
    log?.info?.("VIDEO", `googleflow/${model} (veo) | submitting | aspect: ${params.aspectRatio ?? "default"}`);
    ...
    if (!operationName) {
      return {
        success: false,
        status: 502,
        error: "Google Flow did not return an operation name"
      };
    }
    ...
    return {
      success: false,
      status: 504,
      error: "Google Flow video generation timed out"
    };
  } catch (err) {
    const e = err ?? {};
