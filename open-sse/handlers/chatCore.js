import { injectMemoryAndSkills } from "./chatCore/memorySkillsInjection";
import { resolveChatCoreRequestSetup } from "./chatCore/requestSetup";
import { buildFailureUsageRecord } from "./chatCore/failureUsage";
import { extractSystemRoleMessages } from "./chatCore/claudeSystemRole";
export { extractSystemRoleMessages } from "./chatCore/claudeSystemRole";
import { checkIdempotencyCache } from "./chatCore/idempotency";
import { checkSemanticCache } from "./chatCore/semanticCache";
import { applyClientUsageBuffer } from "./chatCore/clientUsageBuffer";
import { buildPostCallGuardrailContext } from "./chatCore/postCallGuardrailContext";
import { storeSemanticCacheResponse } from "./chatCore/semanticCacheStore";
import { buildNonStreamingResponseHeaders } from "./chatCore/nonStreamingResponseHeaders";
import { buildNonStreamingJsonResponse } from "./chatCore/nonStreamingJsonResponse";
import { maybeConvertJsonBodyToSse } from "./chatCore/jsonBodyToSse";
import { assembleStreamingResponseHeaders } from "./chatCore/streamingResponseHeaders";
import { storeStreamingSemanticCacheResponse } from "./chatCore/streamingSemanticCacheStore";
import { assembleStreamingPipeline } from "./chatCore/streamingPipeline";
import { sanitizeChatRequestBody } from "./chatCore/sanitization";
import { getHeaderValueCaseInsensitive, isNoMemoryRequested, resolveCompressionHeader, isStripReasoningRequested } from "./chatCore/headers";
import { markCodexScopeRateLimited } from "./chatCore/codexFailover";
import { getCombosCached } from "./chatCore/comboContextCache";
export { clearCombosCache, clearUpstreamProxyConfigCache } from "./chatCore/comboContextCache";
import { resolveAccountSemaphoreKey, resolveAccountSemaphoreMaxConcurrency, buildClaudePromptCacheLogMeta } from "./chatCore/executorHelpers";
import { shouldUseNativeCodexPassthrough, redactPassthroughThinkingSignatures, isClaudeCodeSemanticPassthroughRequest } from "./chatCore/passthroughHelpers";
import { buildStreamingResponseHeaders, materializeDeduplicatedExecutionResult, stripNextMiddlewareControlHeaders, stripStaleForwardingHeaders } from "./chatCore/responseHeaders";
import { forwardDashboardEventToLiveWs, maybeSyncClaudeExtraUsageState } from "./chatCore/telemetryHelpers";
...
export { shouldUseNativeCodexPassthrough, redactPassthroughThinkingSignatures, isClaudeCodeSemanticPassthroughRequest, buildStreamingResponseHeaders, stripStaleForwardingHeaders };
import { extractMemoryTextFromResponse, extractMemoryTextFromRequestBody, resolveMemoryOwnerId } from "./chatCore/memoryExtraction";
import { checkHeapPressureGuard } from "../utils/heapPressure";
import { normalizeHeaders } from "../utils/headers";
import { resolveChatCoreRequestFormat } from "./chatCore/requestFormat";
import { resolveChatCoreTargetFormat } from "./chatCore/targetFormat";
import { defaultClaudeToolType } from "./chatCore/claudeToolDefaults";
import { injectSystemPrompt, injectCustomSystemPrompt } from "../services/systemPrompt";
import { translateRequest, needsTranslation } from "../translator/index";
import { FORMATS } from "../translator/formats";
import { sanitizeKiroTools } from "../utils/kiroSanitizer";
import { splitMisplacedToolResults } from "../translator/helpers/claudeHelper";
import { createSSETransformStreamWithLogger, createPassthroughStreamWithLogger, COLORS, withBodyTimeout } from "../utils/stream";
import { ensureStreamReadiness } from "../utils/streamReadiness";
import { resolveSuppressThinkClose, THINKING_MARKER_HEADER } from "../utils/thinkCloseMarker";
import { resolveStreamReadinessTimeout } from "../utils/streamReadinessPolicy";
import { resolveAgentGoalPolicy } from "../utils/agentGoalPolicy";
...
import { applyClaudeEffortVariant } from "./chatCore/claudeEffortVariant";
import { DEFAULT_THINKING_CLAUDE_SIGNATURE } from "../config/defaultThinkingSignature";
import { getStripTypesForProviderModel, stripIncompatibleMessageContent } from "../services/modelStrip";
import { resolveModelAlias } from "../services/modelDeprecation";
import { normalizeMimoThinking } from "../services/mimoThinking";
import { normalizeClaudeAdaptiveThinking } from "../services/claudeAdaptiveThinking";
import { normalizeClaudeHaikuConstraints } from "../services/claudeHaikuConstraints";
import { echoModelInObject } from "../services/responseModelEcho";
import { stripGpt5SamplingWhenReasoning } from "../services/gpt5SamplingGuard";
import { getUnsupportedParams, REGISTRY } from "../config/providerRegistry";
import { supportsMaxTokens } from "../stubs/lib/modelCapabilities";
import { normalizeThinkingForModel } from "../stubs/shared/constants/modelSpecs";
import { buildErrorBody, createErrorResult, parseUpstreamError, formatProviderError, sanitizeErrorMessage } from "../utils/error";
import { reportMalformed200, detectMalformedNonStream } from "../utils/diagnostics";
import { checkTokenLimits } from "../services/tokenLimitCounter";
import { COOLDOWN_MS, HTTP_STATUS, PROVIDER_MAX_TOKENS, STREAM_READINESS_MAX_TIMEOUT_MS, STREAM_READINESS_TIMEOUT_MS, ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE, STREAM_RECOVERY } from "../config/constants";
...
import { getSkillsModelIdForFormat } from "./chatCore/skillsFormat";
import { readNonStreamingResponseBody } from "./chatCore/nonStreamingResponseBody";
import { isSemaphoreCapacityError, createStreamingErrorResult, getUpstreamErrorIdentifier } from "./chatCore/streamErrorResult";
import { wrapReadableStreamWithFinalize } from "./chatCore/streamFinalize";
import { buildCacheUsageLogMeta } from "./chatCore/cacheUsageMeta";
import { buildExecutorClientHeaders } from "./chatCore/executorClientHeaders";
import { resolveExecutionCredentials as resolveExecutionCredentialsFor } from "./chatCore/executionCredentials";
import { resolveExecutorWithProxy as resolveExecutorWithProxyFor } from "./chatCore/executorProxy";
import { normalizeClaudeUpstreamMessages as normalizeClaudeUpstreamMessagesFor } from "./chatCore/claudeUpstreamMessages";
import { persistAttemptLogs as persistAttemptLogsFor } from "./chatCore/attemptLogging";
import { stageTrace } from "./chatCore/stageTrace";
import { attachCompressionUsageReceiptAfterAnalytics as attachCompressionUsageReceiptAfterAnalyticsFor } from "./chatCore/compressionUsageReceipt";
import { prepareUpstreamBody } from "./chatCore/upstreamBody";
import { getQuotaScopeLabelForProvider } from "../services/antigravityQuotaFamily";
import { getCallLogPipelineCaptureStreamChunks, getCallLogPipelineMaxSizeBytes } from "../stubs/lib/logEnv";
import { logAuditEvent } from "../stubs/lib/compliance";
import { emit } from "../stubs/lib/events/eventBus";
import { adaptBodyForCompression } from "../services/compression/bodyAdapter";
import { ensureEngineBreakdown } from "../services/compression/engineBreakdown";
...
import { finalizePendingScope, updatePendingScope } from "../stubs/lib/usage/pendingRequestScope";
import { recordCost } from "../stubs/domain/costRules";
import { calculateCost } from "../stubs/lib/usage/costCalculator";
import { buildClaudePassthroughToolNameMap, restoreClaudePassthroughToolNames, mergeResponseToolNameMap } from "./chatCore/passthroughToolNames";
import { resolveCompressionSettings } from "./chatCore/compressionSettings";
import { isBuiltinStackedPipeline, isStackedCompressionCombo } from "./chatCore/compressionComboPredicates";
import { emitOutputStyleTelemetry } from "./chatCore/outputStyleTelemetry";
import { writeCompressionAnalytics, writeCompressionSkip } from "./chatCore/compressionAnalyticsWrite";
import { runPluginOnRequestHook } from "./chatCore/pluginOnRequest";
import { recordContextEditingTelemetryHook } from "./chatCore/contextEditingTelemetry";
import { recordCompressionCacheStats } from "./chatCore/compressionCacheStats";
import { writeCavemanOutputAnalytics } from "./chatCore/cavemanOutputAnalytics";
import { scheduleQuotaShareConsumption } from "./chatCore/quotaShareConsumption";
import { emitRequestGamificationEvent } from "./chatCore/gamificationEvent";
import { runPluginOnResponseHook } from "./chatCore/pluginOnResponse";
import { scheduleStreamingQuotaShareConsumption } from "./chatCore/streamingQuotaShare";
import { recordStreamingUsageStats } from "./chatCore/streamingUsageStats";
...
import { deleteSessionAccountAffinity } from "../stubs/lib/db/sessionAccountAffinity";
import { getCacheControlSettings } from "../stubs/lib/cacheControlSettings";
import { guardrailRegistry } from "../stubs/lib/guardrails";
import { shouldPreserveCacheControl } from "../utils/cacheControlPolicy";
import { getCachedSettings } from "../stubs/lib/db/readCache";
import { applyCodexGlobalFastServiceTier } from "../stubs/lib/providers/codexFastTier";
import { buildUpstreamHeadersForExecute as buildUpstreamHeadersForExecuteFor } from "./chatCore/upstreamExecuteHeaders";
import { resolveEffectiveServiceTier as resolveEffectiveServiceTierFor, resolveReportedServiceTier as resolveReportedServiceTierFor } from "./chatCore/serviceTier";
import { cacheReasoningFromAssistantMessage } from "../services/reasoningCache";
import { setDetectedToolLimit, parseToolLimitFromError, shouldDetectLimit } from "../services/toolLimitDetector";
import { isCompactResponsesEndpoint } from "../executors/codex";
import { buildCodexQuotaPersistence } from "./chatCore/codexQuota";
import { invalidateCodexQuotaCache } from "../services/codexQuotaFetcher";
import { translateNonStreamingResponse } from "./responseTranslator";
...
import { compressContext, estimateTokens, getTokenLimit, resolveComboContextLimit } from "../services/contextManager";
import { resolveBackgroundTaskRedirect } from "./chatCore/backgroundRedirect";
import { prepareWebSearchFallbackBody } from "../services/webSearchFallback";
import { resolveExplicitStreamAlias, resolveStreamFlag, stripMarkdownCodeFence } from "../utils/aiSdkCompat";
import { generateRequestId } from "../stubs/shared/utils/requestId";
import { extractFacts } from "../stubs/lib/memory/extraction";
import { handleToolCallExecution } from "../stubs/lib/skills/interception";
import { getClaudeCodeCompatibleRequestDefaults } from "../stubs/lib/providers/requestDefaults";
import { buildClaudeCodeCompatibleRequest, isClaudeCodeCompatibleProvider, resolveClaudeCodeCompatibleSessionId } from "../services/claudeCodeCompatible";
import { setGeminiThoughtSignatureMode } from "../services/geminiThoughtSignatureStore";
import { classifyModelScope429, getModelScopeRetryDelayMs, isModelScopeProvider } from "../services/modelscopePolicy";
import { incrementRequestCount } from "../services/geminiRateLimitTracker";
...
export async function handleChatCore({
  ...
  const trace = (label, extra) => stageTrace(label, extra, {
    traceEnabled,
    startTime,
    traceId,
    log
  ...
  const getCurrentConnectionId = () => {
    const credentialConnectionId = typeof credentials?.connectionId === "string" && credentials.connectionId.trim().length > 0 ? credentials.connectionId.trim() : null;
    return credentialConnectionId || connectionId || null;
  };
  let tokensCompressed = null;
  ...
  if (agentGoalPolicy.detected) {
    log?.debug?.("AGENT_GOAL", `long-running goal mode enabled: readinessMax=${agentGoalPolicy.readinessMaxTimeoutMs}ms streamRecovery=${agentGoalPolicy.streamRecoveryEnabled}`);
  }
  let effectiveServiceTier = "standard";
  ...
  const resolveEffectiveServiceTier = requestBody => resolveEffectiveServiceTierFor(provider, credentials?.providerSpecificData, requestBody);
  const resolveReportedServiceTier = (payload, maxDepth = 3) => resolveReportedServiceTierFor(provider, payload, maxDepth);
  ...
  const persistFailureUsage = (statusCode, errorCode) => {
    saveRequestUsage(buildFailureUsageRecord({
      provider,
      model,
  ...
  const persistCodexQuotaState = async (headers, status = 0) => {
    ...
    try {
      const existingProviderData = credentials?.providerSpecificData && typeof credentials.providerSpecificData === "object" ? credentials.providerSpecificData : {};
  ...
  if (bgRedirect) {
    ...
    log?.info?.("BACKGROUND", `Background task redirect (${bgRedirect.reason}): ${originalModel} → ${bgRedirect.degradedModel}`);
  ...
  const initialProviderRequest = body && typeof body === "object" && !Array.isArray(body) ? {
    ...body,
    model: typeof body.model === "string" ? body.model : effectiveModel
  } : body;
  ...
  const {
    body: bodyWithWebSearchFallback,
    fallback: webSearchFallbackPlan
  } = prepareWebSearchFallbackBody(body, {
    provider,
    sourceFormat,
    targetFormat,
    nativeCodexPassthrough
  ...
  if (webSearchFallbackPlan.enabled) {
    body = bodyWithWebSearchFallback;
    log?.info?.("TOOLS", `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to OmniRoute fallback for ${provider}`);
  }
  ...
  const echoModel = settings.echoRequestedModelName === true && typeof requestedModel === "string" && requestedModel ? requestedModel : null;
  const detailedLoggingEnabled = !noLogEnabled && (settings.call_log_pipeline_enabled === true || settings.call_log_pipeline_enabled === "1" || settings.call_log_pipeline_enabled === "true");
  const capturePipelineStreamChunks = detailedLoggingEnabled && getCallLogPipelineCaptureStreamChunks();
  const skillRequestId = generateRequestId();
  let compressionAnalyticsWritePromise = null;
  ...
  const attachCompressionUsageReceiptAfterAnalytics = (usage, source) => attachCompressionUsageReceiptAfterAnalyticsFor(usage, source, {
    pendingWrite: compressionAnalyticsWritePromise,
    skillRequestId
  });
  const pipelineSessionId = (clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function" ? clientRawRequest.headers.get("x-omniroute-session-id") : getHeaderValueCaseInsensitive(clientRawRequest?.headers ?? null, "x-omniroute-session-id")) || skillRequestId;
  ...
  const persistAttemptLogs = args => persistAttemptLogsFor(args, {
    provider,
    connectionId,
    model,
    skillRequestId,
  ...
  const connectionCustomUserAgent = credentials?.providerSpecificData && typeof credentials.providerSpecificData === "object" && typeof credentials.providerSpecificData.customUserAgent === "string" ? credentials.providerSpecificData.customUserAgent.trim() : "";
  ...
  const buildUpstreamHeadersForExecute = modelToCall => buildUpstreamHeadersForExecuteFor({
    modelToCall,
    effectiveModel,
    provider,
    model,
  ...
  const acceptHeader = clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function" ? clientRawRequest.headers.get("accept") || clientRawRequest.headers.get("Accept") : clientRawRequest?.headers?.["accept"] || clientRawRequest?.headers?.["Accept"];
  const streamUserAgent = [typeof userAgent === "string" ? userAgent : "", getHeaderValueCaseInsensitive(clientRawRequest?.headers ?? null, "user-agent") || ""].filter(Boolean).join(" ");
  ...
  const thinkingMarkerHeader = getHeaderValueCaseInsensitive(clientRawRequest?.headers ?? null, THINKING_MARKER_HEADER);
  ...
  const stream = nativeCodexPassthrough && isCompactResponsesEndpoint(endpointPath) ? false : resolveStreamFlag(body?.stream, acceptHeader, sourceFormat, {
  ...
  const memoryOwnerId = isNoMemoryRequested(clientRawRequest?.headers ?? null) ? null : resolveMemoryOwnerId(apiKeyInfo);
  ...
  const compressionBody = body ? adaptBodyForCompression(body).body : null;
  const allMessages = compressionBody?.messages || body?.contents || body?.request?.contents || [];
  let cavemanOutputModeApplied = false;
  let cavemanOutputModeIntensity = null;
  let preCompressionBody = null;
  let compressionResponseMeta = null;
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    const compressionSettings = compressionSettingsResult.settings;
    ...
    try {
      ...
      } = await import("../services/compression/stats");
      ...
      const applyCompressionComboConfig = (compressionCombo, routingOverrideIds = []) => {
        if (!compressionCombo || compressionCombo.pipeline.length === 0) return false;
        const comboLanguagePacks = [...new Set(compressionCombo.languagePacks.map(pack => pack.trim()).filter(pack => pack.length > 0))];
        const comboOutputIntensity = ["lite", "full", "ultra"].includes(compressionCombo.outputModeIntensity) ? compressionCombo.outputModeIntensity : config.cavemanOutputMode?.intensity ?? "full";
        const comboDefaultLanguage = comboLanguagePacks.find(pack => pack === config.languageConfig?.defaultLanguage) ?? comboLanguagePacks[0] ?? config.languageConfig?.defaultLanguage ?? "en";
        ...
        config = {
          ...
          languageConfig: {
            ...
            enabledPacks: comboLanguagePacks.length > 0 ? comboLanguagePacks : config.languageConfig?.enabledPacks ?? ["en"]
          },
          cavemanOutputMode: {
      ...
      if (isCombo && comboName) {
        try {
          ...
          const comboRuntimeConfig = comboConfig?.config && typeof comboConfig.config === "object" ? comboConfig.config : {};
          const comboMode = typeof comboRuntimeConfig.compressionMode === "string" ? comboRuntimeConfig.compressionMode : typeof comboConfig?.compressionOverride === "string" ? comboConfig.compressionOverride : null;
          if (comboMode === "off" || comboMode === "lite" || comboMode === "standard" || comboMode === "aggressive" || comboMode === "ultra" || comboMode === "rtk" || comboMode === "stacked") {
            config = {
              ...config,
              comboOverrides: {
                ...(config.comboOverrides ?? {}),
                ...
                ...(comboConfig?.id ? {
                  [String(comboConfig.id)]: comboMode
                } : {})
              }
            };
            compressionComboKey = comboName;
          }
          const routingComboIds = [comboConfig?.id, comboName, comboName.startsWith("combo/") ? comboName.substring(6) : null].filter(id => typeof id === "string" && id.length > 0);
          if (routingComboIds.length > 0) {
            const {
              getCompressionComboForRoutingCombo
            } = await import("../stubs/lib/db/compressionCombos");
            const assignedCompressionCombo = routingComboIds.map(id => getCompressionComboForRoutingCombo(id)).find(combo => combo !== null) ?? null;
            if (applyCompressionComboConfig(assignedCompressionCombo, routingComboIds)) {
              compressionComboKey = comboName;
            }
          }
        } catch (err) {
          log?.debug?.("COMPRESSION", "Combo compression override lookup skipped: " + (err instanceof Error ? err.message : String(err)));
        }
      }
      let namedCombos = {};
      try {
        ...
        log?.debug?.("COMPRESSION", "Named combos load skipped: " + (err instanceof Error ? err.message : String(err)));
      }
      ...
      const modeBeforeOutputTransform = selectCompressionStrategy(config, compressionComboKey, estimatedTokens, body, {
        provider,
        targetFormat,
        model: effectiveModel
      }, namedCombos, compressionHeader);
      if (modeBeforeOutputTransform === "stacked" && !compressionComboApplied && !config.compressionComboId && isBuiltinStackedPipeline(config.stackedPipeline) &&
      ...
      !activeComboResolves(config, namedCombos)) {
        try {
          ...
          if (isStackedCompressionCombo(defaultCompressionCombo) && applyCompressionComboConfig(defaultCompressionCombo)) {
            log?.debug?.("COMPRESSION", `Default compression combo applied: ${defaultCompressionCombo?.id}`);
          }
        } catch (err) {
          log?.debug?.("COMPRESSION", "Default compression combo lookup skipped: " + (err instanceof Error ? err.message : String(err)));
        }
      }
      // Phase 4A: unified output styles (supersedes cavemanOutputMode via the back-compat shim).
      let outputStyleResult = null;
      if (config.enabled) {
        try {
          const {
            resolveOutputStyleSelection
          } = await import("../services/compression/outputStyles/backCompat");
          const selection = resolveOutputStyleSelection(config);
          if (selection.length > 0) {
            const {
              applyOutputStyles
            } = await import("../services/compression/outputStyles/apply");
            const outputStyleLanguage = config.languageConfig?.enabled === true ? config.languageConfig.defaultLanguage : "en";
            outputStyleResult = applyOutputStyles(body, selection, outputStyleLanguage);
            if (outputStyleResult.applied) {
              body = outputStyleResult.body;
              cavemanOutputModeApplied = true;
              cavemanOutputModeIntensity = outputStyleResult.appliedStyles?.map(s => `${s.id}:${s.level}`).join(",") ?? null;
              estimatedTokens = estimateTokens(body?.messages ?? body?.input ?? []);
              log?.debug?.("COMPRESSION", "Output styles applied");
            } else if (outputStyleResult.skippedReason && outputStyleResult.skippedReason !== "no_styles") {
              log?.debug?.("COMPRESSION", `Output styles skipped: ${outputStyleResult.skippedReason}`);
            }
          }
        } catch (err) {
          log?.debug?.("COMPRESSION", "Output styles skipped: " + (err instanceof Error ? err.message : String(err)));
        }
      }
      const compressionInputBody = body;
      ...
      const adaptiveModelContextLimit = provider && effectiveModel ? getTokenLimit(provider, effectiveModel) : null;
      const requestMaxTokens = typeof compressionInputBody?.max_tokens === "number" ? compressionInputBody.max_tokens : null;
      let adaptiveTelemetry = null;
      const compressionPlan = selectCompressionPlan(config, compressionComboKey, estimatedTokens, compressionInputBody, {
        provider,
        targetFormat,
        model: effectiveModel
      }, namedCombos, compressionHeader, {
      ...
      const mode = compressionPlan.mode;
      if (adaptiveTelemetry && adaptiveTelemetry.fit === false) {
        log?.warn?.("COMPRESSION", `adaptive budget-exceeded: target=${adaptiveTelemetry.target} headroomAfter=${adaptiveTelemetry.headroomAfter} stages=${adaptiveTelemetry.stagesApplied.join(",")} (best-effort plan sent, content preserved)`);
      }
      ...
      if (mode === "stacked" && compressionPlan.stackedPipeline.length > 0 && !compressionComboApplied && !config.compressionComboId) {
        config = {
          ...config,
          stackedPipeline: compressionPlan.stackedPipeline
        };
      }
      ...
      if (mode !== "off") {
        ...
        if (result.stats) {
          ...
          if (result.compressed) {
            ...
            tokensCompressed = Math.max(0, result.stats.originalTokens - result.stats.compressedTokens);
          }
          ...
          if (result.compressed && result.stats) {
            try {
              ...
              void forwardDashboardEventToLiveWs("compression.completed", compressionCompletedPayload);
            } catch (_emitErr) {
              ...
              log?.debug?.("COMPRESSION", "compression.completed emit skipped: " + (_emitErr instanceof Error ? _emitErr.message : String(_emitErr)));
            }
          }
          ...
          if (result.compressed) {
            ...
            log?.info?.("COMPRESSION", `Prompt compressed (${mode}): ${result.stats.originalTokens} -> ${result.stats.compressedTokens} tokens (${result.stats.savingsPercent}% saved, techniques: ${result.stats.techniquesUsed.join(",")})`);
          }
        }
      }
      ...
      log?.warn?.("COMPRESSION", "Compression pipeline error (non-fatal): " + (err instanceof Error ? err.message : String(err)));
    }
    // --- End Modular Compression Pipeline ---

    if (!promptCompressionEnabled) {
      log?.debug?.("CONTEXT", "Skipping proactive context compression: Prompt Compression disabled");
    }
    ...
    if (isCombo && comboName) {
      ...
      try {
        ...
        const {
          parseModel
        } = await import("../services/model");
        const {
          resolveComboTargets
        } = await import("../services/combo");
        let comboConfig = await getComboByName(comboName);
        if (!comboConfig && comboName.startsWith("combo/")) {
          comboConfig = await getComboByName(comboName.substring(6));
        }
        let comboTargetLimits = [];
        if (comboConfig) {
          const allCombosData = await getCombosCached();
          const targets = resolveComboTargets(comboConfig, allCombosData);
          comboTargetLimits = targets.map(t => {
    ...
    const threshold = Math.max(1, Math.floor((Math.max(1, contextLimit) - reservedTokens) * COMPRESSION_THRESHOLD));
    log?.debug?.("CONTEXT", `Checking compression: ${estimatedTokens} tokens vs ${threshold} threshold (${contextLimit} limit, ${reservedTokens} reserved)`);
    ...
    if (promptCompressionEnabled && estimatedTokens > threshold) {
      log?.info?.("CONTEXT", `Proactive compression triggered: ${estimatedTokens} tokens > ${threshold} threshold (${contextLimit} limit)`);
      ...
      if (compressionResult.compressed) {
        ...
        const layersInfo = stats && "layers" in stats && Array.isArray(stats.layers) ? ` (layers: ${stats.layers.map(l => l.name).join(", ")})` : "";
        log?.info?.("CONTEXT", `Context compressed: ${stats.original} → ${stats.final} tokens${layersInfo}`);
        logAuditEvent({
    ...
    log?.debug?.("CONTEXT", `Skipping compression check: body=${!!body}, hasMessages=${Array.isArray(allMessages)}`);
  }
  ...
  if (Array.isArray(translatedBody?.messages) && stripTypes.length > 0) {
    ...
    if (stripResult.removedParts > 0) {
      ...
      log?.warn?.("CONTENT", `Stripped ${stripResult.removedParts} incompatible content part(s) for ${provider}/${model}`);
    }
  }

  // Determine if we should preserve client-side cache_control headers
  // Fetch settings from DB to get user preference
  const cacheControlMode = await getCacheControlSettings().catch(() => "auto");
  ...
  if (preserveCacheControl) {
    log?.debug?.("CACHE", `Preserving client cache_control (client=${userAgent?.substring(0, 20)}, combo=${isCombo}, strategy=${comboStrategy}, provider=${provider})`);
  }
  ...
  const normalizeClaudeUpstreamMessages = (payload, options) => normalizeClaudeUpstreamMessagesFor(payload, options, log);
  try {
    if (nativeCodexPassthrough) {
      ...
      if (sourceFormat === FORMATS.CLAUDE && isClaudeCodeSemanticPassthrough) {
        ...
        const normalizeToolCallId = getModelNormalizeToolCallId(provider || "", model || "", sourceFormat);
        const preserveDeveloperRole = getModelPreserveOpenAIDeveloperRole(provider || "", model || "", sourceFormat);
        normalizedForCc = translateRequest(sourceFormat, FORMATS.OPENAI, model, {
          ...body
        }, stream, credentials, provider, reqLogger, {
          normalizeToolCallId,
      ...
      const ccRequestDefaults = getClaudeCodeCompatibleRequestDefaults(credentials?.providerSpecificData);
      ...
      if (provider === "claude") {
        translatedBody.messages = redactPassthroughThinkingSignatures(translatedBody.messages, DEFAULT_THINKING_CLAUDE_SIGNATURE);
      ...
      if (isClaudeCodeSemanticPassthrough) {
        ...
        if (Array.isArray(translatedBody.messages)) {
          translatedBody.messages = splitMisplacedToolResults(translatedBody.messages);
        }
      } else {
        normalizeClaudeUpstreamMessages(translatedBody, {
          preserveToolResultBlocks: true
      ...
      if (translatedBody.output_format !== undefined) {
        const oc = translatedBody.output_config && typeof translatedBody.output_config === "object" ? translatedBody.output_config : {};
      ...
      if (provider !== "claude" && translatedBody.output_config && typeof translatedBody.output_config === "object") {
        const oc = translatedBody.output_config;
        delete oc.format;
      ...
      if (provider?.startsWith("openai-compatible-") && Array.isArray(translatedBody.tools)) {
        ...
        translatedBody.tools = translatedBody.tools.filter(t => !t.type || t.type === "function" || !!t.function || !!t.name).map(t => {
        ...
        const dropped = before - translatedBody.tools.length;
        if (dropped > 0) {
          log?.debug?.("TOOLS", `Dropped ${dropped} unconvertible tool(s) for openai-compatible provider`);
        }
      }
      const normalizeToolCallId = getModelNormalizeToolCallId(provider || "", model || "", sourceFormat);
      const preserveDeveloperRole = getModelPreserveOpenAIDeveloperRole(provider || "", model || "", sourceFormat);
      translatedBody = translateRequest(sourceFormat, targetFormat, model, translatedBody, stream, credentials, provider, reqLogger, {
        normalizeToolCallId,
        preserveDeveloperRole,
        preserveCacheControl,
    ...
    try {
      const {
        runOnError
      } = await import("../stubs/lib/plugins/hooks");
      await runOnError({
        requestId: traceId,
        body,
        model,
      ...
      log?.debug?.("PLUGIN", `onError hook error (non-fatal): ${pluginErr instanceof Error ? pluginErr.message : String(pluginErr)}`);
    }
    const parsedStatus = Number(error?.statusCode);
    const statusCode = Number.isInteger(parsedStatus) && parsedStatus >= 400 && parsedStatus <= 599 ? parsedStatus : HTTP_STATUS.SERVER_ERROR;
  ...
  if (targetFormat === FORMATS.KIRO) {
    ...
    if (kiroTools) {
      const {
        tools: sanitizedKiroTools,
        nameMap: kiroNameMap
      } = sanitizeKiroTools(kiroTools);
      translatedBody.conversationState.currentMessage.userInputMessage.userInputMessageContext.tools = sanitizedKiroTools;
      if (kiroNameMap.size > 0) {
        const existing = translatedBody._toolNameMap instanceof Map ? translatedBody._toolNameMap : new Map();
  ...
  const nativeClaudeToolNameMap = isClaudePassthrough ? buildClaudePassthroughToolNameMap(body) : null;
  const toolNameMap = translatedToolNameMap instanceof Map && translatedToolNameMap.size > 0 ? translatedToolNameMap : nativeClaudeToolNameMap;
  ...
  translatedBody = previousResponseIdPolicy.body;
  ...
  if (provider === "codex" || provider?.startsWith("codex")) {
    ...
    if (hasEffortSuffix && translatedBody.output_config && typeof translatedBody.output_config === "object") {
      const oc = translatedBody.output_config;
      if (oc.effort) {
        log?.warn?.("PARAMS", `Stripped output_config.effort="${oc.effort}" because model "${finalModelToUpstream}" already encodes effort`);
        delete oc.effort;
        if (Object.keys(oc).length === 0) {
          delete translatedBody.output_config;
        }
  ...
  if (providerCap) {
    for (const field of ["max_tokens", "max_completion_tokens"]) {
      if (typeof translatedBody[field] === "number" && translatedBody[field] > providerCap) {
        log?.debug?.("PARAMS", `Capping ${field} from ${translatedBody[field]} to ${providerCap} for ${provider}`);
  ...
  const resolveExecutorWithProxy = prov => resolveExecutorWithProxyFor(prov, log);
  ...
  if (apiKeyInfo?.id && credentials?.connectionId) {
    try {
      const {
        enforceQuotaShare
      } = await import("../stubs/lib/quota/enforce");
      const decision = await enforceQuotaShare({
        ...
        log?.warn?.("QUOTA_SHARE", `enforceQuotaShare failed; fail-open: ${err instanceof Error ? err.message : String(err)}`);
        return {
          kind: "allow"
        };
      });
      if (decision.kind === "block") {
        const {
          buildErrorBody
        } = await import("../utils/error");
        log?.warn?.("QUOTA_SHARE", `[quotaShare] blocked apiKeyId=${apiKeyInfo.id} provider=${provider ?? "unknown"}: ${decision.reason}`);
        const headers = {
          "Content-Type": "application/json"
        };
      ...
      if (decision.kind === "allow" && decision.deprioritize) {
        ...
        log?.info?.("QUOTA_SHARE", `[quotaShare] soft deprioritize active for apiKeyId=${apiKeyInfo.id} provider=${provider ?? "unknown"}`);
      }
    } catch (err) {
      // Outer fail-open guard — should not be reached (inner .catch covers it)
      log?.warn?.("QUOTA_SHARE", `[quotaShare] enforceQuotaShare unexpected error; fail-open: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  if (quotaSoftDeprioritize && isCombo && comboStepId) {
    try {
      ...
      log?.warn?.("QUOTA_SHARE", `[quotaShare] could not set soft penalty on candidate: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  const getExecutionCredentials = () => resolveExecutionCredentialsFor({
    credentials,
    nativeCodexPassthrough,
    endpointPath,
    targetFormat,
    ...
    const execute = async () => {
      ...
      try {
        const rawResult = await (async () => {
          ...
          const maxAttempts = isModelScopeForRequest ? 3 : provider === "qwen" ? 3 : provider === "codex" ? 3 : 1;

          // ── Codex 429 account-rotation state ─────────────────────────────────
          // Track excluded connection IDs for codex failover across attempts.
          const codexExcludedIds = [];
          // Derive session affinity key once for codex failover (used to clear affinity on 429).
          const codexSessionAffinityKey = provider === "codex" ? extractSessionAffinityKey(body, clientRawRequest?.headers) ?? null : null;
          while (attempts < maxAttempts) {
            ...
            const releaseAccountSemaphore = accountSemaphoreKey && accountSemaphoreMaxConcurrency != null ? await acquireAccountSemaphore(accountSemaphoreKey, {
            ...
            try {
              ...
              const rawExecutorResult = await withRateLimit(provider, attemptConnectionId, modelToCall, async () => {
              ...
              if (provider === "qwen" && res.response.status === 429 && attempts < maxAttempts - 1) {
                const bodyPeek = await res.response.clone().text().catch(() => "");
              ...
              if (isModelScope() && res.response.status === 429 && attempts < maxAttempts - 1) {
                ...
                if (decision.retryable) {
                  ...
                  log?.warn?.("MODELSCOPE_RETRY", `429 ${decision.kind}; retrying in ${delay}ms (model remaining: ${decision.snapshot.modelRemaining ?? "unknown"})`);
                  releaseAccountSemaphore();
                  await new Promise(r => setTimeout(r, delay));
                  attempts++;
                  continue;
              ...
              if (provider === "codex" && comboStrategy !== "context-relay" && res.response.status === 429 && attempts < maxAttempts - 1) {
                const failedConnectionId = execCreds?.connectionId || credentials?.connectionId || connectionId;
                const normalizedHeaders = normalizeHeaders(res.response.headers);
                ...
                log?.warn?.("CODEX_FAILOVER", `429 on connection ${String(failedConnectionId).slice(0, 8)} (attempt ${attempts + 1}/${maxAttempts}), rotating account`);
                ...
                const nextCreds = await getProviderCredentials("codex", null, null, modelToCall || model || requestedModel || null, {
                ...
                log?.info?.("CODEX_FAILOVER", `Rotating codex account: ${String(failedConnectionId).slice(0, 8)} → ${newConnectionId.slice(0, 8)} (attempt ${attempts + 2}/${maxAttempts})`);
                logAuditEvent({
              ...
              if (stream) {
                ...
                if (okStatus) {
                  try {
                    ...
                    if (goalOverride && !sr.enabled) {
                      log?.info?.("AGENT_GOAL", `agentGoalPolicy override: stream recovery enabled for goal request requestId=${traceId} model=${modelToCall || model || requestedModel || "unknown"}`);
                    }
                  } catch {
                ...
                if (streamRecoveryEnabled) {
                  ...
                  const runUpstreamStream = async body => {
                    try {
                      ...
                      const retryOk = retryRes.response.status >= 200 && retryRes.response.status < 300;
                      if (retryOk && retryRes.response.body) {
                        return retryRes.response.body;
                      }
                  ...
                  const continueStream = continueMidStreamEnabled ? assistantSoFar => {
                    const continuationBody = makeContinuationBody(bodyToSend, assistantSoFar);
                    return continuationBody ? runUpstreamStream(continuationBody) : Promise.resolve(null);
                  } : undefined;
                  clientBody = createRecoverableStream(originalBody, () => runUpstreamStream(bodyToSend), {
                    finalize: releaseAccountSemaphore,
                    onRetry: (attempt, err) => log?.warn?.("STREAM_RECOVERY", `transparent early-retry ${attempt}/${STREAM_RECOVERY.EARLY_RETRY_MAX} after ${err?.name || "truncation"}`),
                    continueStream,
                    onContinue: attempt => log?.warn?.("STREAM_RECOVERY", `mid-stream continuation attempt ${attempt}/${STREAM_RECOVERY.EARLY_RETRY_MAX}`)
                  });
                } else {
        ...
        const payload = await readNonStreamingResponseBody(rawResult.response, contentType, upstreamStream);
  ...
  const registeredProviderRequest = translatedBody && typeof translatedBody === "object" && !Array.isArray(translatedBody) ? {
    ...translatedBody,
    model: typeof translatedBody.model === "string" ? translatedBody.model : effectiveModel,
    ...(!Array.isArray(translatedBody.messages) && Array.isArray(body.messages) ? {
  ...
  const msgCount = translatedBody.messages?.length || translatedBody.contents?.length || translatedBody.request?.contents?.length || (translatedBody.conversationState?.history?.length ?? 0) + (translatedBody.conversationState?.currentMessage ? 1 : 0) || 0;
  ...
  if (apiKeyInfo?.id) {
    try {
      ...
      if (tokenBreach) {
        ...
        return createErrorResult(HTTP_STATUS.RATE_LIMITED, `Token limit exceeded for ${scopeLabel}: ${tokenBreach.tokensUsed}/${tokenBreach.limitValue} tokens used in the current window. Please try again later.`, null, "TOKEN_LIMIT_EXCEEDED");
      }
    } catch (err) {
  ...
  try {
    ...
    claudePromptCacheLogMeta = buildClaudePromptCacheLogMeta(targetFormat, finalBody, providerHeaders, clientRawRequest?.headers);
    ...
    updateFromHeaders(provider, responseConnectionId, providerResponse.headers, providerResponse.status, model);

    // Store rate-limit headers for quota saturation signals
    try {
      const {
        storeRateLimitHeaders
      } = await import("../stubs/lib/quota/saturationSignals");
    ...
    if (isSemaphoreCapacityError(error)) {
      ...
      const result = stream ? createStreamingErrorResult(HTTP_STATUS.RATE_LIMITED, failureMessage, error.code) : createErrorResult(HTTP_STATUS.RATE_LIMITED, failureMessage);
      return {
        ...result,
    ...
    const failureStatus = error.name === "AbortError" ? 499 : error.name === "TimeoutError" || error.name === "BodyTimeoutError" ? HTTP_STATUS.GATEWAY_TIMEOUT : error.status && typeof error.status === "number" ? error.status : HTTP_STATUS.BAD_GATEWAY;
    const failureMessage = error.name === "AbortError" ? "Request aborted" : formatProviderError(error, provider, model, failureStatus);
    ...
    const isOwnDeadlineTimeout = failureStatus === HTTP_STATUS.GATEWAY_TIMEOUT && (error.name === "TimeoutError" || error.name === "BodyTimeoutError");
    const upstreamErrorType = upstreamErrorCode === ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE || isOwnDeadlineTimeout ? "upstream_timeout" : failureStatus === 401 ? "authentication_error" : undefined;
    appendRequestLog({
      model,
      provider,
      connectionId,
    ...
    persistFailureUsage(failureStatus, upstreamErrorCode || (error instanceof Error && error.name ? error.name : "upstream_error"));
    console.log(`${COLORS.red}[ERROR] ${failureMessage}${COLORS.reset}`);
    if (stream && upstreamErrorCode) {
      const result = createStreamingErrorResult(failureStatus, failureMessage, upstreamErrorCode, upstreamErrorType);
      return {
        ...result,
    ...
    return createErrorResult(failureStatus, failureMessage, null, upstreamErrorCode, upstreamErrorType);
  }
  ...
  const errorMessageForToolDetection = typeof upstreamErrorBody === "string" ? upstreamErrorBody : JSON.stringify(upstreamErrorBody ?? {});
  ...
  const isQwenExpiredError = provider === "qwen" && parsedStatusCode === HTTP_STATUS.BAD_REQUEST && parsedMessage?.toLowerCase().includes("session has expired");
  ...
  const hadStreamOptions = targetFormat === FORMATS.OPENAI_RESPONSES && "stream_options" in translatedBody;
  if (hadStreamOptions) {
    delete translatedBody.stream_options;
  }
  ...
  if ((providerResponse.status === HTTP_STATUS.UNAUTHORIZED || providerResponse.status === HTTP_STATUS.FORBIDDEN || isQwenExpiredError) && !hadStreamOptions && hasRefreshCapability // Skip refresh if failure may be from stream_options removal, not auth
    ...
    const attemptedRefreshToken = typeof credentials?.refreshToken === "string" ? credentials.refreshToken : null;
    let persistFnRan = false;
    const persistFn = onCredentialsRefreshed ? async refreshResult => {
      persistFnRan = true;
    ...
    const casConnectionId = typeof credentials?.connectionId === "string" ? credentials.connectionId.trim() : "";
    const casReread = casConnectionId ? async () => {
    ...
    const newCredentials = await refreshWithRetry(() => runWithCasGuard(casReread ? {
      expectedRefreshToken: attemptedRefreshToken,
      reread: casReread
    } : null, () => runWithOnPersist(persistFn, () => executor.refreshCredentials(credentials, log))), 3, log, provider // Explicitly pass the provider to avoid universally tripping the "unknown" circuit breaker
    );
    if (newCredentials?.accessToken || newCredentials?.copilotToken) {
      ...
      try {
        ...
        const retryResult = await runWithCapture(providerRequestCapture, () => executor.execute({
          model: retryModelId,
          body: translatedBody,
          stream: upstreamStream,
        ...
        log?.error?.("TOKEN", `${provider?.toUpperCase()} | retry after refresh failed: ${sanitizeErrorMessage(retryErr)}`);
      }
    } else {
        ...
        if (typeof connectionId === "string" && connectionId && attemptedRefreshToken) {
          try {
            ...
            if (wasRefreshTokenRotated(attemptedRefreshToken, latest?.refreshToken)) {
              alreadyRotated = true;
              log?.warn?.("TOKEN", `${provider.toUpperCase()} | refresh_token already rotated by a concurrent refresh — keeping connection active`);
            }
          } catch {
  ...
  if (!providerResponse.ok) {
    ...
    if (statusCode === 429 && isModelScope()) {
      ...
      errorType = decision.kind === "quota_exhausted" ? PROVIDER_ERROR_TYPES.QUOTA_EXHAUSTED : PROVIDER_ERROR_TYPES.RATE_LIMITED;
      log?.warn?.("MODELSCOPE_429", `${decision.kind} (model remaining: ${decision.snapshot.modelRemaining ?? "unknown"}, total remaining: ${decision.snapshot.totalRemaining ?? "unknown"})`);
    }
    ...
    if (errorConnectionId && errorType) {
      try {
        if (errorType === PROVIDER_ERROR_TYPES.FORBIDDEN) {
          ...
          console.warn(`[provider] Node ${errorConnectionId} banned (${statusCode}) — disabling permanently`);
          ...
          if (connectionHasExtraKeys(errorConnectionId, credentials?.providerSpecificData?.extraApiKeys)) {
            await updateProviderConnection(errorConnectionId, {
              lastErrorType: errorType,
              lastError: message,
              errorCode: statusCode
            });
            console.warn(`[provider] Node ${errorConnectionId} account deactivated (${statusCode}) — has extra keys, keeping connection active`);
          } else {
            await updateProviderConnection(errorConnectionId, {
              isActive: false,
            ...
            console.warn(`[provider] Node ${errorConnectionId} account deactivated (${statusCode}) — disabling permanently`);
          }
          ...
          console.warn(`[provider] Node ${errorConnectionId} OAuth token invalid (${statusCode}) — token refresh available`);
          ...
          console.warn(`[provider] Node ${errorConnectionId} project routing error (${statusCode}) — not banning`);
        }
      } catch {
    ...
    reqLogger.logProviderResponse(providerResponse.status, providerResponse.statusText, providerResponse.headers, upstreamErrorBody);
    ...
    if (isModelUnavailableError(statusCode, message)) {
      ...
      if (nextModel) {
        ...
        try {
          ...
          if (fallbackResult.response.ok) {
            ...
            return createErrorResult(statusCode, errMsg, retryAfterMs, upstreamErrorCode, upstreamErrorType, upstreamErrorBody);
          }
        } catch {
          persistAttemptLogs({
            status: statusCode,
      ...
      const familyCandidates = getModelFamily(currentModel).filter(m => m !== currentModel && !triedModels.has(m));
      const nextModel = findLargerContextModel(currentModel, familyCandidates) ?? getNextFamilyFallback(currentModel, triedModels);
      if (nextModel) {
        triedModels.add(nextModel);
        currentModel = nextModel;
        translatedBody.model = nextModel;
        ...
        try {
          ...
          if (fallbackResult.response.ok) {
            ...
            log?.info?.("CONTEXT_OVERFLOW_FALLBACK", `Serving ${nextModel} as fallback for ${model}`);
          } else {
            persistAttemptLogs({
              status: statusCode,
              error: errMsg,
  ...
  if (!stream) {
    ...
    if (isEmptyContentResponse(responseBody)) {
      ...
      if (nextModel) {
        ...
        currentModel = nextModel;
        translatedBody.model = nextModel;
        log?.info?.("EMPTY_CONTENT_FALLBACK", `${model} returned empty content → trying ${nextModel}`);
        try {
          const fallbackResult = await executeProviderRequest(nextModel, false);
          if (fallbackResult.response.ok) {
            const fallbackRaw = await withBodyTimeout(fallbackResult.response.text());
            try {
    ...
    const responseToolNameMap = mergeResponseToolNameMap(toolNameMap, finalBody ?? null);
    if (sourceFormat === FORMATS.CLAUDE && targetFormat === FORMATS.CLAUDE) {
      responseBody = restoreClaudePassthroughToolNames(responseBody, responseToolNameMap);
    }
    reqLogger.logProviderResponse(providerResponse.status, providerResponse.statusText, providerResponse.headers, looksLikeSSE ? {
      _streamed: true,
      _format: "sse-json",
    ...
    if (usage && typeof usage === "object") {
      attachCompressionUsageReceiptAfterAnalytics(usage, "provider");
    }
    ...
    if (sourceFormat === "claude" && !stream) {
      if (typeof translatedResponse?.choices?.[0]?.message?.content === "string") {
        translatedResponse.choices[0].message.content = stripMarkdownCodeFence(translatedResponse.choices[0].message.content);
      }
    }
    ...
    if (translatedResponse?.choices) {
      for (const choice of translatedResponse.choices) {
        if (choice.message?.tool_calls && choice.message.tool_calls.length > 0 && choice.finish_reason !== "tool_calls") {
    ...
    const customSkillExecutionEnabled = Boolean(memoryOwnerId) && memorySettings?.skillsEnabled === true;
    ...
    if (customSkillExecutionEnabled || builtinToolNames.length > 0) {
      ...
      translatedResponse = await handleToolCallExecution(translatedResponse, getSkillsModelIdForFormat(sourceFormat), {
    ...
    const postCallGuardrails = await guardrailRegistry.runPostCallHooks(translatedResponse, guardrailContext);
    translatedResponse = postCallGuardrails.response;
    const responseUsage = (usage && typeof usage === "object" ? usage : null) || (translatedResponse?.usage && typeof translatedResponse.usage === "object" ? translatedResponse.usage : null);
    const estimatedCost = responseUsage ? await calculateCost(provider, model, responseUsage, {
    ...
    await emitRequestGamificationEvent({
      apiKeyId: apiKeyInfo?.id,
      model,
      provider
    });
  ...
  providerResponse = await maybeConvertJsonBodyToSse(providerResponse, {
    log,
    provider,
    model
  });
  const streamReadinessPolicy = resolveStreamReadinessTimeout({
    ...
    maxTimeoutMs: agentGoalPolicy.detected ? Math.max(STREAM_READINESS_MAX_TIMEOUT_MS, agentGoalPolicy.readinessMaxTimeoutMs) : STREAM_READINESS_MAX_TIMEOUT_MS
  });
  if (streamReadinessPolicy.timeoutMs !== streamReadinessPolicy.baseTimeoutMs) {
    log?.debug?.("STREAM", `adaptive readiness timeout=${streamReadinessPolicy.timeoutMs}ms base=${streamReadinessPolicy.baseTimeoutMs}ms reason=${streamReadinessPolicy.reasons.join(",")}`);
  }
  ...
  onClientDisconnectFinalize = event => handleStreamFailure({
    status: 499,
  ...
  const needsResponsesTranslation = targetFormat === FORMATS.OPENAI_RESPONSES && clientResponseFormat === FORMATS.OPENAI && !isResponsesEndpoint && !isDroidCLI;
  ...
  if (needsResponsesTranslation) {
    ...
    transformStream = createSSETransformStreamWithLogger("openai-responses", "openai", provider, reqLogger, responseToolNameMap, model, connectionId, streamStateBody, onStreamComplete, apiKeyInfo, handleStreamFailure, copilotCompatibleReasoning);
    ...
    transformStream = createSSETransformStreamWithLogger(targetFormat, clientResponseFormat, provider, reqLogger, responseToolNameMap, model, connectionId, streamStateBody, onStreamComplete, apiKeyInfo, handleStreamFailure, copilotCompatibleReasoning,
    ...
    transformStream = createPassthroughStreamWithLogger(provider, reqLogger, responseToolNameMap, model, connectionId, streamStateBody, onStreamComplete, apiKeyInfo, handleStreamFailure, clientResponseFormat);
  }
