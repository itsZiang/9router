...
import { getTranscriptionProvider, parseTranscriptionModel } from "../config/audioRegistry";
import { buildAuthHeaders } from "../config/registryUtils";
import { kieExecutor } from "../executors/kie";
import { vertexTranscribe } from "../executors/vertexMedia";
...
async function handleDeepgramTranscription(providerConfig, file, modelId, token, formData) {
  ...
  return Response.json({
    text: text ?? "",
    noSpeechDetected: text === null || text === ""
  }, {
    headers: {
      ...CORS_HEADERS
    }
...
async function pollKieTranscriptionResult(baseUrl, modelId, taskId, token) {
  ...
  try {
    ...
    if (state === "success") {
      const text = data?.data?.response?.text || data?.data?.resultText || data?.data?.text || data?.text || "";
      return Response.json({
        text
      }, {
        headers: {
    ...
    return errorResponse(status, err instanceof Error ? err.message : "Kie transcription generation timed out or failed");
  }
...
export async function handleAudioTranscription({
  ...
  if (!providerConfig) {
    return errorResponse(400, `No transcription provider found for model "${model}". Available: openai, groq, deepgram, assemblyai, nvidia, huggingface, qwen`);
  }
  ...
  if (providerConfig.format === "vertex-gemini") {
    try {
      ...
      const error = err;
      return errorResponse(typeof error?.status === "number" ? error.status : 500, `Vertex transcription failed: ${error?.message || "unknown error"}`);
    }
  }
  ...
  const extraFields = {};
  for (const key of ["language", "prompt", "response_format", "temperature", "timestamp_granularities[]"]) {
