...
import { getVideoProvider, parseVideoModel } from "../config/videoRegistry";
import { kieExecutor } from "../executors/kie";
import { vertexGenerateVideo } from "../executors/vertexMedia";
import { handleGoogleFlowVideoGeneration } from "./videoGeneration/googleFlowHandler";
import { getExecutor } from "../executors/index";
import { isJsonObject, parseKieResultJson } from "../utils/kieTask";
import { buildRunwayApiUrl, buildRunwayHeaders, RUNWAYML_IMAGE_REQUIRED_MODELS } from "../config/runway";
import { submitComfyWorkflow, pollComfyResult, fetchComfyOutput, extractComfyOutputFiles } from "../utils/comfyuiClient";
...
export async function handleVideoGeneration({
  ...
  if (providerConfig.format === "google-flow") {
    return handleGoogleFlowVideoGeneration({
      model,
      providerConfig,
      body,
      credentials,
  ...
  if (providerConfig.format === "kie-video") {
    return handleKieVideoGeneration({
      model,
      provider,
      providerConfig,
      body,
  ...
  if (providerConfig.format === "veoaifree-web") {
    return handleVeoAiFreeVideoGeneration({
      model,
      provider,
      body,
      credentials,
...
async function handleDashscopeVideoGeneration({
  ...
  if (!token) {
    return {
      success: false,
      status: 401,
      error: "Alibaba DashScope API key is required"
    };
  }
  ...
  if (log) {
    log.info("VIDEO", `${provider}/${model} (dashscope-video) | prompt: "${prompt.slice(0, 60)}..."`);
  }
  try {
 ...
 */
async function handleVertexVeoGeneration({
  model,
  body,
  credentials,
  log
  ...
  try {
    const aspectRatio = typeof body.aspect_ratio === "string" ? body.aspect_ratio : typeof body.aspectRatio === "string" ? body.aspectRatio : typeof body.size === "string" ? body.size : undefined;
 ...
 */
async function handleVeoAiFreeVideoGeneration({
  model,
  provider,
  body,
  credentials,
...
async function handleComfyUIVideoGeneration({
  ...
  try {
    ...
    for (const file of outputFiles) {
      const buffer = await fetchComfyOutput(providerConfig.baseUrl, file.filename, file.subfolder, file.type);
  ...
  try {
    ...
    if (data.video) {
      ...
      for (const img of data.images) {
        videos.push({
          b64_json: typeof img === "string" ? img : img.image,
          format: "mp4"
        });
      }
    }
...
async function handleKieVideoGeneration({
  ...
  try {
    ...
    if (!taskId) {
      const errorMessage = createData?.msg || createData?.message || createData?.error || "KIE video generation did not return taskId";
      if (log) {
...
async function handleRunwayVideoGeneration({
  ...
  if (!token) {
    return {
      success: false,
      status: 400,
      error: "No credentials for Runway provider"
    };
  }
  ...
  const submitUrl = buildRunwayApiUrl(useImageToVideo ? "/image_to_video" : "/text_to_video", providerConfig.baseUrl);
  ...
  if (log) {
    const promptPreview = String(body.prompt ?? "").slice(0, 60);
    log.info("VIDEO", `${provider}/${model} (runway ${useImageToVideo ? "image_to_video" : "text_to_video"}) | prompt: "${promptPreview}..."`);
  }
  try {
    ...
    if (!submitResponse.ok) {
      ...
      if (log) {
        log.error("VIDEO", `${provider} submit error ${submitResponse.status}: ${errorText.slice(0, 200)}`);
      }
      saveCallLog({
        method: "POST",
        path: "/v1/videos/generations",
    ...
    let lastTask = null;
    while (Date.now() < deadline) {
      const taskResponse = await fetch(buildRunwayApiUrl(`/tasks/${encodeURIComponent(taskId)}`, providerConfig.baseUrl), {
        method: "GET",
        headers
      });
      if (!taskResponse.ok) {
        ...
        if (log) {
          log.error("VIDEO", `${provider} poll error ${taskResponse.status}: ${errorText.slice(0, 200)}`);
        }
        saveCallLog({
          method: "POST",
          path: "/v1/videos/generations",
    ...
    const timeoutError = `Runway task timeout after ${timeoutMs}ms (taskId=${taskId}, status=${String(lastTask?.status || "unknown")})`;
    saveCallLog({
      method: "POST",
      path: "/v1/videos/generations",
      status: 504,
...
const RUNWAY_TERMINAL_FAILURE_STATUSES = new Set(["FAILED", "CANCELED", "CANCELLED", "ABORTED", "DELETED"]);
function resolveRunwayPromptImage(body) {
  const directCandidates = [body.promptImage, body.prompt_image, body.image, body.image_url, body.imageUrl, body.provider_options?.promptImage, body.provider_options?.prompt_image];
  for (const candidate of directCandidates) {
    if (typeof candidate === "string" && candidate.trim()) return candidate.trim();
    if (candidate && typeof candidate === "object") return candidate;
    if (Array.isArray(candidate) && candidate.length > 0) return candidate;
  }
  const arrayCandidates = [body.imageUrls, body.image_urls, body.provider_options?.imageUrls, body.provider_options?.image_urls];
...
function extractRunwayOutputUrls(task) {
  const rawOutput = Array.isArray(task?.output) ? task.output : Array.isArray(task?.result) ? task.result : [];
...
function extractRunwayFailureMessage(task) {
  const directCandidates = [task?.failure, task?.failureReason, task?.error, task?.errorMessage, task?.message];
  for (const candidate of directCandidates) {
    if (typeof candidate === "string" && candidate.trim()) return candidate.trim();
  }
  if (task?.failure && typeof task.failure === "object") {
    const nestedCandidates = [task.failure.message, task.failure.reason, task.failure.error, task.failure.code];
...
async function normalizeRunwayVideoResult(task, body) {
  ...
  if (urls.length === 0) {
    throw new Error(`Runway task completed without output URLs: ${JSON.stringify(task).slice(0, 400)}`);
  }
