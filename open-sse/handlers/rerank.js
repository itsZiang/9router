...
import { getRerankProvider, parseRerankModel, RERANK_PROVIDERS } from "../config/rerankRegistry";
import { errorResponse } from "../utils/error";
import { attachOmniRouteMetaHeaders } from "../stubs/domain/omnirouteResponseMeta";
import { calculateModalCost } from "../stubs/lib/usage/costCalculator";
...
export function transformRequestForProvider(providerConfig, body) {
  ...
  if (providerConfig.format === "deepinfra") {
    return {
      queries: [body.query],
      documents: (body.documents || []).map(doc => typeof doc === "string" ? doc : doc.text || "")
    };
  }
...
export async function handleRerank({
  ...
  if (!providerConfig) {
    ...
    return errorResponse(400, `No rerank provider found for model "${model}". Available: ${availableProviders}`);
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return errorResponse(res.status, errData.message || errData.error?.message || `Provider returned HTTP ${res.status}`);
    }
    ...
    const costUsd = await calculateModalCost("rerank", providerId, modelId, {
      searchUnits
    });
    saveCallLog({
      method: "POST",
