import { requiresReasoningReplay } from "../../services/reasoningCache";
...
const NUMERIC_SCHEMA_FIELDS = ["minimum", "maximum", "exclusiveMinimum", "exclusiveMaximum", "minLength", "maxLength", "minItems", "maxItems", "minProperties", "maxProperties", "multipleOf"];
...
function mapRecordValues(record) {
  return Object.fromEntries(Object.entries(record).map(([key, value]) => [key, coerceSchemaNumericFields(value)]));
}
...
export function sanitizeToolDescription(tool) {
  ...
  if (Array.isArray(result.functionDeclarations)) {
    result.functionDeclarations = result.functionDeclarations.map(declaration => {
      ...
      return description === undefined ? declaration : {
        ...declaration,
        description
      };
    });
...
export function injectEmptyReasoningContentForToolCalls(messages, provider, model) {
  ...
  return messages.map(message => {
    ...
    if (message.role !== "assistant" || !Array.isArray(message.tool_calls) || message.tool_calls.length === 0 || message.reasoning_content !== undefined) {
      return message;
    }
    return {
      ...message,
...
const SCHEMA_SLOT_KEYS = ["items", "additionalProperties", "propertyNames", "contains", "not", "if", "then", "else", "unevaluatedProperties", "additionalItems"];
...
export function stripInvalidSchemaConstructs(schema) {
  if (Array.isArray(schema)) {
...
export function sanitizeClaudeToolSchemas(tools) {
  if (!Array.isArray(tools)) return tools;
  return tools.map(tool => {
