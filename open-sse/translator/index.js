...
import { ensureToolCallIds, fixMissingToolResponses, stripOrphanedToolResults } from "./helpers/toolCallHelper";
import { NON_ANTHROPIC_THINKING_PLACEHOLDER, prepareClaudeRequest } from "./helpers/claudeHelper";
import { filterToOpenAIFormat } from "./helpers/openaiHelper";
import { providerHonorsOpenAIFormatCacheControl } from "../utils/cacheControlPolicy";
import { coerceToolSchemas, injectEmptyReasoningContentForToolCalls, sanitizeToolDescriptions } from "./helpers/schemaCoercion";
import { getRequestTranslator, getResponseTranslator } from "./registry";
import { bootstrapTranslatorRegistry } from "./bootstrap";
import { hasThinkingConfig, normalizeThinkingConfig } from "../services/provider";
import { applyThinkingBudget } from "../services/thinkingBudget";
import { getResolvedModelCapabilities, supportsReasoning } from "../services/modelCapabilities";
import { normalizeRoles } from "../services/roleNormalizer";
import { lookupReasoning, recordReplay, requiresReasoningReplay } from "../services/reasoningCache";
...
function getReasoningCacheRequestId(body) {
  if (!body || typeof body !== "object") return "";
  const requestId = body._reasoningCacheRequestId ?? body.reasoningCacheRequestId ?? body.request_id ?? body.requestId;
  return typeof requestId === "string" ? requestId.trim() : "";
}
function getAssistantMessageCacheKey(body, messageIndex) {
...
function isReasoningOnlyReplayTarget(provider, model) {
  ...
  return normalizedProvider === "deepseek" || /(^|\/)deepseek/i.test(normalizedModel) || normalizedProvider === "xiaomi-mimo" || /(^|\/)mimo/i.test(normalizedModel);
}
...
export function translateRequest(sourceFormat, targetFormat, model, body, stream = true, credentials = null, provider = null, reqLogger = null, options) {
  let result = body;
  ...
  if (result.messages && Array.isArray(result.messages)) {
    result.messages = normalizeRoles(result.messages, provider || "", model || "", targetFormat, preserveDeveloperRole);
  }
  ...
  if (sourceFormat !== targetFormat) {
    ...
    if (directTranslator && sourceFormat !== FORMATS.OPENAI && targetFormat !== FORMATS.OPENAI) {
      ...
      const directCredentials = provider != null ? {
      ...
      if (sourceFormat !== FORMATS.OPENAI) {
        ...
        if (toOpenAI) {
          ...
          // stripped.
          const preserveCacheControl = options?.preserveCacheControl === true && providerHonorsOpenAIFormatCacheControl(provider);
          const step1Credentials = options?.copilotClient || hasTargetHint || preserveCacheControl ? {
            ...(credentials && typeof credentials === "object" ? credentials : {}),
      ...
      if (targetFormat !== FORMATS.OPENAI) {
        ...
        if (fromOpenAI) {
          ...
          const translationCredentials = hasNs || hasPreCompression || hasCopilot || hasProvider ? {
            ...(credentials && typeof credentials === "object" ? credentials : {}),
            ...(hasNs ? {
              _signatureNamespace: options.signatureNamespace
            } : {}),
            ...(hasPreCompression ? {
              _preCompressionBody: options.preCompressionBody
            } : {}),
            ...(hasCopilot ? {
              _copilotClient: true
            } : {}),
  ...
  if (targetFormat === FORMATS.OPENAI) {
    ...
    result = filterToOpenAIFormat(result, {
      preserveCacheControl: options?.preserveCacheControl === true && providerHonorsOpenAIFormatCacheControl(provider),
  ...
  if (sourceFormat === FORMATS.OPENAI_RESPONSES && result.messages && Array.isArray(result.messages)) {
    result.messages = normalizeRoles(result.messages, provider || "", model || "", targetFormat, preserveDeveloperRole);
  }
  ...
  if (isReasoner && result.messages && Array.isArray(result.messages)) {
    ...
    for (const [messageIndex, msg] of result.messages.entries()) {
      ...
      const hasToolUseBlocks = !hasToolCalls && Array.isArray(msg.content) && msg.content.some(b => b?.type === "tool_use");
      ...
      const shouldReplayReasoningOnly = !hasToolCalls && !hasToolUseBlocks && canReplayReasoningOnly && !hasNonEmptyReasoningContent(msg);
      ...
      if (hasToolUseBlocks) {
        ...
        const hasThinkingBlock = msg.content.some(b => b?.type === "thinking" || b?.type === "redacted_thinking");
      ...
      const cacheKey = hasToolCalls ? msg.tool_calls[0]?.id : getAssistantMessageCacheKey(result, 0);
      if (cacheKey) {
