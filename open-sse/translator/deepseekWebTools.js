...
import { parseToolCallsFromText, parseLooseJsonObject, getRequestedToolNames, resolveRequestedToolName, toArgumentsString, stripRanges } from "./webTools";
 ...
 */
export function serializeDeepSeekToolPrompt(tools) {
  ...
  return ["You can call tools. To call a tool, output ONLY this exact block (no markdown fence):", '<tool>{"name": "<tool_name>", "arguments": { ... }}</tool>', "Rules:", "- Use exactly <tool>...</tool>. Do NOT use <tool:name>, <tool_call>, <name>, <parameter>, id=/name= attributes, or code fences.", '- "name" must be one of the tools below; "arguments" must be a JSON object.', "- When a tool is needed, emit the <tool> block instead of only describing the plan.", "- Emit one <tool> block per call; you may put several blocks back to back.", "- If no tool is needed, just answer normally without any <tool> block.", "", "Available tools:", ...lines].join("\n");
}
 ...
 */
export function buildToolConversationPrompt(messages, toolSystemPrompt) {
  const systemParts = [];
  ...
  if (sawToolActivity) {
    ...
    parts.push("Continue the task using the tool results above. Do NOT repeat tool calls that already " + "succeeded; perform the next step or give the final answer.");
  }
  return parts.join("\n\n").replace(/!\[.*?\]\(.*?\)/g, "");
}
...
function extractCall(tagName, innerRaw, requested, schemaMap) {
  ...
  if (argsChild) {
    ...
    if (json.arguments !== undefined) argsValue = json.arguments;else if (json.params !== undefined) argsValue = json.params;else if (nameFromTag) {
 ...
 */
export function parseDeepSeekToolCalls(text, idSeed = "call", requestedTools) {
  ...
  const ranges = [...acceptedRanges, ...tokens.filter(t => !within(t)).map(t => ({
    start: t.start,
    end: t.end
  }))];
  return {
    content: stripRanges(text, ranges),
    toolCalls
  };
}