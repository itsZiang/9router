...
export default function ExposeModelsPage() {
  ...
  const handleCreate = async (data) => {
    try {
      const res = await fetch("/api/expose-combos", {
        method: "POST",
  ...
  const handleUpdate = async (id, data) => {
    try {
      const res = await fetch(`/api/expose-combos/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const handleDelete = async (id) => {
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(`/api/expose-combos/${id}`, { method: "DELETE" });
          if (res.ok) {
