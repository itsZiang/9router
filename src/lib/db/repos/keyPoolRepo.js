...
export async function addKeysToPool(provider, keys) {
  ...
  db.transaction(() => {
    // Get existing keys in pool for this provider to dedup
    const existing = new Set(
      db.all(`SELECT key FROM keyPool WHERE provider = ?`, [provider]).map((r) => r.key)
    );

    for (const k of keys) {
      if (!k.key || existing.has(k.key)) {
        skipped++;
        continue;
      }
      db.run(
        `INSERT INTO keyPool(id, provider, name, key, createdAt) VALUES(?, ?, ?, ?, ?)`,
...
// --- Per-provider settings via kv table ---

async function kvGet(key) {
  const db = await getAdapter();
  const row = db.get(`SELECT value FROM kv WHERE scope = ? AND key = ?`, [KV_SCOPE, key]);
